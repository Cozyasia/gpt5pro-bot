...
PATCH_VERSION = "v35-photo-background-flow-2026-05-25"
...
PATCH_VERSION = "v34-medical-analysis-fixed-2026-05-25"
...
async def _pedit_removebg(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes):
    ...
    try:
        await context.bot.send_chat_action(update.effective_chat.id, ChatAction.UPLOAD_PHOTO)
        fg = _subject_rgba_from_rembg(img_bytes)
        ...
        await update.effective_message.reply_text(
            "❌ Не удалось удалить фон. Чаще всего причина — rembg/onnxruntime не установились на Render или фото слишком сложное для сегментации."
        )
...
async def _pedit_replacebg_apply(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, prompt: str, label: str = "новый фон"):
    ...
    try:
        ...
        await update.effective_message.reply_text("❌ Не удалось заменить фон. Проверьте rembg/onnxruntime и OPENAI_IMAGE_KEY для генерации фона.")
...
async def on_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "bg:back":
        ...
        await q.edit_message_text("Фото-мастерская. Выберите действие:", reply_markup=photo_quick_actions_kb())
        return

    if data == "bg:custom":
        await q.answer("Свой фон")
