...
PATCH_VERSION = "v33.7.1-bg-photoroom-realbg-2026-07-06"
...
async def cmd_diag_bg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines.append(f"PHOTOROOM_INPUT_MAX_SIDE={PHOTOROOM_INPUT_MAX_SIDE} BG_OUTPUT_MAX_SIDE={BG_OUTPUT_MAX_SIDE}")
...
async def _pedit_replacebg(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, kind: str = "blur", prompt: str = ""):
    if Image is None:
        ...
        return
    try:
        await update.effective_message.reply_text("🖼 Вырезаю объект и заменяю только фон, без перерисовки человека/предмета. Для пресетов использую фотореалистичный фон.")
