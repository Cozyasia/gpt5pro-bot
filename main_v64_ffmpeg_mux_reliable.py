...
PATCH_VERSION = "v64-full-music-clip-ffmpeg-mux-reliable-2026-07-10"
...
def _mux_video_audio_sync(video_bytes: bytes, audio_bytes: bytes | None, target_duration_s: int) -> bytes | None:
    """Build one MP4 from Kling video + Suno audio.
    v64: fail-fast, copy-first remux, fallback to ultrafast re-encode.
    """
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            out_re = os.path.join(td, "final_reencode.mp4")
            ...
            last_err = ""
            for name, cmd, out_path in attempts:
                left = max(20, timeout_s - int(time.time() - started))
                log.info("ffmpeg mux attempt=%s video=%s audio=%s duration=%s timeout_left=%s", name, len(video_bytes), len(audio_bytes), target_duration_s, left)
                try:
...
async def _start_photo_music_clip(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, user_prompt: str):
    ...
    async def _photo_clip_job():
        ...
        try:
            ...
            await update.effective_message.reply_text(f"🎧 Музыка получена. Склеиваю один итоговый MP4 локально через ffmpeg, лимит ~{FFMPEG_MUX_TIMEOUT_S} сек…")
...
async def cmd_diag_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    lines = [
        ...
        f"• ffmpeg mux: timeout={FFMPEG_MUX_TIMEOUT_S}s copy_first={'✅' if FFMPEG_MUX_COPY_FIRST else '—'} preset={FFMPEG_MUX_REENCODE_PRESET} audio_bitrate={FFMPEG_MUX_AUDIO_BITRATE}",
