...
PATCH_VERSION = "v40-comet-creative-router-reels-menu-2026-05-27"
...
SEEDANCE_ENABLED = os.environ.get("SEEDANCE_ENABLED", "1").strip().lower() not in ("0", "false", "no", "off")
...
SEEDANCE_MODEL = os.environ.get("SEEDANCE_MODEL", "seedance-2.0-fast").strip() or "seedance-2.0-fast"
...
async def _run_comet_model_text_video(update: Update, context: ContextTypes.DEFAULT_TYPE, engine: str, prompt: str, duration_s: int, aspect: str) -> bool:
    engine = (engine or "").lower().strip()
    ...
    return await _create_and_poll_i2v(update, COMET_BASE_URL, COMET_API_KEY, payloads, status_paths, caption)
...
async def _run_creative_video(update: Update, context: ContextTypes.DEFAULT_TYPE, engine: str, prompt: str, duration: int, aspect: str, images: list[bytes] | None = None, intent: str = "text_video"):
    ...
    if images and engine in {"seedance", "veo", "hailuo", "vidu", "grok"}:
        ...
        await _create_and_poll_i2v(update, COMET_BASE_URL, COMET_API_KEY, payloads, [status, "/v1/videos/{id}", "/v1/tasks/{id}"], f"{engine.title()} image→video")
        return
    if engine == "sora":
        await _run_comet_text_video(update, context, "sora", prompt, duration, aspect)
        return
    ...
    if engine in {"seedance", "veo", "hailuo", "vidu", "grok"}:
        await _run_comet_model_text_video(update, context, engine, prompt, duration, aspect)
        return
...
def main_keyboard():
    return ReplyKeyboardMarkup(
        [
            [KeyboardButton("🎬 Видео / Reels"), KeyboardButton("🖼 Фото / Дизайн")],
            [KeyboardButton("🗣 Голос / Аудио"), KeyboardButton("📄 Документы")],
            [KeyboardButton("🤖 GPT‑чат"), KeyboardButton("🏡 Бизнес")],
            [KeyboardButton("💼 Мои проекты"), KeyboardButton("⚙️ Баланс / Настройки")],
        ],
        resize_keyboard=True,
        one_time_keyboard=False,
        selective=False,
...
def _home_inline_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🎬 Видео / Reels", callback_data="nav:video"), InlineKeyboardButton("🖼 Фото", callback_data="nav:photo")],
...
def _video_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🧠 AI Reels Studio", callback_data="creative:reels")],
        [InlineKeyboardButton("🪄 Оживить фото", callback_data="creative:revive"), InlineKeyboardButton("📝 Текст → видео", callback_data="creative:t2v")],
        [InlineKeyboardButton("🧩 Фотоальбом → Reels", callback_data="creative:multi_reels")],
        [InlineKeyboardButton("🗣 Говорящий аватар", callback_data="creative:avatar"), InlineKeyboardButton("👄 Lip-sync", callback_data="creative:lipsync")],
        [InlineKeyboardButton("💬 Субтитры", callback_data="creative:subtitles"), InlineKeyboardButton("🎵 Музыка", callback_data="creative:music")],
        [InlineKeyboardButton("🖼 Обложка", callback_data="creative:cover"), InlineKeyboardButton("📲 Пост", callback_data="creative:post")],
...
def _photo_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🎨 Сгенерировать", callback_data="pedit:lumaimg"), InlineKeyboardButton("🪄 Анализ", callback_data="pedit:vision")],
        [InlineKeyboardButton("🧼 Удалить фон", callback_data="pedit:removebg"), InlineKeyboardButton("🌈 Заменить фон", callback_data="pedit:replacebg")],
        [InlineKeyboardButton("🧽 Убрать объект", callback_data="pedit:retouch"), InlineKeyboardButton("🖼 Обложка", callback_data="creative:cover")],
        [InlineKeyboardButton("🎬 Оживить фото", callback_data="creative:revive"), InlineKeyboardButton("🏠 Меню", callback_data="nav:home")],
    ])

def _audio_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🎙 Голос → текст", callback_data="creative:stt"), InlineKeyboardButton("🔊 Текст → голос", callback_data="creative:tts")],
        [InlineKeyboardButton("🎵 Музыка для рилса", callback_data="creative:music"), InlineKeyboardButton("📢 Рекламная озвучка", callback_data="creative:voiceover")],
        [InlineKeyboardButton("🗣 Голос для аватара", callback_data="creative:avatar"), InlineKeyboardButton("🏠 Меню", callback_data="nav:home")],
    ])

def _docs_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("📎 Анализ файла", callback_data="act:study:pdf_summary"), InlineKeyboardButton("🧠 Резюме", callback_data="act:study:pdf_summary")],
        [InlineKeyboardButton("🌍 Перевод", callback_data="act:free"), InlineKeyboardButton("📊 Презентация/PDF", callback_data="biz:presentation")],
        [InlineKeyboardButton("🩺 Медицина", callback_data="mode:medicine"), InlineKeyboardButton("🏠 Меню", callback_data="nav:home")],
    ])

def _business_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🏡 Пост о вилле", callback_data="biz:villa_post"), InlineKeyboardButton("🎬 Рилс о вилле", callback_data="biz:villa_reels")],
        [InlineKeyboardButton("📊 Презентация объекта", callback_data="biz:presentation"), InlineKeyboardButton("💰 ROI / инвестиции", callback_data="biz:roi")],
        [InlineKeyboardButton("📲 WhatsApp", callback_data="biz:whatsapp"), InlineKeyboardButton("🌍 Перевод описания", callback_data="biz:translate")],
        [InlineKeyboardButton("🎨 Brand Kit", callback_data="settings:brand"), InlineKeyboardButton("🏠 Меню", callback_data="nav:home")],
    ])
...
def photo_quick_actions_kb():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🌈 Заменить фон", callback_data="pedit:replacebg"), InlineKeyboardButton("👁 Анализ", callback_data="pedit:vision")],
    ])
...
async def on_nav_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if data in ("creative:stt", "creative:tts", "creative:voiceover"):
            ...
            await q.edit_message_text(txt, reply_markup=_audio_menu_kb())
            await q.answer(); return
        ...
        if data.startswith("creative:manual_video:"):
            ...
            await q.edit_message_text("⚙️ Ручной выбор движка", reply_markup=InlineKeyboardMarkup(rows))
            await q.answer(); return
        if data in ("creative:repeat", "creative:other_engine"):
            await q.edit_message_text("🔁 Для повтора пришлите тот же материал или нажмите нужный сценарий в меню. Ручной выбор движка доступен в 🎛 Движки.", reply_markup=_video_menu_kb())
            await q.answer(); return
...
async def on_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("choose:"):
        try:
            ...
            est = _estimate_engine_cost(engine, duration)
            map_engine = "runway" if engine in {"kling", "sora", "seedance", "veo", "hailuo", "vidu", "grok"} else engine
            async def _start_real_render():
                await _run_creative_video(update, context, engine, prompt, duration, aspect, images=None, intent=meta.get("intent") or "text_video")
...
def build_application() -> "Application":
    ...
    BTN_PROJECTS= re.compile(r"^\s*(?:💼\s*)?Мои\s+проекты\s*$", re.I)
