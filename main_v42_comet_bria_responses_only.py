...
PATCH_VERSION = "v42-comet-bria-responses-only-2026-05-25"
...
# v42: Comet accepts Bria through its OpenAI-compatible /v1/responses relay.
...
COMET_BRIA_MODEL = os.environ.get("COMET_BRIA_MODEL", "replicate/bria/remove-background").strip() or "replicate/bria/remove-background"
COMET_BRIA_ALT_MODELS = [m.strip() for m in os.environ.get("COMET_BRIA_ALT_MODELS", "bria/remove-background").split(",") if m.strip()]
...
async def _v39_comet_bria_cutout(img_bytes: bytes, *, timeout_s: float | None = None) -> tuple[bytes, tuple[int, int]]:
    ...
    # v42: Comet routes this model through /v1/responses, not through native Bria URLs.
    # Use the Replicate namespace by default because Comet search/model pages expose
    # replicate/bria/remove-background for this wrapper. Keep bria/remove-background as a
    # secondary model name only on the same /v1/responses relay.
    model_names = []
    for _m in [COMET_BRIA_MODEL, *COMET_BRIA_ALT_MODELS]:
                ...
                "input": [
                    {
                        ...
                        "content": [
                            ...
                            {"type": "input_text", "text": "Remove the background from this image. Return only a PNG image with transparent background. Preserve the original foreground subject pixels; do not redraw the person or object."},
                        ],
                    }
                ],
            },
...
async def cmd_diag_bria(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "\nДля Comet v42 используем только /v1/responses. Если в Render раньше стояли прямые Bria paths, они игнорируются при COMET_BRIA_FORCE_RESPONSES=1. "
