...
def _has_comet_config() -> bool:
    return bool(COMET_BASE_URL) and bool(COMET_API_KEY)
...
async def _run_runway_video(update: Update, context: ContextTypes.DEFAULT_TYPE, prompt: str, duration_s: int, aspect: str):
    """Runway text→video через CometAPI."""
    if not _has_comet_config():
        await update.effective_message.reply_text("⚠️ runway: не настроен Comet API (нужны COMET_BASE_URL и COMET_API_KEY).")
        return

    msg = await update.effective_message.reply_text("⏳ runway: отправил задачу, жду результат…")

    async def _job():
        try:
            client = CometClient(COMET_BASE_URL, COMET_API_KEY, timeout=COMET_TIMEOUT)
            video_bytes, filename = await client.runway_text_to_video(prompt=prompt, duration_s=duration_s, aspect=aspect)
            with contextlib.suppress(Exception):
                await msg.delete()
            await update.effective_message.reply_video(video=video_bytes, filename=filename, caption="🎞 runway — готово")
        except Exception as e:
            log.exception("Runway via Comet error: %s", e)
            with contextlib.suppress(Exception):
                await msg.edit_text("❌ runway: ошибка. Проверь COMET_BASE_URL/COMET_API_KEY и попробуй ещё раз.")

    context.application.create_task(_job())


async def _run_runway_animate_photo(update: Update, context: ContextTypes.DEFAULT_TYPE, prompt: str = "Subtle natural animation of the input photo, gentle camera motion, realistic micro-movements", duration_s: int = 5, aspect: str = "16:9"):
    """Runway image→video через CometAPI."""
    if not _has_comet_config():
        await update.effective_message.reply_text("⚠️ runway: не настроен Comet API (нужны COMET_BASE_URL и COMET_API_KEY).")
        return

    cached = _get_cached_photo(update)
    if not cached:
        await update.effective_message.reply_text("⚠️ Сначала отправь фото.")
        return
    ...
    async def _job():
        try:
            client = CometClient(COMET_BASE_URL, COMET_API_KEY, timeout=COMET_TIMEOUT)
            video_bytes, filename = await client.runway_image_to_video(
                image_url=cached.get("file_url") or "",
                prompt=prompt,
            ...
            log.exception("Runway animate via Comet error: %s", e)
            with contextlib.suppress(Exception):
                await msg.edit_text("❌ runway: ошибка. Проверь COMET_BASE_URL/COMET_API_KEY и попробуй ещё раз.")
