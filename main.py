...
def ensure_supported_image_bytes(data: bytes) -> tuple[bytes, str]:
    """Гарантирует, что картинка будет в формате JPEG или PNG.
    Некоторые движки (Kling/Comet) не принимают WEBP/HEIC и другие форматы.
    Возвращает (bytes, mime).
    """
    if not data:
    ...
    if mime in ("image/jpeg", "image/png"):
        return data, mime
    # Пробуем конвертировать через Pillow
    try:
        from PIL import Image, ImageOps
        im = Image.open(io.BytesIO(data))
        # исправляем ориентацию по EXIF
        try:
            im = (__import__('PIL.ImageOps', fromlist=['ImageOps']).exif_transpose)(im)
        except Exception:
            pass
        ...
        # если не смогли — отдаём как есть
        return data, mime
