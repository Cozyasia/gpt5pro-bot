...
PATCH_VERSION = "v49-comet-bria-bg-upscale-2026-06-03"
...
COMET_BG_REMOVE_PATH = os.environ.get("COMET_BG_REMOVE_PATH", "/replicate/v1/models/bria/remove-background/predictions").strip() or "/replicate/v1/models/bria/remove-background/predictions"
...
COMET_BG_REMOVE_PATHS = _env_list(
    ...
    f"{COMET_BG_REPLICATE_PATH},/v1/responses,/bria/image/edit/remove-background"
)
COMET_BG_STATUS_PATH = os.environ.get("COMET_BG_STATUS_PATH", "/replicate/v1/predictions/{id}").strip() or "/replicate/v1/predictions/{id}"
COMET_BG_STATUS_PATHS = _env_list("COMET_BG_STATUS_PATHS", f"{COMET_BG_STATUS_PATH},/v1/responses/{{id}},/bria/{{id}}")
...
COMET_UPSCALE_PATHS = _env_list(
    "COMET_UPSCALE_PATHS",
    f"{COMET_UPSCALE_REPLICATE_PATH},/bria/image/edit/increase-resolution,/v1/responses"
)
COMET_UPSCALE_STATUS_PATHS = _env_list(
    "COMET_UPSCALE_STATUS_PATHS",
    "/replicate/v1/predictions/{id},/bria/{id},/v1/responses/{id}"
)
...
def _comet_bg_request_candidates(img_bytes: bytes, image_url: str = "") -> list[tuple[str, dict]]:
    ...
    v48 order:
    1) Comet Replicate-compatible route for bria/remove-background:
       POST /replicate/v1/models/bria/remove-background/predictions
    2) Comet Responses route for bria/remove-background.
    3) Bria image-editing route if the account supports it.

    The previous /bria/image/edit/remove-bg route returned
    "invalid relay mode", so it is intentionally not used by default.
    """
    mime = sniff_image_mime(img_bytes)
    b64 = base64.b64encode(img_bytes).decode("ascii")
    ...
    # 1) Replicate-compatible Comet route. The Comet model catalog exposes
    # replicate/bria/remove-background; Replicate-style input expects {input:{image:...}}.
    replicate_paths = [p for p in COMET_BG_REMOVE_PATHS if "/replicate/v1/models/" in p]
    if COMET_BG_REPLICATE_PATH not in replicate_paths:
    ...
    # 2) OpenAI-compatible route: POST /v1/responses with input_image.
    ...
    # 3) Bria-compatible edit route: POST /bria/image/edit/{action}. Keep only
    # the documented action spellings, not /remove-bg.
    bria_payloads = []
    if image_url and COMET_BG_USE_TELEGRAM_URL:
...
def _comet_upscale_request_candidates(img_bytes: bytes, image_url: str = "", factor: int = 2) -> list[tuple[str, dict]]:
    ...
    # 1) Replicate-compatible route. Prefer data URL under both image and image_url variants.
    replicate_paths = [p for p in COMET_UPSCALE_PATHS if "/replicate/v1/models/" in p]
    if COMET_UPSCALE_REPLICATE_PATH not in replicate_paths:
    ...
    # 3) OpenAI-compatible Responses fallback.
    if any(p == "/v1/responses" for p in COMET_UPSCALE_PATHS):
        for kind, img in image_inputs:
            ...
            candidates.append(("/v1/responses", {"model": COMET_UPSCALE_MODEL, "image": image_value, "desired_increase": factor, "preserve_alpha": True}))
...
async def _pedit_removebg(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes):
    try:
        ...
        if not out:
            ...
            await update.effective_message.reply_text(
                "Не удалось удалить фон через Comet. Проверьте доступность модели bria/remove-background в Comet через replicate-route и корректность COMET_API_KEY.\n"
                f"Диагностика: {diag}{extra}"
            )
            return
...
async def _pedit_replacebg(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, preset: str = "blur"):
    ...
    try:
        ...
        await update.effective_message.reply_text(
            "Не удалось заменить фон через Comet. Проверьте доступность модели bria/remove-background в Comet через replicate-route.\n"
