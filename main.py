...
PATCH_VERSION = "v67-final-webapp-logo-fix-2026-07-10"
...
VOCAL_CLIP_UNIT_COST_USD = _env_float("VOCAL_CLIP_UNIT_COST_USD", max(0.90, AVATAR_UNIT_COST_USD + SUNO_UNIT_COST_USD))
...
BOT_SHORT_DESCRIPTION = os.environ.get(
    ...
    "Мультимодельный нейро-бот: GPT, Images, Runway, Sora 2, Kling, Suno, Deepgram, Photoroom, FaceSwap — в одном Telegram."
).strip()[:120]
BOT_DESCRIPTION = os.environ.get(
    ...
    "генерация изображений, оживление фото, говорящий аватар, клип с вокалом для 1 человека, видео по тексту/голосу, Reels/Shorts, Sora 2/Kling/Runway-видео, Suno-музыка, презентации, PDF-каталоги, логотипы, удаление/замена фона и замена лица. "
...
async def on_mode_cb(update, context):
    ...
    if data == "act:fun:vocalclip":
        ...
        await q.edit_message_text(_vocal_clip_menu_text(), parse_mode="Markdown", reply_markup=_vocal_clip_action_kb("act"))
        return
    ...
    if data == "act:fun:textvideo":
        ...
        await q.edit_message_text(_textvideo_menu_text(), parse_mode="Markdown", reply_markup=_textvideo_action_kb("act"))
        return
    ...
    async def _job():
        ...
        try:
            ...
            audio_url = await _upload_bytes_to_telegram_file_url(
                update, context, audio_bytes, "vocal_clip_suno_track.mp3", "🎧 Вокал/трек получен. Запускаю lip-sync героя…"
            )
            if not audio_url:
            ...
            ok = await _run_kling_avatar(
                ...
                audio_filename="vocal_clip_suno_track.mp3",
...
async def on_cb_fun(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "vocalclip":
        ...
        _clear_transient_flows(context)
        _set_mode_clean(q.from_user.id, "Развлечения", "vocalclip")
        await q.edit_message_text(_vocal_clip_menu_text(), parse_mode="Markdown", reply_markup=_vocal_clip_action_kb("fun"))
        return
    ...
    if action == "textvideo":
        ...
        _clear_transient_flows(context)
        _set_mode_clean(q.from_user.id, "Развлечения", "textvideo")
        await q.edit_message_text(_textvideo_menu_text(), parse_mode="Markdown", reply_markup=_textvideo_action_kb("fun"))
        return
