...
PATCH_VERSION = "v34.1-faceswap-target-segmind-2026-07-06"
...
def _faceswap_cv2_status() -> str:
    ...
    try:
        ...
        return f"ok {getattr(cv2, '__version__', '')}".strip()
    except Exception as e:
...
def _detect_faces_for_choice(img_bytes: bytes) -> list[dict]:
    """Возвращает лица с api_index = порядок Segmind large→small и display_index = слева→направо для UI."""
    ...
    try:
        ...
        for (x, y, w, h) in faces:
            ...
            if ow < 20 or oh < 20:
                continue
            items.append({"x": ox, "y": oy, "w": ow, "h": oh, "area": ow * oh, "cx": ox + ow / 2, "cy": oy + oh / 2})
        # Segmind docs: default order is large→small. Сохраняем этот индекс как api_index.
        by_api = sorted(items, key=lambda f: f["area"], reverse=True)
        for i, f in enumerate(by_api):
            f["api_index"] = i
        # Для пользователя удобнее рисовать и показывать слева направо.
        display = sorted(by_api, key=lambda f: (f["cx"], f["cy"]))
...
async def _faceswap_process(update: Update, context: ContextTypes.DEFAULT_TYPE, quality: str = "fast"):
    ...
    await update.effective_message.reply_text(
        ...
        f"Целевое лицо: индекс {target_index}. Лицо-источник: индекс {source_index}. "
...
async def on_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if data.startswith("faceswap:target:"):
            ...
            await q.message.reply_text(f"✅ Выбрано целевое лицо: индекс {max(0, idx)}. Теперь пришлите фото лица, которое нужно вставить.")
            context.user_data["faceswap_flow"] = "await_source"
            return
        ...
        if data.startswith("faceswap:source:"):
            ...
            context.user_data["faceswap_flow"] = "ready"
            await q.message.reply_text(f"✅ Выбрано лицо-источник: индекс {max(0, idx)}. Выберите качество замены:", reply_markup=face_swap_quality_kb())
            return
