...
PATCH_VERSION = "v33.7-bg-photoroom-api-2026-07-06"
...
async def cmd_diag_bg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines.append(f"PHOTOROOM_API_KEY={'on' if bool(PHOTOROOM_API_KEY) else 'off'} base={PHOTOROOM_BASE_URL}{PHOTOROOM_REMOVE_PATH}")
...
async def _photoroom_api_remove_bytes(img_bytes: bytes) -> bytes | None:
    ...
    # Keep the source filename extension coherent with Telegram mime.
    ext = ".jpg"
    if mime == "image/png":
        ext = ".png"
    ...
    try:
        async with httpx.AsyncClient(timeout=timeout, follow_redirects=True) as client:
            files = {"image_file": (f"image{ext}", img_bytes, mime)}
...
async def _pedit_removebg(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes):
    try:
        ...
        if not out:
            if rembg_remove is None:
                await update.effective_message.reply_text(
                    "❌ Не удалось удалить фон: Photoroom API не ответил. "
                    "Проверьте PHOTOROOM_API_KEY в Render Environment."
                )
            else:
