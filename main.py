...
PATCH_VERSION = "v33.4-sora-fallback-single-mp4-2026-07-05"
...
LOCAL_REMBG_TIMEOUT_S = float(os.environ.get("LOCAL_REMBG_TIMEOUT_S", "180") or 180)
_REMBG_SESSION = None
...
def _get_local_rembg_session():
    ...
    with _REMBG_SESSION_LOCK:
        if _REMBG_SESSION is None:
            log.info("Initializing local rembg session: model=%s", REMBG_MODEL)
    ...
    async with httpx.AsyncClient(timeout=timeout, follow_redirects=True) as client:
        for path in paths:
            ...
            try:
                ...
                if r.status_code >= 400:
                    log.warning("Comet background remove failed: path=%s status=%s body=%s", path, r.status_code, r.text[:500])
                    continue
...
async def _remove_bg_bytes_primary(img_bytes: bytes) -> bytes | None:
    ...
    local_allowed = provider in ("auto", "local", "rembg", "legacy-local", "comet", "cometapi", "bria", "bria-comet", "bria/remove-background")

    if provider in ("auto", "comet", "cometapi", "bria-comet", "bria/remove-background"):
        out = await _comet_bria_remove_bg_bytes(img_bytes)
        if out:
            return _normalize_transparent_png(out)

    if provider in ("auto", "bria", "direct-bria", "bria-direct"):
        out = await _bria_direct_remove_bg_bytes(img_bytes)
        if out:
            return _normalize_transparent_png(out)

    if local_allowed and (BRIA_ALLOW_LOCAL_FALLBACK or provider in ("local", "rembg", "legacy-local")):
    ...
    try:
        ...
        if not out:
            if rembg_remove is None:
                ...
                await update.effective_message.reply_text("❌ Не удалось удалить фон. Проверьте ключи Comet/Bria или ресурсы Render для local rembg.")
            return
        bio = BytesIO(out)
        bio.name = "no_bg.png"
...
async def _pedit_replacebg(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, kind: str = "blur", prompt: str = ""):
    ...
    try:
        ...
        if not out:
            await update.effective_message.reply_text("❌ Не удалось отделить объект от фона. Проверьте Comet/Bria или local rembg.")
            return
        bio = BytesIO(out)
