...
CRYPTO_PAY_TOKEN = os.environ.get("CRYPTO_PAY_TOKEN", "").strip()  # из @CryptoBot → Crypto Pay API (вариант 1: прямые подписки)
...
def engines_kb():
    # Без кнопки «Открыть страницу тарифов»
...
async def on_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if data.startswith("buyc:"):
            await q.answer()
            try:
                _, plan, period = data.split(":")  # buyc:PLAN:PERIOD
                async with httpx.AsyncClient() as client:
                ...
                txt = (
                    ...
                    f"После оплаты доступ активируется автоматически (или пришлите чек, если не подтянулся)."
                )
        ...
        if data.startswith("engine:"):
            ...
            if offer == "ASK_SUBSCRIBE":
                await q.edit_message_text(
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [[InlineKeyboardButton("⭐ Тарифы", callback_data="noop")],
...
# ───────── Вспомогательные для подписки/инвойсов ─────────
def _plan_payload_and_amount(tier: str, months: int) -> tuple[str,int,str]:
    tier = (tier or "pro").lower()
    months = int(months or 1)
    prices = PLAN_PRICE_TABLE.get(tier, PLAN_PRICE_TABLE["pro"])
    if months == 12:
        amount_rub = prices["year"]
        payload = _payload_subscribe(tier, 12)
        title = f"{tier.upper()} 12 мес."
    elif months == 3:
        amount_rub = prices["quarter"]
        payload = _payload_subscribe(tier, 3)
        title = f"{tier.upper()} 3 мес."
    else:
        amount_rub = prices["month"]
        payload = _payload_subscribe(tier, 1)
        title = f"{tier.upper()} 1 мес."
    return payload, int(amount_rub), title
...
async def plans(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([
        ...
        [
            ...
            InlineKeyboardButton("START • ЮKassa", callback_data="buy:start:1"),
        ],
        [
        ...
        [
            ...
            InlineKeyboardButton("PRO • ЮKassa", callback_data="buy:pro:1"),
        ],
        [
        ...
        [
            ...
            InlineKeyboardButton("ULTIMATE • ЮKassa", callback_data="buy:ultimate:1"),
        ],
        [
