...
def engines_keyboard():
    rows = []
    for k, (title, _) in ENGINE_UI.items():
        rows.append([InlineKeyboardButton(title, callback_data=f"engine:{k}")])
    return InlineKeyboardMarkup(rows)

def welcome_text(lang):
    if lang == "ru":
        return "👋 Привет! Я GPT‑5 PRO Bot. Выберите движок ниже."
    return "👋 Welcome! I'm GPT‑5 PRO Bot. Choose engine below."

async def start_final(update, context):
    if await language_guard(update, context):
        return
    uid = update.effective_user.id
    await update.message.reply_text(
        welcome_text(get_user_lang(uid) or "ru"),
        reply_markup=engines_keyboard()
    )

async def on_callback_final(update, context):
    ...
    if data.startswith("lang:"):
        set_user_lang(uid, data.split(":")[1])
        await query.message.reply_text(
            welcome_text(get_user_lang(uid)),
            reply_markup=engines_keyboard()
        )
        return

    if data.startswith("engine:"):
        context.user_data["engine"] = data.split(":")[1]
        await query.message.reply_text("✅ Движок выбран. Напишите запрос.")
        return
# ================== END FINAL PATCH ==================
