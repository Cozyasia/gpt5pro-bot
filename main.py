...
PATCH_VERSION = "v76-video-provider-stability-2026-07-12"
...
RUNWAY_IMAGE2VIDEO_MODELS_ENV = os.environ.get("RUNWAY_IMAGE2VIDEO_MODELS", "gen4_turbo,gen3a_turbo,veo3.1_fast,veo3.1,veo3").strip()
RUNWAY_PUBLIC_FALLBACK_TEXT = os.environ.get("RUNWAY_PUBLIC_FALLBACK_TEXT", "⚠️ Runway сейчас недоступен у провайдера. Переключаю на резервный Kling.").strip()
    ...
    try:
        ...
        if data.startswith("crypto:check:"):
            ...
            if st == "paid":
                ...
                await q.message.reply_text(f"💳 Оплата получена. Баланс пополнен на ≈ ${usd_amount:.2f}.")
            elif st == "active":
...
def _runway_i2v_model_candidates() -> list[str]:
    ...
    v76: список берётся из RUNWAY_IMAGE2VIDEO_MODELS. gen4.5 намеренно не включён
    в дефолт, потому что Comet часто отвечает model_not_found/no available channel.
    """
    env_models = [m.strip() for m in (RUNWAY_IMAGE2VIDEO_MODELS_ENV or "").split(",") if m.strip()]
    return _dedupe_models(*(env_models or ["gen4_turbo", "gen3a_turbo", "veo3.1_fast", "veo3.1", "veo3"]))
