...
PATCH_VERSION = "v33.5.4-bg-cache-fix-2026-07-05"
...
LOCAL_REMBG_ENABLED_RAW = os.environ.get("LOCAL_REMBG_ENABLED", "1").strip().lower()
LOCAL_REMBG_ENABLED = BG_FORCE_LOCAL_REMBG or (LOCAL_REMBG_ENABLED_RAW not in ("0", "false", "no", "off"))
...
async def cmd_diag_bg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lines.append(f"LOCAL_REMBG_ENABLED={LOCAL_REMBG_ENABLED} raw={LOCAL_REMBG_ENABLED_RAW} force={BG_FORCE_LOCAL_REMBG}")
...
HELP_TEXT = globals().get("HELP_TEXT") or (
    "Команды: /start, /engines, /plans, /balance, /img <описание>, /voice_on, /voice_off, /diag_video, /diag_bg.\n"
    ...
    async with httpx.AsyncClient(timeout=timeout, follow_redirects=True) as client:
        for path in paths:
            ...
            try:
                ...
                log.warning("Comet background remove exception path=%s: %s", path, e)
    return None
        ...
        for path in paths:
            ...
            for headers in headers_variants:
                try:
                    ...
                    log.warning("Bria direct remove exception path=%s: %s", path, e)
    return None
...
async def _remove_bg_bytes_primary(img_bytes: bytes) -> bytes | None:
    ...
    Важно: на Render ручная загрузка файлов часто НЕ обновляет старые ENV.
    Поэтому local rembg пробуем первым по умолчанию независимо от старого BG_PROVIDER=comet.
    Comet/Bria оставлены только как fallback.
    """
    provider = (BG_PROVIDER or "auto").lower()

    local_allowed = LOCAL_REMBG_ENABLED and rembg_remove is not None
    remote_allowed = provider not in ("local-only", "rembg-only")

    # 1) Сначала local rembg — чтобы не зависеть от нестабильного Comet/Bria channel.
    if BG_LOCAL_FIRST and local_allowed:
        out = await _local_rembg_remove_bytes(img_bytes)
        if out:
            return _normalize_transparent_png(out)

    # 2) Удалённые провайдеры как запасной путь.
    if remote_allowed and provider in ("auto", "comet", "cometapi", "bria-comet", "bria/remove-background"):
        out = await _comet_bria_remove_bg_bytes(img_bytes)
        if out:
            return _normalize_transparent_png(out)

    if remote_allowed and provider in ("auto", "bria", "direct-bria", "bria-direct"):
        out = await _bria_direct_remove_bg_bytes(img_bytes)
        if out:
            return _normalize_transparent_png(out)

    # 3) Последний шанс local, если BG_LOCAL_FIRST=0 или старый ENV мешал первому пути.
    if (not BG_LOCAL_FIRST) and local_allowed:
        out = await _local_rembg_remove_bytes(img_bytes)
        if out:
            return _normalize_transparent_png(out)
    ...
    if not local_allowed:
        if rembg_remove is None:
            ...
            _bg_note_error("local rembg disabled by ENV")
...
async def _pedit_removebg(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes):
    try:
        ...
        if not out:
            if rembg_remove is None:
                await update.effective_message.reply_text(
                    "❌ Не удалось удалить фон: Comet/Bria не ответили, а local rembg не установлен. "
                    "Проверьте requirements.txt и ENV LOCAL_REMBG_ENABLED=1."
                )
            else:
        ...
        bio = BytesIO(out)
        bio.name = "no_bg.png"
        await update.effective_message.reply_document(InputFile(bio), caption="Фон удалён ✅ PNG с прозрачной подложкой.")
    except Exception as e:
...
async def _pedit_replacebg(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, kind: str = "blur", prompt: str = ""):
    if Image is None:
        ...
        return
    try:
        await update.effective_message.reply_text("🖼 Вырезаю объект и заменяю только фон, без перерисовки человека/предмета.")
        out = await _compose_replace_bg(img_bytes, bg_kind=kind, prompt=prompt)
        if not out:
        ...
        await update.effective_message.reply_photo(InputFile(bio), caption=cap)
    except Exception as e:
    ...
    try:
        ...
        if data.startswith("pedit:"):
            ...
            if data == "pedit:replacebg":
                await q.message.reply_text("Выберите новый фон или напишите свой вариант:", reply_markup=background_presets_kb()); return
            if data.startswith("pedit:bg:"):
                kind = data.split(":")[-1]
                if kind == "custom":
                    _set_replacebg_wait_text(context)
                    await q.message.reply_text("Напишите, какой фон поставить. Например: «дорогой офис с панорамными окнами» или «пляж Самуи на закате».")
                    return
...
async def cmd_diag_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    lines = [
        ...
        f"• BG remove: provider={BG_PROVIDER} comet_model={BG_COMET_MODEL} local_rembg={'✅' if (LOCAL_REMBG_ENABLED and rembg_remove is not None) else '❌'} model={REMBG_MODEL}",
...
async def on_text(
    ...
    if _is_replace_bg_request(text):
        ...
        await update.effective_message.reply_text("Пришлите фото — вырежу объект и заменю только фон.", reply_markup=main_kb)
        return
