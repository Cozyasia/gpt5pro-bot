...
PATCH_VERSION = "v41-comet-bria-root-normalized-debug-2026-05-25"
...
def _mode_desc(key: str) -> str:
    ...
    if key == "work":
        return (
            "💼 *Работа*\n"
            "Деловой режим: документы, аналитика, планы, бизнес-концепции, планы продаж и фирменные логотипы.\n\n"
            "Нажмите быстрый сценарий ниже или напишите свободный запрос. Для логотипа, концепции и плана продаж бот сначала соберёт вводные, а затем подготовит результат."
        )
    if key == "fun":
        return (
            "🔥 *Развлечения*\n"
...
def photo_quick_actions_kb():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("✨ Оживить (Kling)", callback_data="pedit:revive_kling")],
        [InlineKeyboardButton("🧽 Ретушь / убрать надпись", callback_data="pedit:retouch")],
        [InlineKeyboardButton("🧼 Удалить фон → серый", callback_data="pedit:removebg"),
         InlineKeyboardButton("🖼 Заменить фон", callback_data="pedit:replacebg")],
...
COMET_BRIA_REMOVEBG_PATHS = [
    p.strip() for p in os.environ.get(
        "COMET_BRIA_REMOVEBG_PATHS",
        "/v1/responses,/bria/image/edit/remove_background,/bria/image/edit/remove-background,/bria/image/edit/remove_bg,/bria/remove-background,/replicate/v1/models/bria/remove-background/predictions"
    ).split(",") if p.strip()
]
COMET_BRIA_STATUS_PATHS = [
    p.strip() for p in os.environ.get("COMET_BRIA_STATUS_PATHS", "/v1/responses/{id},/bria/{id},/bria/image/edit/{id},/replicate/v1/predictions/{id}").split(",") if p.strip()
]
...
def _v39_comet_bria_key_error() -> str:
    if not COMET_BRIA_API_KEY:
        return (
            "❌ Строгая замена фона через Comet/Bria недоступна: не задан COMET_API_KEY.\n\n"
            "Отдельный ключ remove.bg больше не нужен. Добавьте в Render ENV ваш обычный ключ CometAPI:\n"
            "COMET_API_KEY=ваш_ключ_comet\n\n"
            "Если хотите изолировать расходы/доступ только для Bria, можно дополнительно создать отдельный ключ в Comet "
            "и указать его как COMET_BRIA_API_KEY."
        )
    return ""


def _v39_prepare_source_image(img_bytes: bytes) -> tuple[bytes, str, str, tuple[int, int], str]:
    """Prepare image for Comet/Bria: resized PNG + data URL."""
    if Image is None:
        mime = sniff_image_mime(img_bytes)
        ext = "jpg" if mime == "image/jpeg" else "png"
        b64 = base64.b64encode(img_bytes).decode("ascii")
...
async def _v39_poll_comet_bria_result(client: httpx.AsyncClient, headers: dict, task_id: str, status_url: str | None = None) -> bytes:
        ...
        for url in urls:
            ...
            try:
                ...
                if status in ("failed", "error", "cancelled", "canceled", "rejected"):
                    raise RuntimeError(f"Comet/Bria task failed: {json.dumps(js, ensure_ascii=False)[:900]}")
    ...
    raise TimeoutError(f"Comet/Bria result timeout. Last status: {last_error[:700]}")


async def _v39_comet_bria_cutout(img_bytes: bytes, *, timeout_s: float | None = None) -> tuple[bytes, tuple[int, int]]:
    ...
    # Comet Bria v2 docs use POST /bria/image/edit/remove_background with JSON body:
    # {"image": "<base64 or public URL>", "preserve_alpha": true, "sync": true}.
    # We also keep compatibility payloads for older/proxy formats and /v1/responses.
    json_payloads = [
        # Comet's public model page for bria/remove-background uses /v1/responses.
        # Keep several Responses-compatible shapes because model wrappers differ.
        {
            "model": "bria/remove-background",
            "input": [
                {"role": "user", "content": [
                    {"type": "input_image", "image_url": data_url},
                    {"type": "input_text", "text": "Remove the background and return a transparent PNG cutout of the original foreground subject. Do not redraw the subject."},
                ]}
            ],
        },
        {
            "model": "replicate/bria/remove-background",
            "input": [
                {"role": "user", "content": [
                    {"type": "input_image", "image_url": data_url},
                ]}
        ...
        {"model": "bria/remove-background", "input": {"image": data_url, "sync_mode": True, "enable_base64_output": False}},
        {"model": "bria/remove-background", "input": {"image": data_url, "sync_mode": False, "enable_base64_output": False}},
        {"model": "bria/remove-background", "image": raw_b64, "preserve_alpha": True, "sync": True},
        # Bria-compatible v2 endpoint. Use raw base64, not data URL.
        {"image": raw_b64, "preserve_alpha": True, "sync": True, "visual_input_content_moderation": False, "visual_output_content_moderation": False},
        {"image": raw_b64, "preserve_alpha": True, "sync": False, "visual_input_content_moderation": False, "visual_output_content_moderation": False},
        # Some Comet/proxy variants accept data URLs.
        {"image": data_url, "preserve_alpha": True, "sync": True, "visual_input_content_moderation": False, "visual_output_content_moderation": False},
        {"image": data_url, "format": COMET_BRIA_OUTPUT_FORMAT, "preserve_alpha": True, "sync": True},
        # OpenAI/Responses-style proxy fallback seen on some Comet model pages.
        {
            "model": "bria/remove-background",
            "input": [
                {
                    "role": "user",
                    "content": [
                        {"type": "input_text", "text": "Remove the background and return a transparent PNG cutout of the original subject."},
                        {"type": "input_image", "image_url": data_url},
                    ],
                }
            ],
        },
        # Replicate-style official model fallback.
        {"input": {"image": data_url, "enable_sync_mode": True, "enable_base64_output": False}},
        {"input": {"image": data_url, "enable_sync_mode": False, "enable_base64_output": False}},
        {"model": "bria/remove-background", "input": {"image": data_url}, "format": COMET_BRIA_OUTPUT_FORMAT},
        {"image_url": data_url, "format": COMET_BRIA_OUTPUT_FORMAT},
        {"input_image": data_url, "format": COMET_BRIA_OUTPUT_FORMAT},
        {"source_image": data_url, "format": COMET_BRIA_OUTPUT_FORMAT},
        {"input": {"image": data_url}, "format": COMET_BRIA_OUTPUT_FORMAT},
    ]
    last_error = ""
        ...
        for path in COMET_BRIA_REMOVEBG_PATHS:
            ...
            for payload in json_payloads:
                try:
                    ...
                    if r.status_code >= 400:
                        last_error = f"POST {path} JSON -> {r.status_code}: {r.text[:700]}"
                        continue
                    if _v39_is_image_bytes(r.content, r.headers.get("content-type", "")):
                    ...
                    if task_id or status_url:
                        ...
                        return out, size
                    last_error = f"POST {path} JSON: no image/task in response {json.dumps(js, ensure_ascii=False)[:700]}"
                except Exception as e:
                    last_error = f"POST {path} JSON exception: {e}"
            # 2) Multipart fallback: some proxy endpoints accept file upload.
            try:
    ...
    raise RuntimeError(f"Comet/Bria remove-background failed. Last error: {last_error[:1000]}")
...
async def _v39_strict_background_pipeline(
    ...
    *,
    ...
    """Strict pipeline: Comet/Bria cutout -> background -> pixel composite."""
    cutout, size = await _v39_comet_bria_cutout(img_bytes)
    if gray:
...
async def _pedit_removebg(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes):
    """v39: strict remove background via Comet/Bria + local gray composite."""
    ...
    await update.effective_message.reply_text(
        "🧼 Принял: удаляю фон через Comet/Bria.\n"
        "Сначала получу PNG-вырезку оригинального объекта, затем поставлю серую подложку. "
        "Человек/объект не будет перерисовываться нейросетью."
    )
    with contextlib.suppress(Exception):
        await context.bot.send_chat_action(update.effective_chat.id, ChatAction.UPLOAD_PHOTO)
    try:
        out = await _v39_strict_background_pipeline(img_bytes, background_prompt=None, label="серая подложка", gray=True)
        bio = BytesIO(out); bio.name = "comet_bria_background_gray.jpg"
        await update.effective_message.reply_photo(
            InputFile(bio),
            caption="✅ Готово: фон удалён через Comet/Bria, поставлена нейтральная серая подложка. Основной объект взят из оригинала, не перерисован."
        )
    except Exception as e:
        log.exception("v39 Comet/Bria remove background error: %s", e)
        detail = str(e)[:1500] if COMET_BRIA_DEBUG_TO_CHAT else type(e).__name__
        await update.effective_message.reply_text(
            f"❌ Не удалось удалить фон через Comet/Bria. Техническая причина: {type(e).__name__}.\n\n"
            f"Детали: {detail}\n\n"
            "Проверьте COMET_API_KEY, баланс/доступ к модели bria/remove-background. "
            "Если в ENV стоит COMET_BASE_URL=https://api.cometapi.com/v1 — v41 это уже нормализует автоматически. "
            "Рекомендуемый COMET_BRIA_REMOVEBG_PATHS: /v1/responses"
        )
...
async def _pedit_replacebg(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes):
    ...
    await update.effective_message.reply_text(
        "🖼 Выберите новый фон или напишите свой промпт.\n\n"
        "Строгий режим теперь работает через Comet/Bria: я получаю PNG-вырезку оригинального человека/объекта "
...
async def _pedit_replacebg_apply(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, prompt: str, label: str = "новый фон"):
    """v39: strict background replacement using Comet/Bria original cutout + generated/fallback background."""
    err = _v39_comet_bria_key_error()
    if err:
        await update.effective_message.reply_text(err)
        return
    ...
    if not OPENAI_IMAGE_KEY or OPENAI_IMAGE_KEY.startswith("sk-or-"):
        await update.effective_message.reply_text(
            "⚠️ OPENAI_IMAGE_KEY не задан или это OpenRouter-ключ. Объект сохраню строго через Comet/Bria, "
            "но фон будет простой служебный, без AI-генерации. Для красивых фонов добавьте официальный OPENAI_IMAGE_KEY."
        )
    try:
        with contextlib.suppress(Exception):
            await context.bot.send_chat_action(update.effective_chat.id, ChatAction.UPLOAD_PHOTO)
        await update.effective_message.reply_text(f"🖼 Принял: ставлю фон «{label}» через строгий Comet/Bria pipeline v41…")
        out = await _v39_strict_background_pipeline(img_bytes, background_prompt=prompt, label=label, kind=kind, gray=False)
        bio = BytesIO(out); bio.name = "comet_bria_background_replaced.jpg"
        await update.effective_message.reply_photo(
            InputFile(bio),
            caption=f"✅ Фон заменён: {label}. Основной объект взят из оригинала через Comet/Bria и не перерисован; заменён только фон."
        )
    except Exception as e:
        log.exception("v39 Comet/Bria replace background error: %s", e)
        detail = str(e)[:1500] if COMET_BRIA_DEBUG_TO_CHAT else type(e).__name__
        await update.effective_message.reply_text(
            f"❌ Не удалось заменить фон через Comet/Bria. Техническая причина: {type(e).__name__}.\n\n"
            f"Детали: {detail}\n\n"
            "Проверьте COMET_API_KEY, баланс/доступ к модели bria/remove-background и OPENAI_IMAGE_KEY для красивых фонов. "
            "Рекомендуемый COMET_BRIA_REMOVEBG_PATHS: /v1/responses"
        )


async def cmd_diag_bria(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Show sanitized Comet/Bria routing diagnostics without exposing secrets."""
    key_ok = bool(COMET_BRIA_API_KEY)
    comet_base = COMET_BASE_URL or ""
    await update.message.reply_text(
        "🧪 Comet/Bria diagnostics\n"
        ...
        f"• removebg paths: {', '.join(COMET_BRIA_REMOVEBG_PATHS)}\n"
        f"• debug to chat: {'on' if COMET_BRIA_DEBUG_TO_CHAT else 'off'}\n"
        "\nДля Comet обычно ставим COMET_BRIA_REMOVEBG_PATHS=/v1/responses. "
        "Если ошибка повторится, пришлите строку 'Детали:' из ответа бота."
    )
