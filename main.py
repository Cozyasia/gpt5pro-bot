...
async def cmd_sora(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not COMET_BASE_URL or not COMET_API_KEY:
        await update.message.reply_text("🎞 Sora 2: не настроен Comet API (нужны COMET_BASE_URL=https://api.cometapi.com и COMET_API_KEY).")
        return
...
async def cmd_suno(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not prompt:
        ...
        return
    if not COMET_BASE_URL or not COMET_API_KEY:
        await update.message.reply_text("🎵 Suno: не настроен Comet API (нужны COMET_BASE_URL=https://api.cometapi.com и COMET_API_KEY).")
        return
    client = CometClient(COMET_BASE_URL, COMET_API_KEY, timeout=COMET_TIMEOUT)
...
async def cmd_mj(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not prompt:
        ...
        return
    if not COMET_BASE_URL or not COMET_API_KEY:
        await update.message.reply_text("🎨 Midjourney: не настроен Comet API (нужны COMET_BASE_URL=https://api.cometapi.com и COMET_API_KEY).")
        return
    client = CometClient(COMET_BASE_URL, COMET_API_KEY, timeout=COMET_TIMEOUT)
...
async def _run_runway_animate_photo(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, img_url: str | None, prompt: str, duration_s: int, aspect: str):
    ...
    ВАЖНО: Runway идёт через CometAPI (COMET_BASE_URL/COMET_API_KEY).
    Если Comet не настроен — используем нативный Runway API как фолбэк.
    """

    # 1) Primary: Comet
    if _has_comet_config():
        await context.bot.send_chat_action(update.effective_chat.id, ChatAction.RECORD_VIDEO)
        try:
            from engine import CometClient
            b64 = base64.b64encode(img_bytes).decode('ascii')
            client = CometClient(
            ...
            res = client.runway_image_to_video(
                image_b64=b64,
                ...
                resolution=os.getenv('COMET_VIDEO_RESOLUTION', '720p').strip() or '720p',
            )
            task_id = res['task_id']
            await update.effective_message.reply_text("⏳ Runway (Comet): задача принята. Жду результат...")

            deadline = time.time() + float(os.getenv('COMET_POLL_TIMEOUT_S', '240'))
            while time.time() < deadline:
                st = client.runway_get_task(task_id)
                state = None
                if isinstance(st, dict):
                    state = st.get('state') or st.get('status')
                    if not state and isinstance(st.get('data'), dict):
                        state = st['data'].get('state') or st['data'].get('status')

                if state in ('succeeded', 'completed', 'success'):
                    video_bytes = client.runway_get_content(task_id)
                    await update.effective_message.reply_video(video=video_bytes, caption="✅ Runway (Comet): готово")
                    return
                if state in ('failed', 'error', 'canceled', 'cancelled'):
                    await update.effective_message.reply_text(f"❌ Runway (Comet): ошибка.\n{st}")
                    return

                await asyncio.sleep(VIDEO_POLL_DELAY_S)

            await update.effective_message.reply_text("⏳ Runway (Comet): слишком долго. Попробуй позже.")
            return
        except Exception as e:
            log.exception("Runway (Comet) error: %s", e)
            await update.effective_message.reply_text(f"❌ Runway (Comet): ошибка.\n{e}")
            return

    # 2) Fallback: native Runway API
    if not RUNWAY_API_KEY:
        await update.effective_message.reply_text("⚠️ Runway: не настроен (RUNWAY_API_KEY) и не настроен Comet (COMET_BASE_URL/COMET_API_KEY).")
        return
    ...
    def _runway_ratio_from_aspect(a: str) -> str:
        ...
        return mapping.get(a, RUNWAY_RATIO)
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=60.0) as client_http:
            r = await client_http.post(f"{RUNWAY_BASE_URL}{RUNWAY_CREATE_PATH}", headers=headers, json=payload)
            txt = (r.text or "")[:2000]
            if r.status_code >= 400:
            ...
            if not task_id:
                await update.effective_message.reply_text(f"❌ Runway: не вернул id задачи.\n{txt}")
                return

            await update.effective_message.reply_text("⏳ Runway: задача принята. Жду результат...")

            deadline = time.time() + float(os.getenv('RUNWAY_POLL_TIMEOUT_S', '240'))
            while time.time() < deadline:
                st = await client_http.get(f"{RUNWAY_BASE_URL}{RUNWAY_STATUS_PATH_TPL.format(task_id=task_id)}", headers=headers)
                st_data = st.json() if st.headers.get('content-type', '').startswith('application/json') else {'raw': st.text}
                status = (st_data.get('status') or st_data.get('state') or '').lower()

                if status in ("succeeded", "completed", "success"):
                    out = st_data.get('output') or {}
                    url = out.get('video') or out.get('url')
                    if not url:
                        await update.effective_message.reply_text(f"❌ Runway: нет ссылки на видео.\n{st_data}")
                        return
                    v = await client_http.get(url)
                    await update.effective_message.reply_video(video=v.content, caption="✅ Runway: готово")
                    return

                if status in ("failed", "error"):
                    await update.effective_message.reply_text(f"❌ Runway: ошибка рендера.\n{st_data}")
                    return

                await asyncio.sleep(VIDEO_POLL_DELAY_S)

            await update.effective_message.reply_text("⏳ Runway: слишком долго. Попробуй позже.")
...
async def _run_comet_animate_photo(update: Update, context: ContextTypes.DEFAULT_TYPE, provider: str, img_bytes: bytes, prompt: str):
    if not COMET_BASE_URL or not COMET_API_KEY:
        await update.effective_message.reply_text(
            f"⚠️ {provider}: не настроен Comet API (нужны COMET_BASE_URL=https://api.cometapi.com и COMET_API_KEY)."
        )
        return
...
async def on_text(
    ...
    if selected_engine == "midjourney":
        if not COMET_BASE_URL or not COMET_API_KEY:
            await update.effective_message.reply_text("🎨 Midjourney: не настроен Comet API (нужны COMET_BASE_URL=https://api.cometapi.com и COMET_API_KEY).")
            return
        client = CometClient(COMET_BASE_URL, COMET_API_KEY, timeout=COMET_TIMEOUT)
    ...
    if selected_engine == "sora":
        if not COMET_BASE_URL or not COMET_API_KEY:
            await update.effective_message.reply_text("🎞 Sora 2: не настроен Comet API (нужны COMET_BASE_URL=https://api.cometapi.com и COMET_API_KEY).")
            return
        client = CometClient(COMET_BASE_URL, COMET_API_KEY, timeout=COMET_TIMEOUT)
    ...
    if selected_engine == "suno":
        if not COMET_BASE_URL or not COMET_API_KEY:
            await update.effective_message.reply_text("🎵 Suno: не настроен Comet API (нужны COMET_BASE_URL=https://api.cometapi.com и COMET_API_KEY).")
            return
        client = CometClient(COMET_BASE_URL, COMET_API_KEY, timeout=COMET_TIMEOUT)
