...
PATCH_VERSION = "v34.0-prod-bg-faceswap-2026-07-06"
...
FACESWAP_FALLBACK_PROVIDER = os.environ.get("FACESWAP_FALLBACK_PROVIDER", "segmind").strip().lower() or "segmind"
...
def photo_quick_actions_kb():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🎭 Замена лица", callback_data="pedit:faceswap")],
...
async def _piapi_faceswap(target_img: bytes, source_face: bytes, quality: str = "fast") -> bytes | None:
    ...
    payload = {
        ...
        "input": {"target_image": target_b64, "swap_image": source_b64},
    }
    async with httpx.AsyncClient(timeout=60.0, follow_redirects=True) as client:
        ...
        try:
            ...
            obj = {}
        # Some PiAPI/legacy deployments can return final URL directly.
...
async def _segmind_faceswap_v2(target_img: bytes, source_face: bytes) -> bytes | None:
...
async def _run_faceswap_provider(provider: str, target_img: bytes, source_face: bytes, quality: str) -> bytes | None:
    provider = (provider or "").lower().strip()
    if provider in ("piapi", "pi", "qubico"):
        return await _piapi_faceswap(target_img, source_face, quality=quality)
    if provider in ("segmind", "segmind-v4", "segmind4"):
        if quality == "premium":
            out = await _segmind_faceswap_v4(target_img, source_face, quality=quality)
            if out:
                return out
        return await _segmind_faceswap_v2(target_img, source_face)
    if provider in ("segmind-v2", "segmind2"):
        return await _segmind_faceswap_v2(target_img, source_face)
...
async def _faceswap_process(update: Update, context: ContextTypes.DEFAULT_TYPE, quality: str = "fast"):
    ...
    provider_order = []
    if quality == "premium":
        provider_order = [FACESWAP_PROVIDER, FACESWAP_FALLBACK_PROVIDER, "segmind"]
    else:
        provider_order = [FACESWAP_PROVIDER, FACESWAP_FALLBACK_PROVIDER]
    seen = []
    provider_order = [p for p in provider_order if p and not (p in seen or seen.append(p))]

    await update.effective_message.reply_text("🎭 Запускаю замену лица. Сохраняю исходное тело, одежду, фон и композицию. Обычно 10–90 секунд…")

    async def _go():
        out = None
        for provider in provider_order:
            try:
                out = await asyncio.wait_for(_run_faceswap_provider(provider, target, source, quality), timeout=FACESWAP_TIMEOUT_S + 20)
                if out:
                    break
            except asyncio.TimeoutError:
        ...
        if FACESWAP_RESULT_AS_DOCUMENT:
            ...
            await update.effective_message.reply_document(InputFile(bio), caption="🎭 Лицо заменено ✅ Результат сохранён как исходное фото для дальнейших действий.", reply_markup=photo_quick_actions_kb())
        else:
            bio.name = "face_swap.jpg"
            await update.effective_message.reply_photo(InputFile(bio), caption="🎭 Лицо заменено ✅ Результат сохранён как исходное фото для дальнейших действий.", reply_markup=photo_quick_actions_kb())
...
async def _start_faceswap_flow(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes | None = None):
    ...
    if not target:
        ...
        await update.effective_message.reply_text(
            "🎭 Замена лица. Пришлите фото, где нужно заменить лицо. Затем я попрошу фото лица для вставки."
        )
        return
    _faceswap_target_cache[user_id] = target
    context.user_data["faceswap_flow"] = "await_source"
    msg = (
        "🎭 Фото-цель сохранено. Теперь пришлите фото лица, которое нужно вставить.\n\n"
...
async def on_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if context.user_data.get("faceswap_flow") == "await_target":
            ...
            context.user_data["faceswap_flow"] = "await_source"
            await update.effective_message.reply_text("🎭 Фото-цель сохранено. Теперь пришлите фото лица, которое нужно вставить.")
            return
...
async def on_doc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if mt.startswith("image/"):
            ...
            if context.user_data.get("faceswap_flow") == "await_target":
                _faceswap_target_cache[update.effective_user.id] = raw
                context.user_data["faceswap_flow"] = "await_source"
                await update.effective_message.reply_text("🎭 Фото-цель сохранено как документ. Теперь пришлите фото лица, которое нужно вставить.")
                return
            if context.user_data.get("faceswap_flow") == "await_source":
                _faceswap_source_cache[update.effective_user.id] = raw
                await update.effective_message.reply_text("🎭 Фото лица получено. Выберите качество замены:", reply_markup=face_swap_quality_kb())
                return
            if caption and _is_face_swap_request(caption):
                _clear_medicine_wait(context)
