...
PATCH_VERSION = "v58-comet-generatecontent-ai-selfie-avatar-flow-2026-07-09"
...
COMET_IMAGE_EDIT_MODEL = os.environ.get("COMET_IMAGE_EDIT_MODEL", "gemini-2.5-flash-image-preview").strip() or "gemini-2.5-flash-image-preview"
COMET_IMAGE_EDIT_FALLBACK_MODELS = [m.strip() for m in os.environ.get("COMET_IMAGE_EDIT_FALLBACK_MODELS", "gemini-2.5-flash-image-preview,gemini-2-5-flash-image-preview,gemini-2.5-flash-image,gemini-2-5-flash-image,gemini-3-pro-image").split(",") if m.strip()]
...
COMET_IMAGE_EDIT_TIMEOUT_S = float(os.environ.get("COMET_IMAGE_EDIT_TIMEOUT_S", "240") or 240)
...
async def _run_comet_ai_selfie_bytes(img_bytes: bytes, user_prompt: str, preset_prompt: str = "") -> bytes | None:
    """AI selfie via Comet Nano Banana/Gemini generateContent, with optional old OpenAI-compatible fallback.
    Comet's Nano Banana guide documents Gemini image models through /v1beta/models/{model}:generateContent.
    """
    ...
    edit_bytes, filename, mime = _prepare_image_for_edit(img_bytes)
    img_b64 = base64.b64encode(edit_bytes).decode("ascii")
    mime = mime or sniff_image_mime(edit_bytes) or "image/png"
    ...
    base_paths: list[str] = []
    for p in (COMET_IMAGE_EDIT_PATH, "/v1beta/models/{model}:generateContent", "/v1/models/{model}:generateContent"):
        p = (p or "").strip()
        if p and p not in base_paths:
            base_paths.append(p)

    def _mk_generate_content_payload(style: str) -> dict:
        if style == "camel":
    ...
    # Comet generally accepts Bearer, but some Gemini-compatible routes accept x-goog-api-key.
    header_variants = [
    ...
    async with httpx.AsyncClient(timeout=COMET_IMAGE_EDIT_TIMEOUT_S, follow_redirects=True) as client:
        for model in models:
            for path_tmpl in base_paths:
                ...
                # If user accidentally left OpenAI Images path, skip it in primary generateContent mode.
                if "/images/" in path and "generateContent" not in path:
                    continue
                url = f"{COMET_BASE_URL}{path}"
                for payload_style in ("snake", "camel"):
                    payload = _mk_generate_content_payload(payload_style)
        ...
        if COMET_IMAGE_EDIT_OPENAI_FALLBACK:
            # Old OpenAI-compatible Comet image edit route. Disabled by default because many Comet groups route it to DALL-E.
...
async def cmd_diag_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    lines = [
        ...
        f"• AI selfie: provider={AI_SELFIE_PROVIDER} comet_key={'on' if bool(COMET_API_KEY) else 'off'} model={COMET_IMAGE_EDIT_MODEL} fallbacks={','.join(COMET_IMAGE_EDIT_FALLBACK_MODELS)} path={COMET_IMAGE_EDIT_PATH} cost=${AI_SELFIE_UNIT_COST_USD:.2f}",
...
async def on_text(
    ...
    if context.user_data.get("awaiting_avatar_voice_choice"):
        ...
        if tl in voice_aliases:
            context.user_data["avatar_tts_voice"] = voice_aliases[tl]
            context.user_data.pop("awaiting_avatar_voice_choice", None)
            _set_avatar_wait(context)
            await update.effective_message.reply_text(f"✅ Для аватара выбран голос: {_avatar_tts_voice_label(voice_aliases[tl])}. Теперь пришлите текст, который должен произнести аватар.")
            return
