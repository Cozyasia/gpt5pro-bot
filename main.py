...
PATCH_VERSION = "v44-comet-bria-docs-payload-2026-05-26"
...
async def _v39_strict_background_pipeline(
    ...
    *,
    ...
    cutout, size = await _v39_comet_bria_cutout(img_bytes, source_url=source_url)
    if gray:
...
async def _pedit_removebg(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes):
    ...
    try:
        ...
        out = await _v39_strict_background_pipeline(img_bytes, background_prompt=None, label="серая подложка", gray=True, source_url=source_url)
...
async def _pedit_replacebg_apply(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, prompt: str, label: str = "новый фон"):
    ...
    try:
        ...
        await update.effective_message.reply_text(
            ...
            "Рекомендуемые ENV: COMET_BRIA_MODEL=bria/remove-background, "
...
async def cmd_diag_bria(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "\nДля Comet v44 используем только /v1/responses и model=bria/remove-background по документации Comet. Сначала пробуем payload с публичным Telegram image_url, затем data URL/base64 fallback. "
