...
PATCH_VERSION = "v45-bg-provider-compat-diagnostics-2026-05-31"
...
# Background tools.
# v45 compatibility rule:
# - If BRIA key exists: BRIA is the primary production provider.
# - If no BRIA key exists: automatically fall back to the old local rembg mode.
# - Never replace background by simply blurring the whole image.
BG_PROVIDER = os.environ.get("BG_PROVIDER", "auto").strip().lower() or "auto"
BRIA_API_KEY = (
    os.environ.get("BRIA_API_KEY")
    or os.environ.get("BRIA_KEY")
    or os.environ.get("BRIA_API_TOKEN")
    or os.environ.get("BRIA_TOKEN")
    or os.environ.get("BRIA_AUTH_TOKEN")
    or os.environ.get("BRIA_RMBG_API_KEY")
    or os.environ.get("REMOVE_BG_API_KEY")
    or os.environ.get("REMOVEBG_API_KEY")
    or os.environ.get("REMOVE_BG_KEY")
    or os.environ.get("BACKGROUND_REMOVE_API_KEY")
    or ""
).strip()
...
LOCAL_REMBG_ENABLED = os.environ.get("LOCAL_REMBG_ENABLED", "1").strip().lower() not in ("0", "false", "no", "off")
...
def _bg_runtime_status() -> str:
    parts = []
    parts.append(f"BG_PROVIDER={BG_PROVIDER or 'auto'}")
    parts.append("BRIA=on" if bool(BRIA_API_KEY) else "BRIA=missing")
    parts.append("rembg=on" if rembg_remove is not None else f"rembg=missing({REMBG_IMPORT_ERROR or 'not installed'})")
    return "; ".join(parts)

def _bg_use_bria() -> bool:
    return bool(BRIA_API_KEY) and BG_PROVIDER in {"auto", "bria", "briai", "bria-ai"}

def _bg_use_local() -> bool:
    return LOCAL_REMBG_ENABLED and BRIA_ALLOW_LOCAL_FALLBACK and rembg_remove is not None and BG_PROVIDER in {"auto", "local", "rembg", "bria"}

def _as_bria_base64(img_bytes: bytes) -> str:
    """BRIA v2 accepts raw base64 or public URL. We use raw base64 to avoid hosting the file."""
    return base64.b64encode(img_bytes).decode("ascii")

def _extract_image_url_from_json(data) -> str:
    """Flexible parser for BRIA/aggregator responses."""
    if not isinstance(data, dict):
        return ""
    candidates = [
        data.get("image_url"),
        data.get("url"),
        (data.get("result") or {}).get("image_url") if isinstance(data.get("result"), dict) else None,
        (data.get("result") or {}).get("url") if isinstance(data.get("result"), dict) else None,
        (data.get("image") or {}).get("url") if isinstance(data.get("image"), dict) else None,
        (data.get("output") or {}).get("image_url") if isinstance(data.get("output"), dict) else None,
    ...
    for c in candidates:
        if isinstance(c, str) and c.startswith(("http://", "https://", "data:image/")):
            return c
    return ""
...
async def _bria_image_call(url: str, payloads: list[dict]) -> bytes | None:
    """Call BRIA v2 endpoint, prefer sync=true. Poll status_url if endpoint responds async."""
    if not BRIA_API_KEY:
        return None
    last_err = None
    async with httpx.AsyncClient(timeout=BRIA_TIMEOUT_S, follow_redirects=True) as client:
        for payload in payloads:
            ...
            try:
                ...
                status_url = data.get("status_url") or data.get("statusUrl") or data.get("polling_url")
                if status_url:
                    for _ in range(60):
                        await asyncio.sleep(3)
                        sr = await client.get(status_url, headers=headers)
                        if sr.status_code >= 400:
                            last_err = f"status {sr.status_code}: {sr.text[:300]}"
                            break
                        sd = sr.json()
                ...
                continue
    if last_err:
        log.warning("BRIA image call failed: %s", last_err)
    return None
...
async def _remove_bg_bytes_primary(img_bytes: bytes) -> bytes | None:
    """Production order: BRIA first if configured; otherwise old local rembg mode."""
    if _bg_use_bria():
...
async def _pedit_removebg(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes):
    try:
        out = await _remove_bg_bytes_primary(img_bytes)
        if not out:
            await update.effective_message.reply_text(
                "Не удалось удалить фон. Включите один из рабочих провайдеров:\n"
                "1) BRIA: добавьте BRIA_API_KEY/BRIA_KEY в ENV;\n"
                "2) старый локальный режим: убедитесь, что в requirements.txt есть rembg==2.0.67 и onnxruntime>=1.18.0.\n\n"
                f"Диагностика: {_bg_runtime_status()}"
            )
            return
        bio = BytesIO(out); bio.name = "no_bg.png"; bio.seek(0)
        await update.effective_message.reply_document(InputFile(bio), caption="Фон удалён ✅ Прозрачный PNG.")
    except Exception as e:
...
async def _foreground_cutout(img_bytes: bytes):
    out = await _remove_bg_bytes_primary(img_bytes)
...
async def _pedit_replacebg_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    provider_line = "BRIA Remove/Replace Background" if _bg_use_bria() else "старый локальный remove-bg fallback"
    msg = (
...
async def _pedit_replacebg(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, preset: str = "blur"):
    if Image is None:
        ...
        return
    try:
        # BRIA replace_background first, if configured. It preserves subject better than local composition.
        if _bg_use_bria() and preset not in {"white"}:
            bria_out = await _bria_replace_bg_bytes(img_bytes, preset)
            if bria_out:
                bio = BytesIO(bria_out); bio.name = f"bria_bg_{preset}.png"; bio.seek(0)
                await update.effective_message.reply_photo(InputFile(bio), caption=f"Фон заменён через BRIA ✅ ({preset})")
                return

        # Safe fallback: remove-bg alpha PNG + local background, without blurring the subject.
        src = Image.open(BytesIO(img_bytes)).convert("RGBA")
        fg = await _foreground_cutout(img_bytes)
        if fg is None:
            raise RuntimeError("foreground cutout failed")
        if fg.size != src.size:
            fg = fg.resize(src.size, Image.LANCZOS)
        bg = _make_replacement_bg(src.size, preset, src_img=src)
        bg.alpha_composite(fg)
        out = bg.convert("RGB")
        bio = BytesIO(); out.save(bio, format="JPEG", quality=94); bio.seek(0); bio.name = f"bg_{preset}.jpg"
        captions = {
            "blur": "Заменил фон на размытый, объект оставил резким ✅",
            "white": "Заменил фон на белую подложку ✅",
            "beach": "Заменил фон на пляжную подложку ✅",
            "nature": "Заменил фон на природную подложку ✅",
            "office": "Заменил фон на офисную/студийную подложку ✅",
            "luxury": "Заменил фон на premium/luxury подложку ✅",
        }
        await update.effective_message.reply_photo(InputFile(bio), caption=captions.get(preset, "Фон заменён ✅"))
    except Exception as e:
        log.exception("replacebg error: %s", e)
        await update.effective_message.reply_text(
            "Не удалось заменить фон. Нужно, чтобы работал BRIA или старый локальный rembg.\n"
            f"Диагностика: {_bg_runtime_status()}"
        )
