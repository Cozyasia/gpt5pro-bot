...
# v46: main provider is CometAPI model bria/remove-background.
# It needs only COMET_API_KEY. No BRIA_API_KEY, rembg or onnxruntime required for production.
...
# Keep legacy keys only as optional emergency fallback, but they are NOT needed when BG_PROVIDER=comet.
...
def _bg_runtime_status() -> str:
    ...
    parts.append("rembg_legacy=on" if rembg_remove is not None else f"rembg_legacy=off")
    return "; ".join(parts)

def _bg_use_comet() -> bool:
    return bool(COMET_API_KEY) and BG_PROVIDER in {"auto", "comet", "cometapi", "bria-comet", "bria/remove-background"}

def _bg_use_bria() -> bool:
    # Legacy emergency path only. Production v46 should use CometAPI.
    return bool(BRIA_API_KEY) and BG_PROVIDER in {"bria", "briai", "bria-ai", "legacy-bria"}

def _bg_use_local() -> bool:
    # Legacy emergency path only. Disabled by default because Render often misses onnxruntime wheels.
...
def _looks_like_image_bytes(data: bytes) -> bool:
    return bool(data) and (data.startswith(b"\x89PNG") or data.startswith(b"\xff\xd8\xff") or data.startswith(b"RIFF") or data.startswith(b"GIF8"))

def _extract_image_payloads(data):
    """Return image URLs/data-URLs/base64 strings from any Comet/Responses style JSON."""
    found = []
    seen = set()
    ...
    def add(x):
        ...
        if not s or s in seen:
            return
        # URLs and data URLs are unambiguous.
        if s.startswith(("http://", "https://", "data:image/")):
            seen.add(s); found.append(s); return
        # Some responses use raw base64 under fields like b64_json / base64 / image_base64.
        compact = s.replace("\n", "").replace("\r", "")
    ...
    def walk(obj, key_hint=""):
        if isinstance(obj, dict):
            ...
            for k in ("image_url", "url", "result_url", "output_url", "image", "b64_json", "base64", "image_base64", "data"):
                if k in obj:
                    v = obj.get(k)
            ...
            for k, v in obj.items():
                if k not in {"image_url", "url", "result_url", "output_url", "image", "b64_json", "base64", "image_base64", "data"}:
                    walk(v, k)
...
async def _comet_bg_get_status(client: httpx.AsyncClient, rid: str) -> dict | None:
    if not rid:
        return None
    url = f"{COMET_BASE_URL}{COMET_BG_STATUS_PATH.format(id=rid)}"
    try:
        r = await client.get(url, headers={"Authorization": f"Bearer {COMET_API_KEY}"})
        if r.status_code < 400:
            return r.json()
    except Exception as e:
        log.warning("Comet BG status error: %s", e)
    return None

async def _comet_remove_bg_bytes(img_bytes: bytes) -> bytes | None:
    """CometAPI background removal using model bria/remove-background via /v1/responses."""
    ...
    payloads = [
        {
            ...
            "input": [{
                "role": "user",
                "content": [
                    {"type": "input_text", "text": "Remove the background from this image and return a transparent PNG. Preserve the main foreground subject and soft alpha edges."},
                    {"type": "input_image", "image_url": data_url},
                ],
            }],
        },
        # Compatibility payload: some Comet routes accept flat fields.
        {
            "model": COMET_BG_MODEL,
    ...
    async with httpx.AsyncClient(timeout=COMET_BG_TIMEOUT_S, follow_redirects=True) as client:
        ...
        for payload in payloads:
            try:
                r = await client.post(f"{COMET_BASE_URL}{COMET_BG_REMOVE_PATH}", headers=headers, json=payload)
                if r.status_code >= 400:
                    last_err = f"{r.status_code}: {r.text[:700]}"
                    continue
                data = r.json()
                img = _extract_image_url_from_json(data)
                if img:
                    return await _download_image_url(img)
                rid = str(data.get("id") or data.get("response_id") or data.get("request_id") or data.get("task_id") or "") if isinstance(data, dict) else ""
                status_url = str(data.get("status_url") or data.get("urls", {}).get("get") or "") if isinstance(data, dict) else ""
                start = time.time()
                while rid and time.time() - start < COMET_BG_MAX_WAIT_S:
                    ...
                    if status_url:
                        try:
                            ...
                            sd = None
                    if sd is None:
                        sd = await _comet_bg_get_status(client, rid)
                    if not sd:
                        continue
                    img = _extract_image_url_from_json(sd)
                    if img:
                        return await _download_image_url(img)
                    st = str(sd.get("status") or sd.get("state") or "").lower() if isinstance(sd, dict) else ""
                    if st in {"failed", "error", "rejected", "cancelled", "canceled"}:
                        last_err = str(sd)[:700]
                        break
                last_err = str(data)[:700]
            except Exception as e:
                last_err = repr(e)
                continue
    if last_err:
...
async def _remove_bg_bytes_primary(img_bytes: bytes) -> bytes | None:
    """v46 production order: Comet bria/remove-background first. Legacy fallbacks only if explicitly enabled."""
    if _bg_use_comet():
        out = await _comet_remove_bg_bytes(img_bytes)
        if out:
            return out
    if _bg_use_bria():
...
async def _pedit_removebg(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes):
    try:
        ...
        if not out:
            await update.effective_message.reply_text(
                "Не удалось удалить фон через Comet. Проверьте COMET_API_KEY и доступность модели bria/remove-background в Comet.\n\n"
...
async def _pedit_replacebg(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, preset: str = "blur"):
    ...
    try:
        ...
        await update.effective_message.reply_text(
            "Не удалось заменить фон через Comet. Проверьте COMET_API_KEY и доступность модели bria/remove-background.\n"
            f"Диагностика: {_bg_runtime_status()}"
        )
