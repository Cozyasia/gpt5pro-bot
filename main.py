...
def _parse_video_hints(text: str, default_duration: int = 9, default_aspect: str = "16:9") -> tuple[int, str]:
    """Extract simple hints like '9 секунд', '5s', '9:16', '16:9' from text."""
    t = (text or "").lower()
    # duration
    dur = None
    m = re.search(r"(\d{1,2})\s*(?:сек|секунд|s)\b", t)
    if m:
        try:
            dur = int(m.group(1))
        except Exception:
    ...
    # aspect
    aspect = None
    m2 = re.search(r"\b(\d{1,2})\s*:\s*(\d{1,2})\b", t)
    if m2:
        aspect = f"{int(m2.group(1))}:{int(m2.group(2))}"
    return (dur or default_duration, aspect or default_aspect)
...
# ───────── Photo revive (image→video) dispatch ─────────
async def _tg_download_file_bytes(bot, file_id: str) -> bytes:
    f = await bot.get_file(file_id)
    b = await f.download_as_bytearray()
    return bytes(b)


async def _pedit_revive(update: Update, context: ContextTypes.DEFAULT_TYPE, img: bytes, data: str) -> None:
    """Revive last received photo using selected engine."""
    kind = (data.split(":")[-1] or "").strip().lower()
    file_id = context.user_data.get("last_photo_file_id")
    if not file_id:
        await update.effective_message.reply_text("⚠️ Не вижу последнего фото. Отправьте фото ещё раз.")
        return

    if kind == "runway":
        await _run_runway_animate_photo(update, context, file_id)
        return
    if kind == "luma":
        await _run_luma_animate_photo(update, context, file_id)
        return
    if kind == "sora2":
        await _run_comet_animate_photo(update, context, engine_key="sora2", file_id=file_id)
        return
    if kind == "kling":
        await _run_comet_animate_photo(update, context, engine_key="kling", file_id=file_id)
        return

    await update.effective_message.reply_text(f"⚠️ Неизвестный движок оживления: {kind}")


async def _run_comet_animate_photo(update: Update, context: ContextTypes.DEFAULT_TYPE, engine_key: str, file_id: str) -> None:
    """Image→Video via Comet (Sora2/Kling)."""
    chat_id = update.effective_chat.id
    msg = await context.bot.send_message(chat_id=chat_id, text=f"⏳ Оживляю фото через {engine_key}…")
    try:
        comet = CometClient()
        if not comet.enabled():
            raise CometError("COMET_API_KEY не задан. Проверьте переменные окружения.")
        img_bytes = await _tg_download_file_bytes(context.bot, file_id)
        import base64
        image_b64 = base64.b64encode(img_bytes).decode("utf-8")

        # базовые параметры (можно расширить позже подсказками/меню)
        duration_s = 5
        aspect = "16:9"

        if engine_key == "sora2":
            task_id = await comet.sora2_image_to_video(image_b64=image_b64, prompt="", duration_s=duration_s, aspect_ratio=aspect)
        elif engine_key == "kling":
            task_id = await comet.kling_image_to_video(image_b64=image_b64, prompt="", duration_s=duration_s, aspect_ratio=aspect)
        else:
            raise CometError(f"Неизвестный Comet-движок: {engine_key}")

        await msg.edit_text(f"⏳ Задача создана: {task_id}. Ожидаю результат…")
        video_url = await comet.wait_for_video(task_id, timeout_s=240, poll_s=4)

        async with httpx.AsyncClient(timeout=120) as client:
            r = await client.get(video_url)
        if r.status_code >= 400 or not r.content:
            raise CometError(f"Не удалось скачать видео: HTTP {r.status_code}")

        await context.bot.send_video(chat_id=chat_id, video=r.content)
        await msg.delete()
    except Exception as e:
        log.exception("Comet image-to-video error")
        try:
            await msg.edit_text(f"❌ {engine_key}: не удалось оживить фото.\n{str(e)[:500]}")
        except Exception:
            pass
    ...
    try:
        ...
        if data.startswith("pedit:"):
            ...
            if data.startswith("pedit:revive:"):
                await _pedit_revive(update, context, img, data); return
...
async def _run_luma_animate_photo(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, prompt: str, duration_s: int, aspect: str):
    """Пробуем оживить фото через Luma Dream Machine API.
    Важно: точный формат i2v может зависеть от версии/плана Luma. Если Luma вернет 4xx — покажем понятную ошибку.
    """
    if not LUMA_API_KEY:
    ...
    payload = {
        ...
        "aspect_ratio": aspect,
        # распространённый формат keyframes для i2v (может отличаться у разных версий API)
        "keyframes": {
            "frame0": {
                "type": "image",
    ...
    headers = {"Authorization": f"Bearer {LUMA_API_KEY}", "Accept": "application/json"}
    async with httpx.AsyncClient(timeout=60.0) as client:
        r = await client.post(f"{LUMA_BASE_URL}{LUMA_CREATE_PATH}", headers=headers, json=payload)
        if r.status_code >= 400:
        ...
        if not jid:
            await update.effective_message.reply_text("⚠️ Luma не вернул id задачи.")
            return

        await update.effective_message.reply_text("⏳ Оживляю фото в Luma… Сообщу, когда будет готово.")

        status_url = f"{LUMA_BASE_URL}{LUMA_STATUS_PATH}".format(id=jid)
        started = time.time()
        while True:
            rs = await client.get(status_url, headers=headers)
            js = {}
            ...
            if state in ("completed", "succeeded", "success"):
                # пробуем вытащить URL видео
                vid_url = (
                    (js.get("assets") or {}).get("video")
                    or (js.get("result") or {}).get("url")
                    or (js.get("output") or {}).get("url")
                )
                if not vid_url:
                    await update.effective_message.reply_text("⚠️ Luma: задача завершена, но ссылка на видео не найдена в ответе.")
                    return
                vr = await client.get(vid_url, timeout=60.0)
                if vr.status_code >= 400:
                    await update.effective_message.reply_text("⚠️ Luma: не удалось скачать результат.")
                    return
                await update.effective_message.reply_video(video=vr.content, filename="luma_i2v.mp4", caption="🎬 Luma — оживление фото")
                return

            if state in ("failed", "error", "canceled", "cancelled"):
                await update.effective_message.reply_text("❌ Luma: не удалось анимировать фото.")
                return

            if time.time() - started > 180:
                await update.effective_message.reply_text("⌛ Luma: слишком долго ждём результат. Попробуйте позже.")
                return

            await asyncio.sleep(3)

# ───────── Comet image→video (Sora 2 / Kling) ─────────
...
async def _run_comet_text_to_video(update: Update, context: ContextTypes.DEFAULT_TYPE, engine_key: str, prompt: str) -> None:
    """Text→Video via Comet (Sora2/Kling)."""
    chat_id = update.effective_chat.id
    msg = await context.bot.send_message(chat_id=chat_id, text="⏳ Запускаю генерацию видео…")
    try:
        comet = CometClient()
        if not comet.enabled():
            raise CometError("COMET_API_KEY не задан. Проверьте переменные окружения.")
        duration_s, aspect = _parse_video_hints(prompt)

        if engine_key == "sora2":
            task_id = await comet.sora2_text_to_video(prompt=prompt, duration_s=duration_s, aspect_ratio=aspect)
        elif engine_key == "kling":
            task_id = await comet.kling_text_to_video(prompt=prompt, duration_s=duration_s, aspect_ratio=aspect)
        else:
            raise CometError(f"Неизвестный Comet-движок: {engine_key}")
        ...
        log.exception("Comet text-to-video error")
        try:
            await msg.edit_text(f"❌ {engine_key}: не удалось запустить/получить видео.\n{str(e)[:500]}")
        except Exception:
            pass
