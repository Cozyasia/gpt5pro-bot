...
async def on_cb_mode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Handles mode and action selection from inline keyboards.

    Supported callback_data:
      - mode:set:<study|work|fun>           (set top-level mode and show submenu)
      - act:study:<action>                 (set study action)
      - act:work:<action>                  (set work action)
      - fun:<...>                          (handled by on_cb_fun, but we accept fallback)
      - study:set:<action>                 (legacy)
      - work:set:<action>                  (legacy)
      - school:<...>                       (legacy)
      - work:<...>                         (legacy)
    """
    q = update.callback_query
    ...
    if data.startswith("mode:set:"):
        ...
        if mode not in ("study", "work", "fun"):
            await q.edit_message_text("⚠️ Неизвестный режим.", reply_markup=main_menu_kb())
            return
        context.user_data["mode"] = mode
        if mode == "study":
            await q.edit_message_text("🎓 Режим: Учёба. Выберите действие:", reply_markup=study_kb())
            return
        if mode == "work":
            await q.edit_message_text("💼 Режим: Работа. Выберите действие:", reply_markup=work_kb())
            return
        if mode == "fun":
            await q.edit_message_text("🔥 Режим: Развлечения. Выберите действие:", reply_markup=fun_kb())
            return

    # ---- study actions ----
    if data.startswith("act:study:") or data.startswith("study:set:") or data.startswith("school:"):
        # normalize
        action = None
        if data.startswith("act:study:"):
            action = data.split(":", 2)[2]
        elif data.startswith("study:set:"):
            action = data.split(":", 2)[2]
        elif data.startswith("school:"):
            action = data.split(":", 1)[1]

        context.user_data["mode"] = "study"
        context.user_data["study_action"] = action

        if action in ("pdf", "doc", "docs", "epub"):
            await q.edit_message_text("📚 Учёба: пришлите PDF/EPUB/DOCX или текст — сделаю конспект/выжимку.")
            return
        if action in ("explain", "theory"):
            await q.edit_message_text("🔎 Учёба: напишите тему/вопрос — объясню и приведу примеры.")
            return
        if action in ("tasks", "solve", "problem"):
            await q.edit_message_text("🧮 Учёба: пришлите условие задачи — решу пошагово.")
            return
        if action in ("essay", "report", "referat"):
            await q.edit_message_text("✍️ Учёба: тема + требования + объём — подготовлю текст.")
            return
        if action in ("exam", "plan"):
            await q.edit_message_text("🗒 Учёба: напишите дисциплину и темы — составлю план подготовки.")
            return
        if action in ("runway", "midjourney", "stt_tts"):
            await q.edit_message_text("✅ Выбран инструмент для учебных задач. Отправьте запрос/файл.")
            return

        await q.edit_message_text("✅ Режим Учёба выбран. Отправьте запрос.")
        return

    # ---- work actions ----
    if data.startswith("act:work:") or data.startswith("work:set:") or data.startswith("work:"):
        action = None
        if data.startswith("act:work:"):
            action = data.split(":", 2)[2]
        elif data.startswith("work:set:"):
            action = data.split(":", 2)[2]
        elif data.startswith("work:"):
            action = data.split(":", 1)[1]

        context.user_data["mode"] = "work"
        context.user_data["work_action"] = action

        if action in ("copy", "text", "rewrite"):
            await q.edit_message_text("📝 Работа: пришлите текст/задачу — перепишу, улучшу стиль, сделаю оффер/пост.")
            return
        if action in ("biz", "strategy"):
            await q.edit_message_text("📊 Работа: опишите задачу/контекст — предложу план, гипотезы, структуру.")
            return
        if action in ("analysis", "audit"):
            await q.edit_message_text("🔍 Работа: пришлите данные/файл — сделаю анализ и выводы.")
            return
        if action in ("runway", "midjourney", "stt_tts"):
            await q.edit_message_text("✅ Выбран инструмент для рабочих задач. Отправьте запрос/файл.")
            return

        await q.edit_message_text("✅ Режим Работа выбран. Отправьте запрос.")
        return

    # ---- fallback ----
    await q.edit_message_text("✅ Готово. Отправьте запрос.")
...
_photo_cache = {}  # u
# user_id -> {"bytes": bytes, "url": str|None}
_photo_cache: dict[int, dict] = {}

def _cache_photo(user_id: int, data: bytes, url: str | None = None):
    try:
        _photo_cache[user_id] = {"bytes": data, "url": url}
    except Exception:
        pass

def _get_cached_photo(user_id: int) -> dict | None:
...
async def on_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if data.startswith("pedit:"):
            await q.answer()
            cached = _get_cached_photo(update.effective_user.id)
            ...
            if data.startswith("pedit:revive"):
                ...
                if provider == "runway":
                    ...
                    pay_engine = "luma"
                    async def _go():
                        await _run_luma_animate_photo(update, context, img, prompt="", duration_s=dur, aspect=asp)
...
async def _run_runway_animate_photo(
    ...
    """
    Runway Image→Video.
    Runway API expects a HTTPS URL for promptImage. Data URIs are unreliable and may be rejected.
    We prefer a Telegram-hosted file URL (server-side only).
    Docs: https://docs.dev.runwayml.com/api (Image to video)
    """
    if not RUNWAY_API_KEY:
        await update.effective_message.reply_text("⚠️ RUNWAY_API_KEY не задан в ENV.")
        return

    # Use Telegram URL if present; otherwise fall back to data URI
    ...
    duration = int(duration_s or 8)
    if duration not in (4, 6, 8, 10):
        # Runway typically supports specific durations; keep safe default
        duration = 8

    payload = {
        "model": RUNWAY_MODEL or "gen4_turbo",
    ...
    headers = {
        ...
        "Accept": "application/json",
        "X-Runway-Version": RUNWAY_VERSION or "2024-11-06",
        "Content-Type": "application/json",
    }

    async with httpx.AsyncClient(timeout=120.0) as client:
        r = await client.post(f"{RUNWAY_BASE_URL}/v1/image_to_video", headers=headers, json=payload)
        if r.status_code >= 400:
            await update.effective_message.reply_text(f"⚠️ Runway отклонил задачу ({r.status_code}).")
            return
        rid = (r.json() or {}).get("id")
        if not rid:
            await update.effective_message.reply_text("⚠️ Runway: не удалось получить id задачи.")
            return
        ...
        for _ in range(90):
            ...
            rr = await client.get(f"{RUNWAY_BASE_URL}{RUNWAY_STATUS_PATH.format(id=rid)}", headers=headers)
            ...
            if status in ("succeeded", "completed", "done"):
                # output location differs; support common shapes
                out = js.get("output") or js.get("outputs") or js.get("result") or {}
                url = None
                if isinstance(out, list) and out:
                    url = out[0].get("url") if isinstance(out[0], dict) else out[0]
                elif isinstance(out, dict):
                    url = out.get("url") or out.get("video") or out.get("videoUrl")
                if not url:
                    # some payloads keep it under "artifacts"
                ...
                if url:
                    ...
                    await update.effective_message.reply_text("✅ Runway: готово, но ссылка на видео не найдена в ответе.")
                return

        await update.effective_message.reply_text("⚠️ Runway: таймаут ожидания результата.")


async def _run_luma_animate_photo(
    update: Update,
    context: ContextTypes.DEFAULT_TYPE,
    img_bytes: bytes,
    img_url: str | None,
    ...
    """
    Luma Dream Machine Image→Video.

    В Luma чаще всего принимается HTTPS URL для кадра keyframes.frame0.url.
    Data-URI/base64 может отклоняться (400). Поэтому используем Telegram-hosted URL, если доступен.
    """
    if not LUMA_API_KEY:
        await update.effective_message.reply_text("⚠️ LUMA_API_KEY не задан в ENV.")
        return

    frame0_url = img_url
    if not frame0_url:
        # fallback to data URL (может быть отвергнуто Luma)
        b64 = base64.b64encode(img_bytes).decode("utf-8")
        mime = sniff_image_mime(img_bytes) or "image/jpeg"
        frame0_url = f"data:{mime};base64,{b64}"

    duration = int(duration_s or 5)
    if duration < 2:
        duration = 2
    if duration > 10:
        duration = 10
    ...
    payload = {
        ...
        "aspect_ratio": (aspect or "9:16").strip(),
        "loop": False,
        "duration": duration,
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        r = await client.post(f"{LUMA_BASE_URL}{LUMA_I2V_PATH}", headers=headers, json=payload)
        if r.status_code >= 400:
            await update.effective_message.reply_text(f"⚠️ Luma отклонил задачу ({r.status_code}). Проверь Luma API/план и формат i2v.")
            return
        js = r.json() or {}
        task_id = js.get("id") or js.get("generation_id") or js.get("task_id")
        if not task_id:
            await update.effective_message.reply_text("⚠️ Luma: не удалось получить id задачи.")
            return
        ...
        for _ in range(90):
            await asyncio.sleep(2)
            rr = await client.get(f"{LUMA_BASE_URL}{LUMA_STATUS_PATH.format(id=task_id)}", headers=headers)
            if rr.status_code >= 400:
                continue
            j2 = rr.json() or {}
            st = (j2.get("state") or j2.get("status") or "").lower()
            if st in ("failed", "error"):
                await update.effective_message.reply_text("❌ Luma: не удалось оживить фото.")
                return
            if st in ("completed", "succeeded", "success", "done"):
                url = None
                # common structures
                for k in ("video_url", "videoUrl", "url"):
                    if k in j2 and isinstance(j2[k], str):
                        url = j2[k]
                        break
                if not url:
                    out = j2.get("output") or j2.get("outputs") or {}
                    if isinstance(out, dict):
                        url = out.get("url") or out.get("video")
                ...
                if url:
                    ...
                    await update.effective_message.reply_text("✅ Luma: готово, но ссылка на видео не найдена в ответе.")
                return

        await update.effective_message.reply_text("⚠️ Luma: таймаут ожидания результата.")
...
async def _run_comet_animate_photo(update: Update, context: ContextTypes.DEFAULT_TYPE, provider: str, img_bytes: bytes, prompt: str):
    if not COMET_BASE_URL or not COMET_API_KEY:
        await update.effective_message.reply_text(f"⚠️ {provider}: не настроен Comet API (нужны COMET_BASE_URL и COMET_API_KEY).")
        return
    client = CometClient(COMET_BASE_URL, COMET_API_KEY, timeout=COMET_TIMEOUT)
    b64 = base64.b64encode(img_bytes).decode("ascii")
    ...
    try:
        if provider == "sora":
            video_bytes, filename = await client.sora2_image_to_video(prompt or "Animate the input photo realistically", b64, mime=mime)
        elif provider == "kling":
            video_bytes, filename = await client.kling_image_to_video(prompt or "Animate the input photo realistically", b64, mime=mime)
        else:
            raise CometError("Unknown provider")
        with contextlib.suppress(Exception):
            await msg.delete()
        await update.effective_message.reply_video(video=video_bytes, filename=filename, caption=f"🎞 {provider} — оживление фото")
    except CometError as e:
...
async def _run_comet_text_to_video(update: Update, context: ContextTypes.DEFAULT_TYPE, provider: str, prompt: str):
    ...
    try:
        ...
        with contextlib.suppress(Exception):
            await msg.delete()
        await update.effective_message.reply_video(video=video_bytes, filename=filename, caption=f"🎞 {provider} — готово")
    except CometError as e:
        await msg.edit_text(f"❌ {provider}: ошибка.\n{e}")
...
async def on_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        img = bytes(data)
        # Telegram-hosted file URL (used only server-side for providers that require HTTPS URLs)
        tg_url = None
        try:
            if getattr(f, "file_path", None) and TELEGRAM_TOKEN:
                tg_url = f"https://api.telegram.org/file/bot{TELEGRAM_TOKEN}/{f.file_path}"
        except Exception:
            tg_url = None
        _cache_photo(update.effective_user.id, img, tg_url)
...
def build_application() -> "Application":
    ...
    app.add_handler(CallbackQueryHandler(on_cb_mode, pattern=r"^(?:mode:|act:|school:|work:|study:)" ))
