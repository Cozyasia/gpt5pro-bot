...
def _telegram_file_url_from_file(tg_file) -> str | None:
    """Публичный URL на файл Telegram вида:
    https://api.telegram.org/file/bot<TOKEN>/<file_path>
    """
    try:
        fp = getattr(tg_file, "file_path", None)
        if not fp or not BOT_TOKEN:
            return None
        ...
        if data.startswith("pedit:"):
            ...
            img = _cached[0]
                ...
                if provider == "runway":
                    ...
                    pay_engine = "runway"
                    async def _go():
                        await _run_runway_animate_photo(update, context, img, img_url=img_url, prompt="", duration_s=dur, aspect=asp)
                elif provider == "luma":
                    est = 0.40
                    pay_engine = "luma"
                    async def _go():
                        await _run_luma_animate_photo(update, context, img, img_url=img_url, prompt="", duration_s=dur, aspect=asp)
