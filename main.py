...
async def _run_comet_i2v(update: Update, context: ContextTypes.DEFAULT_TYPE, engine: str, img_bytes: bytes, prompt: str, duration_s: int, aspect: str):
    ...
    if engine == "sora":
        ...
        # ВАЖНО: Comet/Sora на /v1/videos уже возвращал:
        # Unknown parameter: aspect_ratio и Unknown parameter: duration.
        # Поэтому aspect_ratio не шлём, а длительность передаём как seconds.
        # Если конкретный канал Comet не примет seconds, в конце есть fallback без seconds.
        payloads = []

        # 1) Вариант с image_urls. Для многих Sora-шлюзов это основной image→video формат.
        payloads.append((
            SORA_CREATE_PATH,
            {
                "model": SORA_MODEL,
        ...
        # 2) Вариант с image_url — оставляем для Comet-совместимости.
        payloads.append((
            SORA_CREATE_PATH,
            {
                "model": SORA_MODEL,
        ...
        # 3) Вариант input_reference. В Comet поле input_reference должно быть строкой,
        # не объектом: иначе API отвечает "cannot unmarshal object ... input_reference of type string".
        payloads.append((
            SORA_CREATE_PATH,
            {
                "model": SORA_MODEL,
        ...
        # 4) Ещё один распространённый alias для image→video-шлюзов.
        payloads.append((
            SORA_CREATE_PATH,
            {
                "model": SORA_MODEL,
        ...
        # Запасной вариант через Telegram URL, если провайдер не принимает data-uri.
        ...
        # Последний fallback: некоторые Comet-каналы сами выбирают длительность
        # и отклоняют любые duration/seconds.
        payloads.extend([
            (SORA_CREATE_PATH, {"model": SORA_MODEL, "prompt": prompt, "image_urls": [data_url]}),
            (SORA_CREATE_PATH, {"model": SORA_MODEL, "prompt": prompt, "image_url": data_url}),
            (SORA_CREATE_PATH, {"model": SORA_MODEL, "prompt": prompt, "input_reference": data_url}),
            (SORA_CREATE_PATH, {"model": SORA_MODEL, "prompt": prompt, "input_image": data_url}),
        ])
        if tg_url and tg_url.startswith("https://"):
            payloads.extend([
                (SORA_CREATE_PATH, {"model": SORA_MODEL, "prompt": prompt, "image_urls": [tg_url]}),
                (SORA_CREATE_PATH, {"model": SORA_MODEL, "prompt": prompt, "image_url": tg_url}),
                (SORA_CREATE_PATH, {"model": SORA_MODEL, "prompt": prompt, "input_reference": tg_url}),
                (SORA_CREATE_PATH, {"model": SORA_MODEL, "prompt": prompt, "input_image": tg_url}),
            ])
