...
def _fix_text(s: str) -> str:
    """Best-effort repair for mojibake/garbled UTF-8-in-Latin1 strings."""
    if s is None:
        return ''
    if not isinstance(s, str):
        s = str(s)
    # Fast path: nothing suspicious
    if not any(ch in s for ch in ('Ã','â','Ð','Ñ','ð')) and not any(0x80 <= ord(c) <= 0x9F for c in s):
        return s
    try:
        return s.encode('latin1').decode('utf-8')
    except Exception:
        # fallback: keep printable chars
...
def t(user_id: int, key: str) -> str:
    ...
    return _fix_text((I18N.get(lang) or I18N["ru"]).get(key, key))
...
def system_prompt_for(lang: str) -> str:
    ...
    return _fix_text(mapping.get(lang, mapping["ru"]))
...
def _lang_choose_kb(user_id: int | None = None) -> InlineKeyboardMarkup:
    ...
    if uid and has_lang(uid):
        cur = get_lang(uid)
        cur_name = _fix_text(lang_label(cur, cur))
        rows.append([InlineKeyboardButton(f"â¡ï¸ ÐÑÐ¾Ð´Ð¾Ð»Ð¶Ð¸ÑÑ ({cur_name})", callback_data="lang:__keep__")])
    for code in LANGS:
        rows.append([InlineKeyboardButton(lang_label(code), callback_data=f"lang:{code}")])
