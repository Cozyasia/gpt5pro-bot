...
PATCH_VERSION = "v40-comet-bria-remove-background-endpoint-2026-05-25"
...
COMET_BRIA_BASE_URL = os.environ.get("COMET_BRIA_BASE_URL", COMET_BASE_URL or "https://api.cometapi.com").strip().rstrip("/")
COMET_BRIA_REMOVEBG_PATHS = [
    p.strip() for p in os.environ.get(
        "COMET_BRIA_REMOVEBG_PATHS",
        "/bria/image/edit/remove_background,/bria/image/edit/remove-background,/bria/image/edit/remove_bg,/bria/remove-background,/v1/responses,/replicate/v1/models/bria/remove-background/predictions"
    ).split(",") if p.strip()
]
COMET_BRIA_STATUS_PATHS = [
    p.strip() for p in os.environ.get("COMET_BRIA_STATUS_PATHS", "/bria/{id},/bria/image/edit/{id},/replicate/v1/predictions/{id}").split(",") if p.strip()
]
...
async def _v39_comet_bria_cutout(img_bytes: bytes, *, timeout_s: float | None = None) -> tuple[bytes, tuple[int, int]]:
    ...
    json_payloads = [
        # Bria-compatible v2 endpoint: preferred. Use raw base64, not data URL.
...
async def _pedit_removebg(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes):
    ...
    try:
        ...
        await update.effective_message.reply_text(
            ...
            "Проверьте COMET_API_KEY, баланс/доступ к модели bria/remove-background и при необходимости настройте "
            "COMET_BRIA_REMOVEBG_PATHS, если в вашем Comet-кабинете указан другой endpoint."
        )
...
async def _pedit_replacebg_apply(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, prompt: str, label: str = "новый фон"):
    ...
    try:
        with contextlib.suppress(Exception):
            await context.bot.send_chat_action(update.effective_chat.id, ChatAction.UPLOAD_PHOTO)
        await update.effective_message.reply_text(f"🖼 Принял: ставлю фон «{label}» через строгий Comet/Bria pipeline…")
        ...
        await update.effective_message.reply_text(
            ...
            "Проверьте COMET_API_KEY, баланс/доступ к модели bria/remove-background, OPENAI_IMAGE_KEY для красивых фонов "
            "и при необходимости настройте COMET_BRIA_REMOVEBG_PATHS."
        )
