...
PATCH_VERSION = "v83-video-credits-chats-midjourney-2026-07-12"
...
RUNWAY_5S_COST_USD = _env_float("RUNWAY_5S_COST_USD", 0.32)
...
def _video_provider_cost_usd(engine: str, duration_s: int) -> float:
    ...
    if engine == "runway":
        return round(RUNWAY_5S_COST_USD * (2 if d >= 10 else 1), 4)
...
ENGINE_INFO_TEXT = {
    ...
    "runway": (
        "🎥 Runway: видео по тексту и оживление фото.\n"
        "Для text→video доступен вместе с Sora 2 без людей и Kling.\n"
...
def _textvideo_menu_text() -> str:
    return (
        ...
        "• *Runway* — люди и кинематографичные сцены; доступность зависит от канала Comet/Runway.\n\n"
...
async def _run_runway_video(update: Update, context: ContextTypes.DEFAULT_TYPE, prompt: str, duration_s: int, aspect: str):
            ...
            for base in _runway_direct_base_candidates():
                for model in _runway_direct_text_model_candidates():
                    ...
                    try:
                        ...
                        return bool(await _poll_video_task_generic(
    ...
    if not RUNWAY_API_KEY and not COMET_API_KEY:
        await update.effective_message.reply_text(
            "❌ Runway: нет RUNWAY_API_KEY и COMET_API_KEY в Render ENV. Для Runway нужен хотя бы один ключ."
        )
        return False
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=90.0) as client:
            ...
            for path, payload in payload_variants:
                ...
                return bool(await _poll_video_task_generic(update, client, headers, RUNWAY_BASE_URL, status_paths, rid, "Runway image→video", RUNWAY_MAX_WAIT_S))
