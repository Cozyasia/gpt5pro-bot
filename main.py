...
PATCH_VERSION = "v61-clean-photo-music-clip-pipeline-2026-07-09"
...
PHOTO_CLIP_SEND_BASE_IF_MUX_FAILS = os.environ.get("PHOTO_CLIP_SEND_BASE_IF_MUX_FAILS", "1").strip().lower() not in ("0", "false", "no", "off")
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with open(vin, "wb") as f:
                f.write(video_bytes)
            cmd = [ffmpeg, "-y", "-stream_loop", "-1", "-i", vin]
            if audio_bytes:
                with open(a_in, "wb") as f:
                    f.write(audio_bytes)
                cmd += ["-stream_loop", "-1", "-i", a_in, "-t", str(target_duration_s), "-map", "0:v:0", "-map", "1:a:0", "-c:v", "libx264", "-preset", "veryfast", "-pix_fmt", "yuv420p", "-c:a", "aac", "-b:a", "160k", "-shortest", out]
            else:
                cmd += ["-t", str(target_duration_s), "-an", "-c:v", "libx264", "-preset", "veryfast", "-pix_fmt", "yuv420p", out]
            res = subprocess.run(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=240)
            if res.returncode != 0:
                log.warning("ffmpeg mux failed rc=%s stderr=%s", res.returncode, res.stderr.decode("utf-8", "ignore")[-1200:])
                return None
            with open(out, "rb") as f:
                final = f.read()
            return final if final and len(final) > 512 else None
    except Exception as e:
...
async def _start_photo_music_clip(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, user_prompt: str):
    ...
    async def _go():
        try:
            await update.effective_message.reply_text(
                f"🎬 Фото→видеоклип принят: делаю один итоговый MP4. Видео-движок: Kling; музыка: {'Suno' if SUNO_AUTO_FOR_PHOTO_CLIP else 'выключена'}; итоговая длина: ~{target_duration} сек."
            )
            await context.bot.send_chat_action(update.effective_chat.id, ChatAction.RECORD_VIDEO)
            video_bytes = await _run_kling_photo_clip_result(img_bytes, user_prompt, base_duration, aspect)
            if not video_bytes:
                raise RuntimeError("Kling не вернул видео")

            audio_bytes = None
            if PHOTO_CLIP_MUX_AUDIO and SUNO_AUTO_FOR_PHOTO_CLIP and SUNO_ENABLED and SUNO_API_KEY:
                await update.effective_message.reply_text("🎵 Генерирую музыку/песню через Suno и собираю её в итоговый MP4…")
                audio_bytes = await _run_suno_music_result_bytes(update, user_prompt)
                if not audio_bytes:
                    await update.effective_message.reply_text("⚠️ Suno не вернул музыку вовремя. Отправлю клип без музыки, чтобы не держать задачу бесконечно.")
            elif PHOTO_CLIP_MUX_AUDIO:
                await update.effective_message.reply_text("⚠️ Музыка не будет добавлена: включите SUNO_AUTO_FOR_PHOTO_CLIP=1 и проверьте SUNO_API_KEY/COMET_API_KEY.")

            final_bytes = None
            if PHOTO_CLIP_PIPELINE:
                final_bytes = await asyncio.to_thread(_mux_video_audio_sync, video_bytes, audio_bytes, target_duration)
            if not final_bytes and PHOTO_CLIP_SEND_BASE_IF_MUX_FAILS:
                final_bytes = video_bytes
            if not final_bytes:
                raise RuntimeError("не удалось собрать итоговый MP4")

            caption = f"Kling photo→music clip ✅ Один итоговый MP4 · ~{target_duration} сек"
            if audio_bytes:
                caption += " · музыка добавлена"
            else:
                caption += " · без музыки"
            ...
            log.exception("photo music clip pipeline failed: %s", e)
    ...
    await _try_pay_then_do(
        ...
        remember_kind="kling_photo_music_clip_v61_pipeline",
