...
PATCH_VERSION = "v79-tariffs-payment-methods-2026-07-12"
...
RUNWAY_PUBLIC_FALLBACK_TEXT = os.environ.get("RUNWAY_PUBLIC_FALLBACK_TEXT", "⚠️ Runway сейчас нестабилен у провайдера. Запускаю резервный Kling, чтобы вы получили результат.").strip()
...
SORA_UNIT_COST_USD = _env_float("SORA_UNIT_COST_USD", 1.20)
SORA_PRO_UNIT_COST_USD = _env_float("SORA_PRO_UNIT_COST_USD", 1.80)
KLING_UNIT_COST_USD = _env_float("KLING_UNIT_COST_USD", 0.60)
...
PLAN_PRICE_TABLE = {
    # v78: безопасная экономика — подписка + кредиты для тяжёлых генераций.
...
def activate_subscription_with_tier(user_id: int, tier: str, months: int):
    ...
    # v78: при покупке подписки начисляем месячный пакет кредитов.
    try:
...
# v78 credits: 1 кредит = примерно 10 ₽ = $0.10 при USD_RUB=100.
CREDIT_RUB_VALUE = float(os.environ.get("CREDIT_RUB_VALUE", "10") or 10)
CREDIT_USD_VALUE = float(os.environ.get("CREDIT_USD_VALUE", str(CREDIT_RUB_VALUE / max(1e-9, USD_RUB))) or 0.10)
SUBSCRIPTION_CREDITS = {
    "free": 0,
    "start": int(os.environ.get("START_INCLUDED_CREDITS", "20") or 20),
    "pro": int(os.environ.get("PRO_INCLUDED_CREDITS", "120") or 120),
    "ultimate": int(os.environ.get("ULT_INCLUDED_CREDITS", "350") or 350),
}
CREDIT_PACKAGES_RUB = {
    int(os.environ.get("CREDIT_PACK_SMALL_CREDITS", "100") or 100): int(os.environ.get("CREDIT_PACK_SMALL_RUB", "990") or 990),
    int(os.environ.get("CREDIT_PACK_MID_CREDITS", "300") or 300): int(os.environ.get("CREDIT_PACK_MID_RUB", "2490") or 2490),
    int(os.environ.get("CREDIT_PACK_BIG_CREDITS", "700") or 700): int(os.environ.get("CREDIT_PACK_BIG_RUB", "4990") or 4990),
}
...
LIMITS = {
    # В v78 тяжёлые функции оплачиваются кредитами из единого баланса.
...
def _textvideo_menu_text() -> str:
    return (
        ...
        "Опишите сцену текстом или отправьте voice — я распознаю речь и запущу video generation.\n\n"
        "Движки:\n"
        "• *Kling* — стабильнее через Comet, хороший универсальный text→video.\n"
        "• *Runway* — можно пробовать для более кинематографичной картинки, но доступ зависит от канала/ключа.\n\n"
        "Пример: `ночной неоновый город, спорткар едет по мокрой улице, cinematic, 16:9, 10 секунд`."
    )


def _textvideo_action_kb(prefix: str = "act") -> InlineKeyboardMarkup:
    base = "act:fun" if prefix == "act" else "fun"
    rows = [[InlineKeyboardButton("🎬 Kling text→video", callback_data=f"{base}:tv_engine_kling")]]
    if TEXT_VIDEO_ALLOW_RUNWAY:
        rows.append([InlineKeyboardButton("🚀 Runway text→video", callback_data=f"{base}:tv_engine_runway")])
    rows.extend([
...
async def _handle_textvideo_engine_choice(update: Update, context: ContextTypes.DEFAULT_TYPE, q, engine: str, prefix: str = "act"):
    ...
    label = "Runway" if engine == "runway" else "Kling"
    await q.answer(label)
    await q.message.reply_text(
        f"✅ Выбран движок: {label}. Теперь напишите текстовый промпт или отправьте voice.\n"
        "Укажите сцену, стиль, длительность и формат: например `10 секунд, 16:9`."
    )
...
async def _handle_textvideo_prompt_choice(update: Update, context: ContextTypes.DEFAULT_TYPE, q, prefix: str = "act"):
    ...
    await q.message.reply_text(
        ...
        "Можно сначала выбрать Runway/Kling кнопкой выше."
    )
...
async def on_cb_fun(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "fun:clip":
        return await q.message.reply_text("Пришли текст/голос и формат (Reels/Shorts), музыку/стиль — соберу клип. Для генерации видео доступны Sora 2 и Kling; Runway — только оживление фото.")
...
# v79: прямые платежи ЮKassa по ссылке/QR/банковским приложениям.
# Это НЕ Telegram Payments invoice, а API ЮKassa /v3/payments + polling статуса.
YOO_SHOP_ID = os.environ.get("YOO_SHOP_ID", "").strip() or os.environ.get("YOOKASSA_SHOP_ID", "").strip()
YOO_SECRET_KEY = os.environ.get("YOO_SECRET_KEY", "").strip() or os.environ.get("YOOKASSA_SECRET_KEY", "").strip()
...
SUBS_TIERS = {
    "start": {
        ...
        "rub": PRICE_START_RUB,
        "usd": PRICE_START_USD,
        "credits": SUBSCRIPTION_CREDITS.get("start", 20),
        "features": [
            ...
            "⚙️ Обычная очередь",
            f"🪙 {SUBSCRIPTION_CREDITS.get('start', 20)} кредитов каждый месяц",
        ],
    },
    "pro": {
        "title": "PRO",
        "rub": PRICE_PRO_RUB,
        "usd": PRICE_PRO_USD,
        "credits": SUBSCRIPTION_CREDITS.get("pro", 120),
        "features": [
            ...
            "⚡ Ускоренная очередь",
            f"🪙 {SUBSCRIPTION_CREDITS.get('pro', 120)} кредитов каждый месяц",
        ],
    },
    "ultimate": {
        "title": "ULTIMATE",
        "rub": PRICE_ULT_RUB,
        "usd": PRICE_ULT_USD,
        "credits": SUBSCRIPTION_CREDITS.get("ultimate", 350),
        "features": [
            ...
            f"🪙 {SUBSCRIPTION_CREDITS.get('ultimate', 350)} кредитов каждый месяц",
        ],
    },
}
...
def _plans_overview_text(user_id: int) -> str:
    parts = [
        "⭐ Подписка и тарифы",
        "Выбери подходящий уровень. Подписка открывает доступ, а кредиты оплачивают тяжёлые генерации, чтобы тарифы не уходили в минус.",
...
def plan_pay_kb(plan_key: str) -> InlineKeyboardMarkup:
    ...
    if YOO_CARD_ENABLED:
        app_row2.append(InlineKeyboardButton("💳 Карта", callback_data=f"pay:yookassa:{plan_key}"))
    if app_row2:
...
YOO_DIRECT_METHODS = {
    ...
    "yoo_sberpay": {"type": "sberbank",     "confirmation": "external", "label": "🟢 SberPay"},
...
async def _yoo_create_direct_payment(user_id: int, plan_key: str, months: int, method_key: str) -> dict:
    ...
    payload = {
        ...
        "payment_method_data": {"type": method["type"]},
        "confirmation": {"type": confirmation_type},
    }
    if confirmation_type in ("redirect", "external"):
        # Для external ЮKassa обычно тоже принимает return_url; если метод его проигнорирует, вреда нет.
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        ...
        if r.status_code >= 400:
            raise RuntimeError(f"YooKassa {r.status_code}: {r.text[:500]}")
        return r.json()
    ...
    if data.startswith("pay:"):
        ...
        if method in YOO_DIRECT_METHODS:
            ...
            if not _yoo_direct_configured():
                await q.message.reply_text(
                    "⚠️ Быстрая оплата ЮKassa пока не настроена: нужны YOO_SHOP_ID и YOO_SECRET_KEY."
                )
                return
            try:
                ...
                await q.message.reply_text("⚠️ Не удалось создать быструю оплату ЮKassa. Попробуйте карту, CryptoBot или позже.")
            return
...
async def _start_vocal_clip(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, user_prompt: str):
    ...
    async def _job():
        ...
        try:
            ...
            if ok:
                _register_engine_spend(update.effective_user.id, "runway", VOCAL_CLIP_UNIT_COST_USD)
        except Exception as e:
...
async def _start_text_video(update: Update, context: ContextTypes.DEFAULT_TYPE, prompt: str):
    ...
    if engine == "runway" and not TEXT_VIDEO_ALLOW_RUNWAY:
        engine = "kling"
    if engine not in ("runway", "kling"):
        engine = "kling"
    est = max(1.0, RUNWAY_UNIT_COST_USD) if engine == "runway" else KLING_UNIT_COST_USD
    label = "Runway" if engine == "runway" else "Kling"

    async def _go():
        await update.effective_message.reply_text(f"🎬 Запускаю video generation через {label}: {duration} сек · {aspect}.")
        if engine == "runway":
    ...
    await _try_pay_then_do(
        ...
        remember_payload={"prompt": prompt[:500], "duration": duration, "aspect": aspect},
    )
...
async def _start_photo_music_clip(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, user_prompt: str):
    ...
    async def _go():
        if PHOTO_CLIP_BACKGROUND_TASK:
            # Не держим webhook-обработчик: иначе Telegram может повторить update, и появляются дубли.
            try:
                context.application.create_task(_photo_clip_job())
            except Exception:
                asyncio.create_task(_photo_clip_job())
            return
        await _photo_clip_job()
    ...
    try:
        ...
        if data.startswith("buyyoo:"):
            ...
            try:
                ...
                if not method_key:
                    ...
                    return
                if not _yoo_direct_configured():
                    await q.message.reply_text("⚠️ Быстрая оплата ЮKassa пока не настроена: нужны YOO_SHOP_ID и YOO_SECRET_KEY.")
                    return
                ...
                await q.message.reply_text("⚠️ Не удалось создать оплату ЮKassa. Попробуйте другой способ.")
            return
        ...
        if data.startswith("engine:"):
            ...
            if engine == "luma" and LUMA_TEMP_DISABLED:
                await q.message.reply_text("⚠️ Luma временно отключена и скрыта из меню. Используйте Sora 2, Kling, Images или другие доступные движки. Runway — только для оживления фото.")
                return
        ...
        if data.startswith("choose:"):
            ...
            if engine == "luma" and LUMA_TEMP_DISABLED:
                _pending_actions.pop(aid, None)
                await q.message.reply_text("⚠️ Luma временно отключена. Выберите Sora 2 или Kling.")
                return
...
async def cmd_diag_runway(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    /diag_runway test — пробует создать реальную i2v задачу по последнему фото пользователя.
    """
    args = [str(a).lower() for a in (context.args or [])]
    if "reset" in args:
        _provider_reset("runway_i2v")
        await update.effective_message.reply_text("✅ Runway provider cooldown сброшен.")
        return
    ...
    lines = [
        ...
        f"• hide_tech_errors={'✅' if RUNWAY_HIDE_TECH_ERRORS else '—'}  fallback_kling={'✅' if RUNWAY_AUTO_FALLBACK_KLING else '—'}",
        f"• cooldown_left={_provider_cooldown_left('runway_i2v')}s  fails={_provider_fail_counts.get('runway_i2v', 0)}",
    ]
    ...
    await update.effective_message.reply_text("\n".join(lines)[:2500] + "\n\n▶️ Запускаю реальный короткий тест Runway i2v. Это может списать кредит у провайдера.")
    ok = await _run_runway_comet_animate_photo(
        update, context, img,
        "subtle portrait animation, keep identity, small natural motion",
        5, "9:16",
    )
    if ok:
        await update.effective_message.reply_text("✅ Runway/Comet test: задача отработала.")
    else:
        await update.effective_message.reply_text("⚠️ Runway/Comet test: сейчас недоступен или не принял задачу. Резервный Kling остаётся основным fallback.")
...
async def _run_runway_animate_photo(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, prompt: str, duration_s: int, aspect: str):
    ...
    if not RUNWAY_API_KEY:
        if RUNWAY_AUTO_FALLBACK_KLING and COMET_API_KEY:
            ...
            return
        await update.effective_message.reply_text("⚠️ Runway сейчас недоступен, а резервный Kling не настроен.")
        return
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=90.0) as client:
            ...
            log.warning("Runway direct create failed finally: %s", last_err)
            if RUNWAY_AUTO_FALLBACK_KLING and COMET_API_KEY:
                await update.effective_message.reply_text(RUNWAY_PUBLIC_FALLBACK_TEXT)
                await _run_comet_i2v(update, context, "kling", img_bytes, prompt, seconds, aspect)
                return
            await update.effective_message.reply_text("⚠️ Runway сейчас не создал задачу. Попробуйте Kling или повторите позже.")
        ...
        if RUNWAY_AUTO_FALLBACK_KLING and COMET_API_KEY:
            ...
            await _run_comet_i2v(update, context, "kling", img_bytes, prompt, seconds, aspect)
            return
        await update.effective_message.reply_text("⚠️ Runway сейчас недоступен. Попробуйте Kling или повторите позже.")
...
async def _send_topup_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # v78: пользователю показываем кредиты, внутри баланс хранится в USD-эквиваленте.
    small_cr = int(os.environ.get("CREDIT_PACK_SMALL_CREDITS", "100") or 100)
    mid_cr = int(os.environ.get("CREDIT_PACK_MID_CREDITS", "300") or 300)
    big_cr = int(os.environ.get("CREDIT_PACK_BIG_CREDITS", "700") or 700)
...
async def _try_pay_then_do(
    ...
    # Спец-доступ для промо-пользователей: GPT безлимитный, а каждая платная функция — N раз в день отдельно.
    promo_ok, promo_left, promo_limit, promo_feature = _try_consume_promo_daily5_quota(user_id, username, engine, remember_kind)
    if promo_ok:
        try:
            with contextlib.suppress(Exception):
                await update.effective_message.reply_text(
                    f"🎁 Промо-доступ: функция «{promo_feature}». Осталось сегодня: {promo_left}/{promo_limit}."
                )
            await coro_func()
        except Exception as e:
    ...
    if is_promo_daily5_user(user_id, username) and promo_feature:
        await update.effective_message.reply_text(
            f"Промо-лимит на функцию «{promo_feature}» сегодня исчерпан: {promo_limit}/{promo_limit}. "
            "GPT-чат остаётся безлимитным, остальные функции будут снова доступны завтра."
        )
        return
    ...
    if ok:
        try:
            ...
            # v78: если задача не запустилась исключением, возвращаем списанные кредиты.
            with contextlib.suppress(Exception):
                if get_subscription_tier(user_id) != "free" and not is_unlimited(user_id, username):
                    _wallet_total_add(user_id, float(est_cost_usd))
            log.exception("paid action failed: %s", e)
            await update.effective_message.reply_text(f"❌ Задача не запустилась. Кредиты возвращены. Попробуйте позже или напишите в поддержку.")
        return
    if offer == "ASK_SUBSCRIBE":
        await update.effective_message.reply_text(
            "Для выполнения нужен тариф или кредиты на балансе.",
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("⭐ Тарифы", web_app=WebAppInfo(url=TARIFF_URL))],
                 [InlineKeyboardButton("➕ Пополнить баланс", callback_data="topup")]]
            )
    ...
    await update.effective_message.reply_text(
        f"Недостаточно кредитов. Нужно примерно: {_credits_fmt_from_usd(need_usd)}. Пополните кредиты или выберите тариф:",
        reply_markup=InlineKeyboardMarkup(
            [
                [InlineKeyboardButton("⭐ Тарифы", web_app=WebAppInfo(url=TARIFF_URL))],
                [InlineKeyboardButton("➕ Пополнить баланс", callback_data="topup")],
