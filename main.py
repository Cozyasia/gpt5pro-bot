...
PATCH_VERSION = "v34.8-memory-voice-clean-2026-07-07"
...
async def maybe_handle_live_query(update: Update, context: ContextTypes.DEFAULT_TYPE, user_text: str) -> bool:
    ...
    Поддерживает короткие follow-up ответы: «биткоин», «доллар», «завтра» и т.п.
    """
    ...
    text = _chat_memory_followup_query(user_id, chat_id, original_text)
...
def _chat_memory_followup_query(user_id: int | None, chat_id: int | None, text: str) -> str:
    """
    Если пользователь отвечает одним словом/короткой фразой на предыдущий вопрос,
    расширяем запрос для live/GPT слоя, чтобы не терялся контекст.
    """
    t = (text or "").strip()
    if not CHAT_MEMORY_ENABLED or not t or len(t) > 60:
        return t
