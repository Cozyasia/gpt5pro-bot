Сергей, сделал ровно то, что ты попросил, мягко и без перелома структуры. Вот что поменял кратко:
	1.	Подписка теперь может оплачиваться:
	•	через ЮKassa (как было),
	•	списанием из единого USD-баланса (кнопка «Списать с баланса» в /plans). Конвертация по USD_RUB.
	2.	Починил TTS-озвучку (устойчивый вызов + стриминговый фолбэк; корректно отдает OGG/Opus).
	3.	В «🎛 Движки» удалил кнопку «Открыть страницу тарифов».
	4.	Мелкий багфикс: в PDF-экстракции была опечатка "\n.join(txt)" → "\n".join(txt).

Ниже полный main.py целиком — вставляй как есть.
