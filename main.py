...
PATCH_VERSION = "v46-official-removebgbd-fallback-2026-05-26"
...
async def _pedit_removebg(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes):
    """v39: strict remove background via Comet/RemovebgBD + local gray composite."""
    ...
    try:
        ...
        bio = BytesIO(out); bio.name = "strict_background_gray.jpg"
        await update.effective_message.reply_photo(
            InputFile(bio),
            caption="✅ Готово: фон удалён через строгий cutout-провайдер, поставлена нейтральная серая подложка. Основной объект взят из оригинала, не перерисован."
        )
    except Exception as e:
        log.exception("v46 strict cutoutBD remove background error: %s", e)
        ...
        await update.effective_message.reply_text(
            ...
            "Fallback: добавьте REMOVEBG_BD_API_KEY или BRIA_API_TOKEN/REMOVE_BG_API_KEY. "
...
async def _pedit_replacebg_apply(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, prompt: str, label: str = "новый фон"):
    ...
    try:
        with contextlib.suppress(Exception):
            await context.bot.send_chat_action(update.effective_chat.id, ChatAction.UPLOAD_PHOTO)
        await update.effective_message.reply_text(f"🖼 Принял: ставлю фон «{label}» через строгий pipeline v46…")
        source_url = _get_cached_photo_url(update.effective_user.id)
...
async def cmd_diag_bria(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "🧪 Strict background diagnostics v46\n"
