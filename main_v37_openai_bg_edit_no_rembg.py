...
PATCH_VERSION = "v37-openai-bg-edit-no-rembg-2026-05-25"
...
# =====================================================================
# v37: Photo background tools via OpenAI image edit, without rembg/onnxruntime
# =====================================================================

async def _openai_image_edit_raw_bytes(img_bytes: bytes, prompt: str, *, timeout_s: float = 180.0) -> bytes | None:
    """Generic OpenAI Images edit call with a raw prompt.

    v37 uses this for background removal/replacement instead of local rembg/onnxruntime
    to avoid Render memory-limit restarts.
    """
    if not OPENAI_IMAGE_KEY:
        return None
    if OPENAI_IMAGE_KEY.startswith("sk-or-"):
        raise RuntimeError("OPENAI_IMAGE_KEY looks like an OpenRouter key. OpenAI Images/Edit requires an official OpenAI image key or a compatible image-edit proxy.")
    ...
    async with httpx.AsyncClient(timeout=timeout_s, follow_redirects=True) as client:
        for data in attempts:
            try:
                ...
                if r.status_code >= 400:
                    try:
                        last_err = f"{r.status_code}: {_api_error_preview(r)}"
                    except Exception:
                        last_err = f"{r.status_code}: {r.text[:500]}"
                    log.warning("v37 background image edit failed: %s", last_err)
                    continue
                ...
                url = item.get("url")
                if not url and "_extract_first_url" in globals():
                    with contextlib.suppress(Exception):
                        url = _extract_first_url(js)
                if url:
                    rr = await client.get(url, timeout=timeout_s)
                    rr.raise_for_status()
                    return rr.content
                ...
                last_err = str(e)
                log.warning("v37 background image edit exception: %s", e)
                continue
    raise RuntimeError(last_err or "background image edit failed")


def _v37_openai_bg_key_error() -> str:
    if not OPENAI_IMAGE_KEY:
        return (
            "❌ Редактирование фона недоступно: не задан OPENAI_IMAGE_KEY или OPENAI_API_KEY.\n\n"
            "Для v37 локальный rembg отключён, чтобы Render не падал по памяти. "
            "Удаление/замена фона выполняется через Images/Edit."
        )
    if OPENAI_IMAGE_KEY.startswith("sk-or-"):
        return (
            "❌ Редактирование фона недоступно: OPENAI_IMAGE_KEY похож на OpenRouter-ключ.\n\n"
            "Для удаления/замены фона нужен официальный OpenAI image key или совместимый image-edit proxy."
        )
    return ""


def _v37_prompt_remove_bg_gray() -> str:
    return (
        "Edit the provided image. Keep the main foreground person or object exactly the same as in the original: "
        "same face, body, clothes, pose, proportions, sharpness, lighting and details. "
        "Remove the original background completely and replace everything behind and around the subject with a clean "
        "neutral light gray studio background (#D9D9D9). Do not blur the subject. Do not change the subject. "
        "Do not add text, logos, extra people, artifacts, frames or decorative elements. Keep a realistic clean cutout."
    )


def _v37_prompt_replace_bg(user_prompt: str) -> str:
    p = (user_prompt or "clean realistic studio background").strip()
    return (
        "Edit the provided image. Keep the main foreground person or object exactly the same as in the original: "
        "same face, body, clothes, pose, proportions, sharpness, lighting and details. "
        "Replace only the background with this new background: "
        f"{p}. "
        "The new background must look realistic and naturally matched to the subject. Do not blur the subject. "
        "Do not change the subject identity, clothing, body, face, hands or pose. Do not add text, logos or extra people."
    )


async def _pedit_removebg(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes):
    """v37: remove background by OpenAI image edit, no local rembg."""
    err = _v37_openai_bg_key_error()
    if err:
        await update.effective_message.reply_text(err)
        return

    await update.effective_message.reply_text(
        "🧼 Принял: удаляю фон через AI-редактор без локального rembg. "
        "Сделаю нейтральную серую подложку и постараюсь сохранить основной объект резким."
    )
    with contextlib.suppress(Exception):
        await context.bot.send_chat_action(update.effective_chat.id, ChatAction.UPLOAD_PHOTO)

    try:
        out = await _openai_image_edit_raw_bytes(img_bytes, _v37_prompt_remove_bg_gray(), timeout_s=180.0)
        if not out:
            await update.effective_message.reply_text("❌ Не удалось получить результат удаления фона от Images/Edit.")
            return
        bio = BytesIO(out)
        bio.name = "background_removed_gray.png"
        await update.effective_message.reply_photo(
            InputFile(bio),
            caption="✅ Готово: фон заменён на нейтральную серую подложку. Локальный rembg/onnxruntime не использовался."
        )
    except Exception as e:
        log.exception("v37 openai remove background error: %s", e)
        await update.effective_message.reply_text(
            f"❌ Не удалось удалить фон через Images/Edit. Техническая причина: {type(e).__name__}. "
            "Проверьте OPENAI_IMAGE_KEY и логи ответа API."
        )


async def _pedit_replacebg(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes):
    """v37: open background replacement menu; no rembg check."""
    err = _v37_openai_bg_key_error()
    if err:
        await update.effective_message.reply_text(err)
        return
    await update.effective_message.reply_text(
        "🖼 Выберите новый фон или напишите свой промпт.\n\n"
        "Я отправлю фото в AI-редактор: он должен сохранить основной объект и заменить только фон. "
        "Локальный rembg/onnxruntime больше не используется, чтобы не выбивать память Render.",
        reply_markup=background_replace_kb(),
    )


async def _pedit_replacebg_apply(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, prompt: str, label: str = "новый фон"):
    """v37: replace background via OpenAI image edit instead of local segmentation."""
    err = _v37_openai_bg_key_error()
    if err:
        await update.effective_message.reply_text(err)
        return
    ...
    try:
        with contextlib.suppress(Exception):
            await context.bot.send_chat_action(update.effective_chat.id, ChatAction.UPLOAD_PHOTO)
        out = await _openai_image_edit_raw_bytes(img_bytes, _v37_prompt_replace_bg(prompt), timeout_s=180.0)
        if not out:
            await update.effective_message.reply_text("❌ Не удалось получить результат замены фона от Images/Edit.")
            return
        bio = BytesIO(out)
        bio.name = "background_replaced.png"
        await update.effective_message.reply_photo(
            InputFile(bio),
            caption=f"✅ Фон заменён: {label}. Обработка выполнена через AI-редактор без локального rembg/onnxruntime."
        )
    except Exception as e:
        log.exception("v37 openai replace background error: %s", e)
        await update.effective_message.reply_text(
            f"❌ Не удалось заменить фон через Images/Edit. Техническая причина: {type(e).__name__}. "
            "Проверьте OPENAI_IMAGE_KEY и логи ответа API."
        )
