...
PATCH_VERSION = "v34.9-live-intent-memory-fix-2026-07-08"
...
BOT_SHORT_DESCRIPTION = os.environ.get(
    ...
    "GPT-5, Images, Sora 2, Kling, Runway, Midjourney, Deepgram, Photoroom и FaceSwap — в одном Telegram-боте."
).strip()[:120]
BOT_DESCRIPTION = os.environ.get(
    ...
    "генерация изображений, оживление фото, Reels/Shorts, Sora 2/Kling-видео, Midjourney, удаление/замена фона и замена лица. "
...
SYSTEM_PROMPT = (
    ...
    "режимы Учёба, Работа, Развлечения, Медицина, Движки, OpenAI Images, Midjourney, Runway, Sora 2, Kling, "
    "Deepgram/OpenAI STT/TTS, Tavily/live-поиск, Photoroom для удаления/замены фона, PiAPI и Segmind для FaceSwap, "
    "удаление водяных знаков/надписей, ретушь, outpaint/расширение кадра, раскадровка, Reels/Shorts и мини-фильмы. "
...
HELP_TEXT = globals().get("HELP_TEXT") or (
    ...
    "Основные режимы: 🎓 Учёба, 💼 Работа, 🔥 Развлечения, 🩺 Медицина, 🧠 Движки, 💳 Баланс.\n"
    "Фото: оживление через Runway/Kling/Sora 2 без людей, замена лица, удаление и замена фона, удаление водяного знака/надписи, расширение кадра, раскадровка и анализ фото.\n"
    "Документы и голос: PDF/EPUB/DOCX/TXT/FB2, сводки, конспекты, таблицы, речь ↔ текст, озвучка.\n"
...
START_TEXT = (
    ...
    "• 💼 *Работа* — письма, КП, документы, аналитика, сводки, ToDo, брифы, таблицы, скриншоты, диктовка и озвучка.\n"
    "• 🔥 *Развлечения* — оживление фото, Reels/Shorts, мини-фильмы, сценарии, замена лица, удаление/замена фона, ретушь и удаление водяных знаков.\n"
    ...
    "\n"
    "🧩 *Под капотом:* GPT/OpenRouter, OpenAI Images/TTS, Deepgram, Tavily, Runway, Sora 2, Kling, Midjourney, Photoroom, PiAPI, Segmind и CometAPI.\n"
    "\n"
...
def _modes_root_text() -> str:
    return (
        ...
        "• 💼 Работа — письма, документы, аналитика, планы, брифы, скрины и таблицы.\n"
        "• 🔥 Развлечения — фото, видео, Reels/Shorts, мини-фильмы, фон, лицо, ретушь.\n"
        "• 🩺 Медицина — справочный разбор анализов, выписок, заключений и снимков.\n"
        "• 🧠 Движки — ручной выбор GPT/OpenRouter, OpenAI Images/TTS, Deepgram, Tavily, Runway, Sora 2, Kling, Midjourney, Photoroom, PiAPI, Segmind и CometAPI.\n"
...
def _mode_desc(key: str) -> str:
    ...
    if key == "work":
        return (
            ...
            "STT/TTS (диктовка/озвучка). Генеративные фото/видео-движки вынесены в отдельный режим «Движки».\n\n"
            "Быстрые действия ниже. Можно написать свободный запрос (например: "
...
def capability_answer(text: str) -> str | None:
    ...
    if _is_general_capability_query(tl):
        return (
            ...
            "• 💼 Работа — письма, КП, документы, аналитика, сводки, ToDo, брифы, таблицы и скриншоты.\n"
...
_CAPS_PATTERN = re.compile(
    ...
    r"медицин|медкарт|выписк|анамнез|анализ|снимок|мрт|кт|заключени|врачебн|диагноз|узи|рентген|лицо|лица|лиц|faceswap|face\s*swap)",
    re.I | re.S,
)
