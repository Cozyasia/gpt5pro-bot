...
class CometClient:
    def __init__(self, base_url: str, api_key: str, timeout: float = 120.0):
        ...
        # Sora 2 (CometAPI v1/videos)
        self.sora2_submit_endpoint = _join(self.base_url, '/v1/videos')
        self.sora2_status_template = _join(self.base_url, '/v1/videos/{task_id}')
    ...
    async def _post_multipart(self, url: str, data: Dict[str, Any], files: Optional[Dict[str, Any]]) -> Dict[str, Any]:
        headers = {"Authorization": f"Bearer {self.api_key}"}
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            r = await client.post(url, headers=headers, data=data, files=files)
            text = r.text or ""
            if r.status_code >= 400:
                raise CometError(f"HTTP {r.status_code} from CometAPI: {text[:500]}")
            try:
                return r.json()
            except Exception:
                raise CometError(f"CometAPI returned non-JSON: {text[:500]}")
    ...
    def _extract_video_url(self, payload: Dict[str, Any]) -> Optional[str]:
        candidates = [payload.get("video_url"), payload.get("url")]
        ...
        if data:
            candidates += [
                ...
                (data.get("assets") or {}).get("video") if isinstance(data.get("assets"), dict) else None,
            ]
        for c in candidates:
    ...
    async def _poll_sora2(self, task_id: str, timeout_s: int = 600, poll_s: int = 6) -> Dict[str, Any]:
        deadline = time.time() + timeout_s
        url = self.sora2_status_template.format(task_id=task_id)
        while time.time() < deadline:
            j = await self._get_json(url)
            status = (self._extract_status(j) or "").lower()
            if status in ("completed", "succeeded", "success", "done"):
                return j
            if status in ("failed", "error", "canceled", "cancelled"):
                raise CometError(f"Sora 2 failed: {j}")
            await asyncio.sleep(poll_s)
        raise CometError("Sora 2: timeout waiting for result")
    ...
    async def sora2_text_to_video(self, prompt: str, model: str = "sora-2") -> Tuple[bytes, str]:
        """Sora 2 Text → Video via CometAPI `/v1/videos` (multipart/form-data)."""
        data_fields = {"model": model, "prompt": prompt}
        submit = await self._post_multipart(self.sora2_submit_endpoint, data=data_fields, files=None)
        task_id = self._extract_task_id(submit)
        if not task_id:
            raise CometError(f"Sora 2 submit: unexpected response (no task id): {submit}")

        done = await self._poll_sora2(task_id)
        url = self._extract_video_url(done)
        if not url:
            raise CometError(f"Sora 2: completed but no video URL in response: {done}")

        content = await self._download(url)
        return content, f"sora2_{task_id}.mp4"

    async def sora2_image_to_video(self, prompt: str, image_b64: str, mime: str = "image/jpeg", model: str = "sora-2") -> Tuple[bytes, str]:
        """Sora 2 Image → Video via CometAPI `/v1/videos` (multipart/form-data)."""
        import base64
        img_bytes = base64.b64decode(image_b64) if image_b64 else b""
        if not img_bytes:
            raise CometError("Sora 2 i2v: empty image bytes")

        ext = "png" if mime.endswith("png") else ("webp" if mime.endswith("webp") else ("jpg" if mime.endswith("jpeg") else "img"))
        files = {"image": (f"image.{ext}", img_bytes, mime)}
        data_fields = {"model": model, "prompt": prompt}

        submit = await self._post_multipart(self.sora2_submit_endpoint, data=data_fields, files=files)
        task_id = self._extract_task_id(submit)
        if not task_id:
            raise CometError(f"Sora 2 i2v submit: unexpected response (no task id): {submit}")

        done = await self._poll_sora2(task_id)
        url = self._extract_video_url(done)
        if not url:
            raise CometError(f"Sora 2 i2v: completed but no video URL in response: {done}")
