...
async def runway_text_to_video(self, prompt: str, duration_s: int = 5, aspect: str = "16:9", model: str | None = None) -> Tuple[bytes, str]:
    """Runway text→video via CometAPI Runway proxy."""
    model = model or os.environ.get("RUNWAY_MODEL", "gen3a_turbo")
    payload = {"promptText": prompt, "model": model, "duration": int(duration_s), "aspectRatio": aspect}
    headers = {"X-Runway-Version": os.environ.get("RUNWAY_VERSION", "2024-11-06")}
    job = await self._request_json("POST", "/runwayml/v1/text_to_video", json=payload, extra_headers=headers)
    task_id = job.get("id") or job.get("taskId") or job.get("task_id")
    if not task_id:
        raise RuntimeError(f"Unexpected Runway response: {job}")
    video_url = await self._poll_runway_task(task_id)
    video_bytes = await self._download_bytes(video_url)
    return video_bytes, "runway.mp4"

async def runway_image_to_video(self, image_url: str, prompt: str, duration_s: int = 5, aspect: str = "16:9", model: str | None = None) -> Tuple[bytes, str]:
    """Runway image→video via CometAPI Runway proxy."""
    model = model or os.environ.get("RUNWAY_MODEL", "gen3a_turbo")
    payload = {"imageUri": image_url, "promptText": prompt, "model": model, "duration": int(duration_s), "aspectRatio": aspect}
    headers = {"X-Runway-Version": os.environ.get("RUNWAY_VERSION", "2024-11-06")}
    job = await self._request_json("POST", "/runwayml/v1/image_to_video", json=payload, extra_headers=headers)
    task_id = job.get("id") or job.get("taskId") or job.get("task_id")
    if not task_id:
        raise RuntimeError(f"Unexpected Runway response: {job}")
    video_url = await self._poll_runway_task(task_id)
...
async def _poll_runway_task(self, task_id: str) -> str:
    headers = {"X-Runway-Version": os.environ.get("RUNWAY_VERSION", "2024-11-06")}
    deadline = time.time() + int(os.environ.get("RUNWAY_TIMEOUT", "300"))
    last = None
    while time.time() < deadline:
        job = await self._request_json("GET", f"/runwayml/v1/tasks/{task_id}", extra_headers=headers)
        last = job
        status = (job.get("status") or job.get("state") or "").lower()
        if status in {"succeeded", "success", "completed", "done"}:
            out = job.get("output") or job.get("result") or {}
            url = None
            if isinstance(out, list) and out:
                out0 = out[0]
                url = out0.get("url") if isinstance(out0, dict) else out0
            elif isinstance(out, dict):
                url = out.get("url") or out.get("uri") or out.get("video")
            url = url or job.get("videoUrl") or job.get("video_url")
            if not url:
                raise RuntimeError(f"Runway completed but no video URL: {job}")
            return url
        if status in {"failed", "error", "canceled", "cancelled"}:
            raise RuntimeError(f"Runway task failed: {job}")
        await asyncio.sleep(3)
    raise RuntimeError(f"Runway timeout. Last: {last}")
...
async def _download_bytes(self, url: str) -> bytes:
    ...
    def runway_image_to_video(self, *, image_b64: str, prompt: str, duration_s: int = 5, aspect: str = "16:9", resolution: str = "720p"):
        url = f"{self.base_url}/runway/v1/videos/image2video"
        if not image_b64:
        ...
        r = self._client.post(url, headers=self._headers(), json=payload, timeout=60)
        data = _safe_json(r)
        if r.status_code >= 400:
            raise RuntimeError(f"Runway submit failed: HTTP {r.status_code} {data}")
        task_id = None
        ...
        if not task_id:
            raise RuntimeError(f"Runway submit returned unexpected payload: {data}")
        return {"task_id": task_id}

    def runway_get_task(self, task_id: str):
        url = f"{self.base_url}/runway/v1/videos/{task_id}"
        r = self._client.get(url, headers=self._headers(), timeout=30)
        data = _safe_json(r)
        if r.status_code >= 400:
            raise RuntimeError(f"Runway get_task failed: HTTP {r.status_code} {data}")
        return data

    def runway_get_content(self, task_id: str) -> bytes:
        url = f"{self.base_url}/runway/v1/videos/{task_id}/content"
        r = self._client.get(url, headers=self._headers(), timeout=60)
        if r.status_code >= 400:
            data = _safe_json(r)
            raise RuntimeError(f"Runway get_content failed: HTTP {r.status_code} {data}")
        return r.content
