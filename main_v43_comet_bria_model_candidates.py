...
PATCH_VERSION = "v43-comet-bria-model-candidates-2026-05-25"
...
# v43: Comet background removal uses its /v1/responses relay.
# Some Comet accounts do not have bria/remove-background enabled in the default distributor group,
# so the default candidate is the Replicate namespace and all attempted model errors are surfaced.
...
async def _v39_comet_bria_cutout(img_bytes: bytes, *, timeout_s: float | None = None) -> tuple[bytes, tuple[int, int]]:
    ...
    if not model_names:
        model_names = ["replicate/bria/remove-background"]
    ...
    raise RuntimeError(f"Comet/Bria remove-background failed. Tried models={model_names}. Errors: {summary[:1600]}")
...
async def _pedit_replacebg_apply(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, prompt: str, label: str = "новый фон"):
    ...
    try:
        ...
        await update.effective_message.reply_text(
            ...
            f"Детали: {detail}\n\n"
            "Проверьте COMET_API_KEY, доступ к модели и OPENAI_IMAGE_KEY для красивых фонов. "
            "Рекомендуемые ENV: COMET_BRIA_MODEL=replicate/bria/remove-background, "
            "COMET_BRIA_ALT_MODELS=, COMET_BRIA_REMOVEBG_PATHS=/v1/responses"
        )
