...
PATCH_VERSION = "revive-fun-med-v10-2026-05-22"
...
async def _run_comet_i2v(update: Update, context: ContextTypes.DEFAULT_TYPE, engine: str, img_bytes: bytes, prompt: str, duration_s: int, aspect: str):
    ...
    if engine == "sora":
        ...
        # ВАЖНО: Comet/Sora на /v1/videos вернул Unknown parameter: aspect_ratio.
        # Для image→video aspect/orientation берётся из изображения, поэтому aspect_ratio не шлём.
        # Пробуем несколько совместимых схем: image_urls, image_url, input_reference и input_image.
        payloads = []
