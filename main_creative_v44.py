...
PATCH_VERSION = "v44-bria-bg-seedance-doubao-2026-05-30"
...
# Background tools: BRIA is primary for transparent PNG and clean background replacement.
# Local rembg is fallback only, not the production provider.
BG_PROVIDER = os.environ.get("BG_PROVIDER", "bria").strip().lower() or "bria"
BRIA_API_KEY = (os.environ.get("BRIA_API_KEY") or os.environ.get("BRIA_TOKEN") or os.environ.get("BRIA_API_TOKEN") or "").strip()
...
async def _remove_bg_bytes_primary(img_bytes: bytes) -> bytes | None:
    """Production order: BRIA first, rembg only as emergency fallback."""
    if BG_PROVIDER == "bria" and BRIA_API_KEY:
        out = await _bria_remove_bg_bytes(img_bytes)
        if out:
            return out
    if BRIA_ALLOW_LOCAL_FALLBACK and rembg_remove is not None:
        try:
...
async def _pedit_removebg(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes):
    try:
        ...
        if not out:
            if BRIA_API_KEY:
                await update.effective_message.reply_text("Не удалось удалить фон через BRIA. Проверьте BRIA_API_KEY/лимиты или пришлите другое фото.")
            else:
                await update.effective_message.reply_text("BRIA_API_KEY не задан. Для старого стабильного удаления фона добавьте BRIA_API_KEY в ENV.")
            return
...
async def _pedit_replacebg_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = (
        "🌈 Выберите новый фон. Основной провайдер — BRIA Remove/Replace Background. "
...
async def _pedit_replacebg(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, preset: str = "blur"):
    if Image is None:
        ...
        return
    try:
        # For generated backgrounds use BRIA replace_background first. It preserves subject better than local blur.
        if BG_PROVIDER == "bria" and BRIA_API_KEY and preset not in {"white"}:
            ...
            if bria_out:
                ...
                return
        # Safe fallback: BRIA/removebg alpha PNG + local background, without blurring the subject.
        ...
        await update.effective_message.reply_text("Не удалось заменить фон. Проверьте BRIA_API_KEY/лимиты или попробуйте другое фото с более чётким объектом.")
