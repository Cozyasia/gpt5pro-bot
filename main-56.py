...
PATCH_VERSION = "v33.5.6-bg-primary-fix-2026-07-05"
...
async def _remove_bg_bytes_primary(img_bytes: bytes) -> bytes | None:
    ...
    1) local rembg first (subprocess-worker by default, with timeout),
    2) optional Comet/Bria fallback only when provider is not local-only.
    """
    provider = (BG_PROVIDER or "auto").lower()
    local_allowed = bool(LOCAL_REMBG_ENABLED and rembg_remove is not None)
    remote_allowed = provider not in ("local-only", "rembg-only", "local", "rembg")
