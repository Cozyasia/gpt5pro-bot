...
PATCH_VERSION = "v56-full-v35.6-plus-avatar-photoclip-ai-selfie-comet-2026-07-09"
...
CHAT_MEMORY_TTL_DAYS     = int(os.environ.get("CHAT_MEMORY_TTL_DAYS", "30") or "30")
...
if PHOTO_CLIP_MODE not in ("std", "pro"):
    PHOTO_CLIP_MODE = "pro"
SUNO_AUTO_FOR_PHOTO_CLIP = os.environ.get("SUNO_AUTO_FOR_PHOTO_CLIP", "0").strip().lower() in ("1", "true", "yes", "on")
...
AVATAR_UNIT_COST_USD = _env_float("AVATAR_UNIT_COST_USD", 0.65)
PHOTO_CLIP_UNIT_COST_USD = _env_float("PHOTO_CLIP_UNIT_COST_USD", 0.80)
...
COMET_IMAGE_EDIT_MODEL = os.environ.get("COMET_IMAGE_EDIT_MODEL", "gemini-3-pro-image").strip() or "gemini-3-pro-image"
COMET_IMAGE_EDIT_PATH = os.environ.get("COMET_IMAGE_EDIT_PATH", "/v1/images/edits").strip() or "/v1/images/edits"
COMET_IMAGE_EDIT_STATUS_PATH = os.environ.get("COMET_IMAGE_EDIT_STATUS_PATH", "").strip()
COMET_IMAGE_EDIT_TIMEOUT_S = float(os.environ.get("COMET_IMAGE_EDIT_TIMEOUT_S", "180") or 180)
AI_SELFIE_UNIT_COST_USD = _env_float("AI_SELFIE_UNIT_COST_USD", 0.20)
AI_SELFIE_DEFAULT_ASPECT = os.environ.get("AI_SELFIE_DEFAULT_ASPECT", "4:5").strip() or "4:5"
AI_SELFIE_IMAGE_SIZE = os.environ.get("AI_SELFIE_IMAGE_SIZE", "2K").strip() or "2K"
AI_SELFIE_MAX_SIDE = int(os.environ.get("AI_SELFIE_MAX_SIDE", "1536") or 1536)
...
BOT_PUBLIC_NAME = os.environ.get("BOT_PUBLIC_NAME", "Neuro-Bot GPT-5 Pro").strip()[:64]
BOT_SHORT_DESCRIPTION = os.environ.get(
    "BOT_SHORT_DESCRIPTION",
    "GPT-5, Images, Sora 2, Kling, Runway, Midjourney, Suno, Deepgram, Photoroom и FaceSwap — в одном Telegram-боте."
).strip()[:120]
BOT_DESCRIPTION = os.environ.get(
    "BOT_DESCRIPTION",
    "Neuro-Bot GPT-5 Pro — мультимодельный ИИ-бот для учёбы, работы, медицины и контента. "
    "Умеет: GPT-чат, PDF/DOCX, фото и скриншоты, анализ МРТ/КТ/анализов, озвучка и распознавание речи, "
    "генерация изображений, оживление фото, Reels/Shorts, Sora 2/Kling-видео, Midjourney, Suno-музыка, презентации, PDF-каталоги, логотипы, удаление/замена фона и замена лица. "
    "Выберите режим или просто напишите задачу."
).strip()[:512]
...
async def maybe_handle_live_query(update: Update, context: ContextTypes.DEFAULT_TYPE, user_text: str) -> bool:
    ...
    if ctx:
        prompt = (
            ...
            "Учитывай историю диалога, если вопрос выглядит как продолжение. "
            "Если пользователь просит прогноз, дай сценарии и риски, а не только текущую цену. "
...
# ───────── DB: короткая память диалога ─────────
def _chat_memory_init():
    """Создаёт таблицы для короткой истории чата и состояния диалога."""
    con = sqlite3.connect(DB_PATH)
    cur = con.cursor()
    cur.execute("""
    ...
    cur.execute("CREATE INDEX IF NOT EXISTS idx_chat_memory_user_chat_ts ON chat_memory(user_id, chat_id, created_ts)")
    cur.execute("""
...
def _chat_memory_cleanup(cur=None):
    ...
    if cur is not None:
        cur.execute("DELETE FROM chat_memory WHERE created_ts < ?", (cutoff,))
        return
    con = sqlite3.connect(DB_PATH); c = con.cursor()
    c.execute("DELETE FROM chat_memory WHERE created_ts < ?", (cutoff,))
    con.commit(); con.close()
...
def _chat_memory_add(user_id: int | None, chat_id: int | None, role: str, content: str):
    ...
    if role not in ("user", "assistant") or not content:
        return
    # Telegram/LLM не любят слишком длинную память: сохраняем краткую версию.
    content = re.sub(r"\s+", " ", content)[:2500]
    try:
        _chat_memory_init()
        con = sqlite3.connect(DB_PATH); cur = con.cursor()
        cur.execute(
            "INSERT INTO chat_memory(user_id, chat_id, role, content, created_ts) VALUES (?,?,?,?,?)",
            (int(user_id), int(chat_id), role, content, int(time.time())),
        )
        # Оставляем хвост, чтобы база не росла бесконечно.
        keep = max(4, CHAT_MEMORY_MAX_MESSAGES * 3)
        cur.execute("""
            DELETE FROM chat_memory
            WHERE user_id=? AND chat_id=? AND id NOT IN (
                SELECT id FROM chat_memory WHERE user_id=? AND chat_id=? ORDER BY id DESC LIMIT ?
            )
        """, (int(user_id), int(chat_id), int(user_id), int(chat_id), keep))
...
def _chat_memory_recent(user_id: int | None, chat_id: int | None, limit: int | None = None) -> list[dict]:
    ...
    try:
        ...
        con = sqlite3.connect(DB_PATH); cur = con.cursor()
        cur.execute(
            "SELECT role, content FROM chat_memory WHERE user_id=? AND chat_id=? ORDER BY id DESC LIMIT ?",
        ...
        log.warning("chat memory read failed: %s", e)
        return []
...
def _chat_memory_followup_query(user_id: int | None, chat_id: int | None, text: str) -> str:
    """
    Расширяет только реальные короткие follow-up ответы, а не любой короткий вопрос.
    Например: «биткоин», «на месяц», «да», «а рост?» — да.
    «что ты умеешь?» — нет.
    """
    t = (text or "").strip()
    if not CHAT_MEMORY_ENABLED or not t or len(t) > 80:
        return t
    if _is_general_capability_query(t):
        return t
    # Не расширяем полноценные вопросы, кроме коротких рыночных follow-up.
    is_short_followup = bool(
        re.fullmatch(r"[\wа-яА-ЯёЁ\-\s]{1,40}", t)
        and re.search(r"(биткоин|btc|эфир|eth|solana|sol|ton|bnb|месяц|недел|рост|паден|прогноз|курс|цена|да|нет|его|по нему)", t, re.I)
    )
    ...
    if not ctx:
        return t
    if not re.search(r"(прогноз|рост|паден|курс|цена|актив|рынок|крипто|btc|биткоин|bitcoin|уточн|о каком)", ctx, re.I):
        return t
    return (
        "С учётом предыдущего диалога продолжи незавершённую задачу.\n"
        f"Последние сообщения:\n{ctx}\n"
        f"Текущий ответ пользователя: {t}"
    )


def _dialog_state_set(user_id: int | None, chat_id: int | None, key: str, value: dict):
    if not user_id or not chat_id or not key:
        return
    try:
        _chat_memory_init()
        con = sqlite3.connect(DB_PATH); cur = con.cursor()
        cur.execute(
            "INSERT OR REPLACE INTO dialog_state(user_id, chat_id, state_key, state_json, updated_ts) VALUES (?,?,?,?,?)",
            (int(user_id), int(chat_id), key, json.dumps(value or {}, ensure_ascii=False), int(time.time())),
        )
        con.commit(); con.close()
    except Exception as e:
        log.warning("dialog state set failed: %s", e)
...
def _dialog_state_get(user_id: int | None, chat_id: int | None, key: str) -> dict:
    ...
    try:
        _chat_memory_init()
        con = sqlite3.connect(DB_PATH); cur = con.cursor()
        cur.execute("SELECT state_json FROM dialog_state WHERE user_id=? AND chat_id=? AND state_key=?", (int(user_id), int(chat_id), key))
        row = cur.fetchone(); con.close()
        if not row:
            return {}
...
def _dialog_state_clear(user_id: int | None, chat_id: int | None, key: str):
    ...
    try:
        con = sqlite3.connect(DB_PATH); cur = con.cursor()
        cur.execute("DELETE FROM dialog_state WHERE user_id=? AND chat_id=? AND state_key=?", (int(user_id), int(chat_id), key))
        con.commit(); con.close()
    except Exception:
        pass
...
IMG_PROCESS_COST_USD = float(os.environ.get("IMG_PROCESS_COST_USD", "0.05"))
...
async def _send_free_quota_exhausted(update: Update, context: ContextTypes.DEFAULT_TYPE, kind: str):
    ...
    await update.effective_message.reply_text(
        ...
        "Чтобы продолжить сегодня — подключите тариф или пополните баланс.",
...
# ───────── Prompts ─────────
SYSTEM_PROMPT = (
    "Ты — Neuro-Bot GPT-5 Pro внутри Telegram. Отвечай на русском, по сути и без лишней воды. "
...
def _tts_bytes_sync(text: str, audio_format: str = "opus") -> bytes | None:
    try:
...
# ───────── UI / тексты ─────────
START_TEXT = (
    "👋 Привет! Я *Neuro-Bot GPT-5 Pro* — мультимодельный Telegram-бот для текста, документов, фото, видео, речи и live-поиска.\n"
    ...
    "• 🔥 *Развлечения* — оживление фото, говорящий аватар, фото→видеоклип, Reels/Shorts, мини-фильмы, сценарии, замена лица, удаление/замена фона, ретушь, удаление водяных знаков и музыка Suno.\n"
    ...
    "\n"
    "🧩 *Под капотом:* GPT/OpenRouter, OpenAI Images/TTS, Deepgram, Tavily, Runway, Sora 2, Kling, Midjourney, Suno, Photoroom, PiAPI, Segmind и CometAPI.\n"
    "\n"
...
def engines_kb():
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🎥 Runway оживить", callback_data="engine:runway")],
        [InlineKeyboardButton("🎞 Sora 2 видео", callback_data="engine:sora")],
...
ENGINE_INFO_TEXT = {
    ...
    "runway": (
        "🎥 Runway: в этой сборке используется только для оживления фото / image→video.\n"
        "Для видео по тексту используем Sora 2 или Kling.\n"
        "Ключи: COMET_API_KEY и/или RUNWAY_API_KEY."
    ),
    "sora": (
...
def _modes_root_text() -> str:
    return (
        ...
        "• 🔥 Развлечения — фото, видео, Reels/Shorts, мини-фильмы, фон, лицо, ретушь и музыка.\n"
...
def _mode_desc(key: str) -> str:
    ...
    if key == "work":
        return (
            ...
            "python-pptx/reportlab для презентаций и PDF-каталогов; STT/TTS для диктовки и озвучки.\n\n"
    ...
    if key == "fun":
        return (
            ...
            "Главные быстрые действия: оживить фото, говорящий аватар, фото→видеоклип, сделать Reels/Shorts, создать мини-фильм, заменить лицо, удалить или заменить фон на фото.\n"
...
def _mode_kb(key: str) -> InlineKeyboardMarkup:
    ...
    if key == "work":
        return InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("📊 Создать презентацию", callback_data="act:work:presentation")],
            [InlineKeyboardButton("📕 Создать PDF-каталог", callback_data="act:work:catalog_pdf")],
...
def _avatar_menu_text() -> str:
    return (
        ...
        "Варианты запуска:\n"
        "• загрузить портрет, потом прислать текст для речи;\n"
        "• загрузить портрет, потом прислать voice/audio;\n"
        "• если фото уже загружено — использовать последнее фото.\n\n"
...
def _ai_selfie_menu_text() -> str:
    return (
        ...
        "Это та же логика, что у конкурентов: загружается ваше селфи, затем модель пересобирает новую реалистичную фотосцену "
...
async def _handle_avatar_upload_choice(update: Update, context: ContextTypes.DEFAULT_TYPE, q, prefix: str = "act"):
    ...
    await q.edit_message_text(
        "🗣 Пришлите портрет человека. После загрузки я попрошу текст, voice или аудиофайл для речи аватара.",
...
async def _handle_avatar_script_choice(update: Update, context: ContextTypes.DEFAULT_TYPE, q, prefix: str = "act", voice_mode: bool = False):
    ...
    if img:
        ...
        if voice_mode:
            ...
            await q.message.reply_text("📝 Использую последнее загруженное фото. Теперь пришлите текст, который должен произнести аватар.")
        await q.answer("Готово")
    else:
        context.user_data["awaiting_avatar_photo"] = True
        await q.edit_message_text(
            "Сначала пришлите портрет человека. После загрузки я попрошу текст, voice или аудио для речи.",
            reply_markup=_avatar_action_kb(prefix),
        )
...
async def on_mode_cb(update, context):
    ...
    if data == "act:work:presentation":
        ...
        context.user_data["awaiting_work_presentation_brief"] = True
        await q.edit_message_text(
            "📊 Опишите презентацию: тема, цель, аудитория, количество слайдов, стиль и ключевые тезисы. Я верну структуру и PPTX-файл.",
            reply_markup=_mode_kb("work"),
        )
        return
    ...
    if data == "act:work:catalog_pdf":
        ...
        context.user_data["awaiting_work_catalog_brief"] = True
        await q.edit_message_text(
            "📕 Опишите каталог: товары/объекты/услуги, цены, характеристики, контакты и стиль. Можно прислать текстом — соберу PDF-каталог.",
            reply_markup=_mode_kb("work"),
        )
        return
    ...
    if data == "act:fun:avatar":
        ...
        await q.edit_message_text(_avatar_menu_text(), parse_mode="Markdown", reply_markup=_avatar_action_kb("act"))
        return
    ...
    if data == "act:fun:photoclip":
        ...
        await q.edit_message_text(_photoclip_menu_text(), parse_mode="Markdown", reply_markup=_photoclip_action_kb("act"))
        return
    ...
    if data == "act:fun:aiselfie":
        ...
        await q.edit_message_text(_ai_selfie_menu_text(), parse_mode="Markdown", reply_markup=_ai_selfie_action_kb("act"))
        return
    ...
    if data == "act:fun:aiselfie_upload":
        ...
        await q.edit_message_text("🤳 Пришлите своё селфи. После загрузки я попрошу написать, с кем/где сделать AI-фото.", reply_markup=_ai_selfie_action_kb("act"))
        return
    ...
    if data in ("act:fun:aiselfie_last", "act:fun:aiselfie_custom"):
        ...
        if img:
            ...
            context.user_data["awaiting_ai_selfie_photo"] = True
            await q.edit_message_text("Сначала пришлите своё селфи. После загрузки напишете, с кем/где сделать AI-фото.", reply_markup=_ai_selfie_action_kb("act"))
        return
    ...
    if data.startswith("act:fun:as_preset_"):
        ...
        if img:
            ...
            context.user_data["awaiting_ai_selfie_photo"] = True
            context.user_data["ai_selfie_preset_prompt"] = preset
            await q.edit_message_text("🤳 Пресет выбран. Теперь пришлите своё селфи — после загрузки я попрошу имя знаменитости/персонажа.", reply_markup=_ai_selfie_action_kb("act"))
        return
...
def _fun_revive_help_text() -> str:
    return (
        ...
        "Рекомендация: Runway — стабильное оживление портретов; Kling — динамика, аватары и клипы; Sora 2 — только для фото без людей, если канал в Comet доступен."
    )
...
async def on_cb_fun(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "fun:avatar":
        context.user_data["awaiting_avatar_photo"] = True
        return await q.edit_message_text("Загрузи портрет человека, затем нажми 🗣 Говорящий аватар. После этого пришли текст, voice или аудио для речи.")
    if data == "fun:photoclip":
        context.user_data["awaiting_photo_clip_photo"] = True
        return await q.edit_message_text("Загрузи фото человека, затем нажми 🎵 Фото → видеоклип и опиши стиль: музыка, движение, настроение, 5/10 сек, 9:16 или 16:9.")
    if data == "fun:aiselfie":
        context.user_data["awaiting_ai_selfie_photo"] = True
        return await q.edit_message_text("🤳 Загрузи своё селфи, затем напиши, с кем/где сделать AI-фото: знаменитость, персонаж, премьера, реклама, travel/luxury.")
    if data == "fun:clip":
...
def capability_answer(text: str) -> str | None:
    ...
    if _is_general_capability_query(tl):
        return (
            "Да. Я — Neuro-Bot GPT-5 Pro, мультимодельный Telegram-бот для текста, документов, фото, видео, речи и live-данных.\n\n"
            "Основные режимы:\n"
...
def plan_pay_kb(plan_key: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("💳 ЮKassa", callback_data=f"pay:yookassa:{plan_key}"),
        ],
        [
            InlineKeyboardButton("💠 CryptoBot", callback_data=f"pay:cryptobot:{plan_key}"),
        ],
        [
            InlineKeyboardButton("🧾 С баланса", callback_data=f"pay:balance:{plan_key}"),
        ],
        [
...
async def _generate_business_presentation(update: Update, context: ContextTypes.DEFAULT_TYPE, brief: str):
    ...
    try:
        ...
        slide.placeholders[1].text = "Черновик создан Neuro-Bot GPT-5 Pro"
...
async def _generate_business_logo(update: Update, context: ContextTypes.DEFAULT_TYPE, brief: str):
    prompt = (
        "Professional logo design, clean vector style, high-end brand identity, no mockup, no watermark. "
        "Create a logo based on this Russian brief. Keep text readable if brand name is provided. Brief: " + (brief or "")
    )
    await update.effective_message.reply_text("🎨 Запускаю создание логотипа. Лучше всего указывать: название, нишу, стиль, цвета и где будет использоваться логотип.")
    await _start_luma_img(update, context, prompt)
            ...
            for path, payload in payloads:
                try:
                    ...
                    if task_id:
                        ...
                        await _poll_suno_task(update, client, headers, task_id)
                        return
...
async def on_cb_suno(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "suno:back_fun":
        ...
        await q.edit_message_text(_mode_desc("fun"), parse_mode="Markdown", reply_markup=_mode_kb("fun"))
        return
    ...
    if data.startswith("suno:preset:"):
        ...
        await q.edit_message_text(_suno_preset_instruction(kind), reply_markup=_suno_menu_kb())
        return
...
async def _text_to_public_mp3_url(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str) -> str:
    ...
    if not text or not OPENAI_TTS_KEY:
        return ""
    mp3 = await asyncio.to_thread(_tts_bytes_sync, text[:TTS_MAX_CHARS], "mp3")
    if not mp3:
        return ""
...
async def _run_kling_avatar(
    ...
    prompt = _avatar_prompt_from_text(script_text)
    payload = {
        "image": raw_b64,
        "prompt": prompt,
...
async def _start_talking_avatar(
    ...
    async def _go():
        ok = await _run_kling_avatar(update, context, img_bytes, script_text, audio_bytes, audio_filename, audio_file_url, audio_mime)
        if ok:
            _register_engine_spend(update.effective_user.id, "runway", AVATAR_UNIT_COST_USD)
...
def _photo_clip_prompt(user_prompt: str) -> str:
    user_prompt = (user_prompt or "").strip()
    if not user_prompt:
        user_prompt = "энергичный музыкальный клип, герой в кадре, плавное движение камеры, cinematic, social media, high quality"
    return (
        "Create a short cinematic music-video style clip from this person photo. "
        "Keep the same identity and face, avoid deformation, natural body/head motion, dynamic camera, premium lighting. "
        "User direction: " + user_prompt[:450]
    )
...
async def _run_kling_photo_clip(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, prompt: str, duration_s: int, aspect: str):
    if not (KLING_API_KEY or COMET_API_KEY):
        await update.effective_message.reply_text("❌ Kling photo→clip: COMET_API_KEY/KLING_API_KEY не задан в Render ENV.")
        return False
    await context.bot.send_chat_action(update.effective_chat.id, ChatAction.RECORD_VIDEO)
    raw_b64 = base64.b64encode(img_bytes).decode("ascii")
    ...
    payload = {
        "image": raw_b64,
        "prompt": _photo_clip_prompt(prompt),
        "model_name": KLING_MODEL,
        "model": KLING_MODEL,
        "mode": PHOTO_CLIP_MODE,
        "duration": d,
        "aspect_ratio": aspect,
        "sound": "on" if PHOTO_CLIP_SOUND else "off",
    }
    return await _create_and_poll_i2v(
        update, COMET_BASE_URL, KLING_API_KEY or COMET_API_KEY,
...
async def _run_suno_music_optional(update: Update, context: ContextTypes.DEFAULT_TYPE, prompt: str):
    """Optional separate Suno audio next to the generated photo clip; keeps existing v35.6 Suno logic intact."""
    if not SUNO_AUTO_FOR_PHOTO_CLIP or not SUNO_API_KEY:
        return
    try:
        await _run_suno_music(update, context, prompt or "music video, catchy song, cinematic social media clip")
    except Exception as e:
        log.warning("optional Suno for photo clip failed: %s", e)
        with contextlib.suppress(Exception):
            await update.effective_message.reply_text(f"⚠️ Suno не запустился: {str(e)[:500]}")


async def _start_photo_music_clip(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, user_prompt: str):
    duration, aspect = parse_video_opts(user_prompt or "")
    ...
    async def _go():
        ok = await _run_kling_photo_clip(update, context, img_bytes, user_prompt, duration, aspect)
        if ok:
            _register_engine_spend(update.effective_user.id, "runway", PHOTO_CLIP_UNIT_COST_USD)
            await _run_suno_music_optional(update, context, user_prompt or "music video, catchy song, cinematic social media clip")

    await _try_pay_then_do(
        update, context, update.effective_user.id,
        "runway", PHOTO_CLIP_UNIT_COST_USD, _go,
        remember_kind="kling_photo_music_clip",
        remember_payload={"prompt": user_prompt, "duration": duration, "aspect": aspect},
    )
...
def _ai_selfie_final_prompt(user_prompt: str, preset_prompt: str = "") -> str:
    ...
    if not AI_SELFIE_ALLOW_PUBLIC_FIGURES:
        safety_line = "Do not depict real public figures; use a generic fictional celebrity-like person instead."
    else:
        safety_line = (
            "If the user names a public figure or celebrity, create an obviously synthetic fan/selfie-style creative image, "
            "not documentary evidence of a real meeting, endorsement, political support, or news event."
        )
    return (
        "You are editing the uploaded user selfie into a new AI-generated photorealistic scene. "
        "Preserve the user's facial identity, approximate age, skin tone, hair, and natural likeness. "
        "Create a fresh realistic selfie/photo composition based on the user's instruction. "
        "Do not simply paste the face; rebuild lighting, perspective, shadows, and camera look coherently. "
        "Use iPhone/social-media selfie aesthetics when appropriate. Avoid distorted hands, duplicate faces, extra eyes, melted skin, text, captions, watermarks, logos, and UI overlays. "
        f"{safety_line} "
        f"Preset direction: {preset_prompt[:600]} "
        f"User direction: {user_prompt[:900]}"
    ).strip()
...
async def _run_comet_ai_selfie_bytes(img_bytes: bytes, user_prompt: str, preset_prompt: str = "") -> bytes | None:
    ...
    for m in (COMET_IMAGE_EDIT_MODEL, "gemini-3-pro-image", "gemini-3.1-flash-image"):
        m = (m or "").strip()
        if m and m not in model_candidates:
            model_candidates.append(m)

    path_candidates: list[str] = []
    for p in (COMET_IMAGE_EDIT_PATH, "/v1/images/edits", "/v1/images/generations"):
        p = (p or "").strip()
    ...
    last_err = ""
    async with httpx.AsyncClient(timeout=COMET_IMAGE_EDIT_TIMEOUT_S, follow_redirects=True) as client:
            ...
            for model in model_candidates:
                for data in attempts:
                    ...
                    try:
                        ...
                        if r.status_code >= 400:
                            last_err = f"{r.status_code}: {_api_error_preview(r)}" if "_api_error_preview" in globals() else f"{r.status_code}: {r.text[:500]}"
                            log.warning("Comet AI selfie failed path=%s model=%s: %s", path, model, last_err)
                            continue
                        ...
                        try:
                            ...
                            js = {}
                        last_err = f"нет изображения в ответе Comet: {json.dumps(js, ensure_ascii=False)[:500]}"
                    except Exception as e:
                        last_err = str(e)
                        log.warning("Comet AI selfie exception path=%s model=%s: %s", path, model, e)
                        continue
    raise RuntimeError(last_err or "Comet AI selfie failed")
...
async def _run_ai_selfie_image(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, user_prompt: str, preset_prompt: str = "") -> bool:
    ...
    try:
        out = None
        if AI_SELFIE_PROVIDER in ("gemini", "auto", "nano", "nanobanana", "nano_banana") and GEMINI_IMAGE_API_KEY:
            out = await _run_gemini_ai_selfie_bytes(img_bytes, user_prompt, preset_prompt)
        if not out and AI_SELFIE_PROVIDER in ("openai", "auto", "gpt", "gptimages", "gpt_images"):
            out = await _run_openai_ai_selfie_fallback(img_bytes, user_prompt, preset_prompt)
        if not out and GEMINI_IMAGE_API_KEY:
            out = await _run_gemini_ai_selfie_bytes(img_bytes, user_prompt, preset_prompt)
        if not out and OPENAI_IMAGE_KEY and not OPENAI_IMAGE_KEY.startswith("sk-or-"):
            out = await _run_openai_ai_selfie_fallback(img_bytes, user_prompt, preset_prompt)
        if not out:
            await update.effective_message.reply_text(
                "❌ AI-селфи недоступно: добавьте GEMINI_IMAGE_API_KEY/GEMINI_API_KEY в Render ENV. "
                "Резервно можно использовать официальный OPENAI_IMAGE_KEY."
            )
            return False
        bio = BytesIO(out)
        ...
        await update.effective_message.reply_text(f"❌ AI-селфи не получилось. Причина: {str(e)[:1200]}")
        return False
...
async def _start_ai_selfie(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, user_prompt: str, preset_prompt: str = ""):
    async def _go():
        ok = await _run_ai_selfie_image(update, context, img_bytes, user_prompt, preset_prompt)
        if ok:
            _register_engine_spend(update.effective_user.id, "img", AI_SELFIE_UNIT_COST_USD)
...
async def _pedit_removebg(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes):
    if not context.user_data.pop("_image_processing_quota_ok", False):
        async def _go():
            ...
            await _pedit_removebg(update, context, img_bytes)
        await _try_pay_then_do(
            update, context, update.effective_user.id,
...
async def _pedit_replacebg(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, kind: str = "blur", prompt: str = ""):
    if not context.user_data.pop("_image_processing_quota_ok", False):
        async def _go():
            context.user_data["_image_processing_quota_ok"] = True
            await _pedit_replacebg(update, context, img_bytes, kind=kind, prompt=prompt)
        await _try_pay_then_do(
            update, context, update.effective_user.id,
            "img", IMG_PROCESS_COST_USD, _go,
...
async def _pedit_outpaint(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes):
    if not context.user_data.pop("_image_processing_quota_ok", False):
        async def _go():
            context.user_data["_image_processing_quota_ok"] = True
            await _pedit_outpaint(update, context, img_bytes)
        await _try_pay_then_do(
            update, context, update.effective_user.id,
            "img", IMG_PROCESS_COST_USD, _go,
...
def _faceswap_provider_order(quality: str, user_id: int) -> list[str]:
    ...
    if FACESWAP_FORCE_SEGMIND_FOR_MULTI and selected_multi:
        ...
        if quality == "premium":
            ...
            if FACESWAP_GROUP_ALLOW_SEGMIND_FALLBACK:
                seg.append("segmind-v2")
        else:
            seg.append("segmind-v2")
            if FACESWAP_GROUP_ALLOW_SEGMIND_FALLBACK:
                seg.append("segmind-v4")
...
async def _faceswap_process(update: Update, context: ContextTypes.DEFAULT_TYPE, quality: str = "fast"):
    ...
    selected_multi = (_faceswap_target_face_count_cache.get(user_id, 0) or 0) > 1 or (_faceswap_source_face_count_cache.get(user_id, 0) or 0) > 1
    precise = bool(FACESWAP_PRECISE_COMPOSITE and selected_multi and selected_target_face)
        ...
        if typ in ("subscribe", "buy", "buy_sub", "sub"):
            ...
            await update.effective_message.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    ...
                    [InlineKeyboardButton("🧾 С баланса USD",  callback_data=f"buywallet:{tier}:{months}")],
                ])
            )
            return
    ...
    try:
        ...
        if data.startswith("engine:"):
            ...
            if engine in ENGINE_INFO_TEXT:
                await q.edit_message_text(ENGINE_INFO_TEXT[engine], disable_web_page_preview=True)
                return
        ...
        if data.startswith("pedit:"):
            ...
            if data == "pedit:avatar":
                _clear_medicine_wait(context)
                _set_avatar_wait(context)
                await q.message.reply_text("🗣 Портрет выбран. Пришлите текст, voice или аудиофайл MP3/WAV/M4A/AAC для речи аватара.")
                return
...
async def cmd_diag_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    lines = [
        ...
        f"• Kling Avatar: create={KLING_AVATAR_CREATE_PATH}  status={KLING_AVATAR_STATUS_PATH}  mode={KLING_AVATAR_MODE}  cost=${AVATAR_UNIT_COST_USD:.2f}",
        f"• Photo→clip: sound={'on' if PHOTO_CLIP_SOUND else 'off'}  mode={PHOTO_CLIP_MODE}  cost=${PHOTO_CLIP_UNIT_COST_USD:.2f}",
        f"• Suno optional: {'✅ auto' if SUNO_AUTO_FOR_PHOTO_CLIP else '— off'}  create={SUNO_CREATE_PATH}  model={SUNO_MODEL}",
        f"• AI selfie: provider={AI_SELFIE_PROVIDER} comet_key={'on' if bool(COMET_API_KEY) else 'off'} model={COMET_IMAGE_EDIT_MODEL} path={COMET_IMAGE_EDIT_PATH} cost=${AI_SELFIE_UNIT_COST_USD:.2f}",
...
async def _try_pay_then_do(
    ...
    if free_kind and get_subscription_tier(user_id) == "free" and not is_unlimited(user_id, username):
        ...
        if q_ok:
            try:
                label = _free_quota_label(free_kind)
                with contextlib.suppress(Exception):
                    await update.effective_message.reply_text(f"🎁 Бесплатное действие: {label}. Осталось сегодня: {q_left}/{q_limit}.")
                await coro_func()
            except Exception as e:
                log.exception("free action failed: %s", e)
                await update.effective_message.reply_text(f"❌ Задача не запустилась: {e}")
            return
        await _send_free_quota_exhausted(update, context, free_kind)
        return
    try:
...
async def on_text(
    ...
    if _is_ai_selfie_intent(text):
        ...
        _set_ai_selfie_wait(context)
        await update.effective_message.reply_text(
            "Да, сделаю AI-селфи как у конкурентов: загрузите своё фото, затем напишите, с кем/где сделать сцену. Например: «селфи с известным актёром на красной дорожке, iPhone selfie, 4:5».",
            reply_markup=main_kb,
        )
        return
    ...
    if _is_avatar_intent(text):
        ...
        if img and script and len(script) > 8:
            await _start_talking_avatar(update, context, img, script_text=script)
            return
        _set_avatar_wait(context)
        await update.effective_message.reply_text(
            "Да, сделаю говорящий аватар. Загрузите портрет человека, затем пришлите текст, voice или аудиофайл 2–60 секунд.",
            reply_markup=main_kb,
        )
        return
    ...
    # Работа/Бизнес: презентации, PDF-каталог, логотипы и Suno-музыка.
    if context.user_data.get("awaiting_work_presentation_brief"):
        context.user_data.pop("awaiting_work_presentation_brief", None)
        _mode_track_set(update.effective_user.id, "work_presentation")
        await _generate_business_presentation(update, context, text)
        return

    if context.user_data.get("awaiting_work_catalog_brief"):
        context.user_data.pop("awaiting_work_catalog_brief", None)
        _mode_track_set(update.effective_user.id, "work_catalog_pdf")
        await _generate_business_catalog_pdf(update, context, text)
        return
    ...
    try:
        ...
        if caption and _is_avatar_intent(caption):
            ...
            if script and len(script) > 8:
                ...
                _set_avatar_wait(context)
                await update.effective_message.reply_text("Портрет получен. Теперь пришлите текст, voice или аудиофайл для говорящего аватара.")
            return
    ...
    try:
        ...
        if mt.startswith("image/"):
            ...
            if caption and _is_avatar_intent(caption):
                ...
                _clear_medicine_wait(context)
                if script and len(script) > 8:
                    await _start_talking_avatar(update, context, raw, script_text=script)
                else:
                    _set_avatar_wait(context)
                    await update.effective_message.reply_text("Портрет получен. Теперь пришлите текст, voice или аудиофайл для говорящего аватара.")
                return
...
async def on_mode_fun_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    txt = (
        "🔥 *Развлечения*\n"
        "Здесь быстрые творческие сценарии: оживить фотографию, сделать говорящий аватар, создать фото→видеоклип, "
...
async def on_cb_fun(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "avatar":
        ...
        _clear_transient_flows(context)
        _set_mode_clean(q.from_user.id, "Развлечения", "avatar")
        await q.edit_message_text(_avatar_menu_text(), parse_mode="Markdown", reply_markup=_avatar_action_kb("fun"))
        return
    ...
    if action == "photoclip":
        ...
        _clear_transient_flows(context)
        _set_mode_clean(q.from_user.id, "Развлечения", "photoclip")
        await q.edit_message_text(_photoclip_menu_text(), parse_mode="Markdown", reply_markup=_photoclip_action_kb("fun"))
        return
