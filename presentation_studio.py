...
# ─────────────────────────────────────────────────────────────────────────────
# v101 final-review note return + resilient PDF/PPTX build and delivery
# ─────────────────────────────────────────────────────────────────────────────

_v100_on_callback = on_callback
_v100_process_text_value = _process_text_value


def _v101_compact_error(exc: Exception, limit: int = 700) -> str:
    value = f"{type(exc).__name__}: {exc}".strip()
    value = re.sub(r"\s+", " ", value)
    return value[:limit]


def _v101_valid_file(path: str | Path) -> bool:
    try:
        p = Path(path)
...
def _v99_normalize_image(path: str, pdir: Path, idx: int) -> str:
    """v101: stricter image normalization to reduce RAM and PPTX size."""
    try:
        ...
        with Image.open(path) as src:
            ...
            if max(im.size) > max_side:
                im.thumbnail((max_side, max_side), Image.Resampling.LANCZOS)
            out = pdir / f"_normalized_slide_{idx + 1:02d}.jpg"
            im.save(out, "JPEG", quality=76, optimize=True, progressive=True, subsampling=2)
            # Re-open the produced file; this catches incomplete writes immediately.
...
def _v101_render_attempts(project: dict[str, Any], slides: list[dict[str, Any]], images: dict[int, str], out_path: Path, kind: str) -> tuple[bool, list[str], str]:
    """Build one format, retrying with safe fallbacks instead of losing the project."""
    errors: list[str] = []
    renderer = _build_pptx if kind == "pptx" else _build_pdf
    attempts = [
        ("full", project, images),
        ("without_visuals", project, {}),
    ]
    no_logo_project = deepcopy(project)
    no_logo_project["logo_selected"] = ""
    no_logo_project["logo_symbol_paths"] = []
    attempts.append(("without_visuals_and_logo", no_logo_project, {}))

    for mode, attempt_project, attempt_images in attempts:
        with contextlib.suppress(Exception):
            if out_path.exists():
                out_path.unlink()
        try:
            renderer(attempt_project, slides, attempt_images, out_path)
            if _v101_valid_file(out_path):
                return True, errors, mode
            errors.append(f"{mode}: файл не создан или пуст")
        except Exception as exc:
            log.exception("%s build failed mode=%s", kind.upper(), mode)
            errors.append(f"{mode}: {_v101_compact_error(exc)}")
    return False, errors, "failed"


async def _build_files(project: dict[str, Any], update: Update, context: ContextTypes.DEFAULT_TYPE) -> tuple[str, str]:
    """v101 render pipeline with stage checkpoints and independent format recovery."""
    user_id = project["user_id"]
    project.setdefault("profile", {})["brand_name"] = _normalize_exact_brand(project.get("profile", {}).get("brand_name", ""))
    project["palette"] = _parse_palette(project)
    project["last_build_errors"] = []
    project["build_warnings"] = []
    project["last_build_step"] = "deck_content"
    _save(user_id, project)

    slides = await _generate_deck_content(project, update)
    _validate_slides_or_raise(project, slides)
    project["last_build_step"] = "image_generation"
    _save(user_id, project)

    images = await _prepare_slide_images(project, slides, update, context)
    pdir = _project_dir(user_id, project["project_id"])
    project["last_build_step"] = "image_normalization"
    _save(user_id, project)
    images = _v99_sanitize_images(images, pdir)
    project["slide_images"] = {str(k): v for k, v in images.items()}
    _save(user_id, project)
    ...
    project["last_build_step"] = "pptx_render"
    _save(user_id, project)
    pptx_ok, pptx_errors, pptx_mode = await asyncio.to_thread(
        _v101_render_attempts, project, slides, images, pptx_path, "pptx"
    )

    project["last_build_step"] = "pdf_render"
    _save(user_id, project)
    pdf_ok, pdf_errors, pdf_mode = await asyncio.to_thread(
        _v101_render_attempts, project, slides, images, pdf_path, "pdf"
    )

    errors = [*[f"PPTX {e}" for e in pptx_errors], *[f"PDF {e}" for e in pdf_errors]]
    warnings: list[str] = []
    if pptx_ok and pptx_mode != "full":
        warnings.append(f"PPTX собран в резервном режиме: {pptx_mode}")
    if pdf_ok and pdf_mode != "full":
        warnings.append(f"PDF собран в резервном режиме: {pdf_mode}")

    project["final_slides"] = slides
    project["pptx_path"] = str(pptx_path) if pptx_ok else ""
    project["pdf_path"] = str(pdf_path) if pdf_ok else ""
    project["last_build_errors"] = errors[-12:]
    project["build_warnings"] = warnings[-8:]
    project["last_build_step"] = "files_ready" if (pptx_ok or pdf_ok) else "render_failed"
    project["stage"] = "done" if (pptx_ok and pdf_ok) else "final_review"
    _save(user_id, project)

    if not pptx_ok and not pdf_ok:
        raise RuntimeError("; ".join(errors[-4:]) or "PDF и PPTX не созданы")
    return (str(pdf_path) if pdf_ok else "", str(pptx_path) if pptx_ok else "")


async def _v101_send_document(update: Update, path: str, caption: str) -> str:
    if not path or not _v101_valid_file(path):
        return "missing"
    for attempt in range(3):
        try:
            with open(path, "rb") as fh:
                await update.effective_message.reply_document(
                    InputFile(fh, filename=Path(path).name),
                    caption=caption,
                    read_timeout=120,
                    write_timeout=180,
                    connect_timeout=30,
            ...
            # Telegram can deliver the document after the acknowledgement timed out.
            log.warning("Document delivery timed out path=%s attempt=%s", path, attempt + 1)
            if attempt == 0:
                await asyncio.sleep(2.0)
                continue
            return "uncertain"
        except (NetworkError, TelegramError):
            log.exception("Document delivery failed path=%s attempt=%s", path, attempt + 1)
            await asyncio.sleep(1.5 * (attempt + 1))
        except Exception:
            log.exception("Unexpected document delivery failure path=%s", path)
            return "failed"
    return "failed"


async def _process_text_value(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str) -> bool:
    """Return final-review additions directly to final review instead of restarting style/palette."""
    if not update.effective_user or not update.effective_message:
        return False
    project = _load(update.effective_user.id)
    if not project:
        return False
    stage = project.get("stage", "")
    return_stage = project.get("note_return_stage", "")
    clean = (text or "").strip()
    if return_stage in {"final_review", "engine_choice"} and stage in {"await_visual_note", "await_style_note", "await_palette_note"}:
        if not clean:
            return True
        if stage == "await_visual_note":
            project.setdefault("visual_notes", []).append(clean)
            label = f"Дополнение к визуальному брифу №{len(project['visual_notes'])} сохранено."
        elif stage == "await_style_note":
            project.setdefault("style_notes", []).append(clean)
            label = f"Дополнение к стилю №{len(project['style_notes'])} сохранено."
        else:
            project.setdefault("palette_notes", []).append(clean)
            project["palette"] = _parse_palette(project)
            label = f"Дополнение к палитре №{len(project['palette_notes'])} сохранено."
        project.pop("note_return_stage", None)
        project["stage"] = return_stage
        _save(project["user_id"], project)
        if return_stage == "final_review":
            await _reply(update, label + " Возвращаюсь к финальной сборке.\n\n" + _review_text(project), FINAL_KB)
        else:
            await _reply(update, label + " Возвращаюсь к выбору движка изображений.", ENGINE_KB)
        return True
    return await _v100_process_text_value(update, context, text)
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Context-aware additions from the engine/final screen.
    if project and action in {"visual_note", "style_note", "palette_note"} and project.get("stage") in {"engine_choice", "final_review"}:
        with contextlib.suppress(Exception):
            await q.answer()
        return_stage = project.get("stage")
        project["note_return_stage"] = return_stage
        project["stage"] = {
            "visual_note": "await_visual_note",
            "style_note": "await_style_note",
            "palette_note": "await_palette_note",
        }[action]
        _save(project["user_id"], project)
        prompt = {
            "visual_note": "Введите очередное дополнение к визуальному брифу. После сохранения я сразу верну вас к сборке.",
            "style_note": "Введите очередное дополнение к стилю. После сохранения я сразу верну вас к сборке.",
            "palette_note": "Введите очередное дополнение к палитре. После сохранения я сразу верну вас к сборке.",
        }[action]
        await _reply(update, prompt)
        raise ApplicationHandlerStop

    if project and action == "build":
        with contextlib.suppress(Exception):
            await q.answer()
        project["stage"] = "building"
        project["last_build_step"] = "starting"
        project["last_build_errors"] = []
        _save(project["user_id"], project)
        await _reply(update, "Начинаю сборку. Создаю недостающие визуалы, затем отдельно проверяю PPTX и PDF.")
        await context.bot.send_chat_action(update.effective_chat.id, ChatAction.UPLOAD_DOCUMENT)
        ...
        try:
            if _STUDIO_PAID_RUNNER is not None:
                built = await _STUDIO_PAID_RUNNER(
                    update, context, "img", "presentation_render",
                    float(_STUDIO_RENDER_COST_USD or 0.0), _render_action
                )
            else:
                built = await _render_action()
        except Exception as exc:
            fresh = _load(project["user_id"]) or project
            fresh["stage"] = "final_review"
            fresh.setdefault("last_build_errors", []).append(_v101_compact_error(exc))
            _save(fresh["user_id"], fresh)
            details = "\n".join(f"• {e}" for e in fresh.get("last_build_errors", [])[-4:])
            await _reply(
                update,
                "Не удалось собрать файлы. Проект сохранён, повторная генерация брифа не требуется.\n"
                f"Этап: {fresh.get('last_build_step', 'не определён')}\n{details}",
                FINAL_KB,
            )
            raise ApplicationHandlerStop
        ...
        if not built:
            ...
            fresh["stage"] = "final_review"
            _save(fresh["user_id"], fresh)
            await _reply(update, "Сборка не завершена. Проект сохранён; нажмите «Собрать PDF + PPTX» ещё раз.", FINAL_KB)
            raise ApplicationHandlerStop

        pdf_path, pptx_path = built
        pdf_status = await _v101_send_document(update, pdf_path, "Готовый PDF")
        pptx_status = await _v101_send_document(update, pptx_path, "Редактируемый PPTX")
        fresh = _load(project["user_id"]) or project
        warnings = fresh.get("build_warnings", []) or []
        lines = ["Сборка завершена."]
        lines.append(f"PDF: {pdf_status}")
        lines.append(f"PPTX: {pptx_status}")
        if warnings:
            lines.append("Резервные режимы:")
            lines.extend(f"• {w}" for w in warnings)
        if pdf_status in {"failed", "missing"} or pptx_status in {"failed", "missing"}:
            fresh["stage"] = "final_review"
            _save(fresh["user_id"], fresh)
            lines.append("Один формат не отправился. Готовый файл сохранён; можно повторить сборку или скачать доступный формат.")
            kb = FINAL_KB
        else:
            kb = _kb([
                [("📄 Скачать PDF ещё раз", "ps:download_pdf"), ("📊 Скачать PPTX ещё раз", "ps:download_pptx")],
    ...
    await _v100_on_callback(update, context)


# ─────────────────────────────────────────────────────────────────────────────
# v102: project launcher + fact extraction + layout and visual prompt fixes
# ─────────────────────────────────────────────────────────────────────────────

PROJECT_EMPTY_KB = _kb([
    [("🆕 Создать новый проект", "ps:new")],
])

PROJECT_EXISTING_KB = _kb([
    [("🆕 Создать новый проект", "ps:new"), ("▶️ Продолжить проект", "ps:resume")],
    [("🗑 Удалить сохранённый проект", "ps:cancel")],
])


def _v102_strip_numbering(value: str) -> str:
    value = re.sub(r"^\s*(?:#{1,6}\s*)?", "", str(value or ""))
    value = re.sub(r"^\s*\d{1,3}(?:\.\d+)*\s*[.)-]?\s*", "", value)
    return value.strip(" \t:—-.,;*#")


def _v102_heading_key(value: str) -> str:
    value = _v102_strip_numbering(value).lower().replace("ё", "е")
    value = re.sub(r"\s+", " ", value)
    return value.strip(" :—-.,;")


def _v102_is_major_heading(line: str) -> bool:
    raw = str(line or "").strip()
    if not raw:
        return False
    clean = _v102_strip_numbering(raw)
    letters = re.sub(r"[^A-Za-zА-Яа-яЁё]", "", clean)
    # Numbered process steps such as "1. Забор воздуха" are content, not section headings.
    # A numbered line is treated as a major heading only when the heading text itself
    # is uppercase (for example "10. ЦЕЛЕВАЯ АУДИТОРИЯ B2C").
    if re.match(r"^\s*\d{1,3}\s*[.)]\s*\S", raw):
        return bool(letters and clean == clean.upper() and 1 <= len(clean.split()) <= 10)
    return bool(letters and clean == clean.upper() and 2 <= len(clean.split()) <= 8)


_V102_SUBLABELS = {
    "система объединяет", "основные этапы", "подходит для", "комплектация",
    "включает", "основные цвета", "правила", "обязательно", "запрещено",
    "тестовая цена", "тестовая стоимость", "обязательно разместить",
}


def _v102_section_items(raw: str, aliases: list[str], limit: int = 8) -> list[str]:
    lines = (raw or "").replace("\r\n", "\n").replace("\r", "\n").split("\n")
    alias_keys = [_v102_heading_key(a) for a in aliases]
    start = -1
    for i, line in enumerate(lines):
        key = _v102_heading_key(line)
        if any(key == a or key.startswith(a + " ") for a in alias_keys if a):
            start = i + 1
            break
    if start < 0:
        return []
    ...
    for line in lines[start:]:
        ...
        if not raw_line:
            continue
        if _v102_is_major_heading(raw_line) and items:
            break
        clean = re.sub(r"^\s*(?:[-–—•*]|\d{1,3}[.)])\s*", "", raw_line).strip()
        clean = clean.strip(" \t;—-")
        key = _v102_heading_key(clean)
        if not clean:
            continue
        if key in _V102_SUBLABELS:
            continue
        if re.match(r"^(?:слайд\s+\d+)", key):
            continue
        if _v102_is_major_heading(raw_line) and not items:
            # A secondary all-caps label before content; ignore it.
            continue
        clean = clean.rstrip(";")
...
def _v102_exact_field(raw: str, aliases: list[str], max_len: int = 220) -> str:
    lines = (raw or "").replace("\r\n", "\n").replace("\r", "\n").split("\n")
    keys = [_v102_heading_key(a) for a in aliases]
    for i, line in enumerate(lines):
        line_key = _v102_heading_key(line)
        # Inline "Label: value"
        for alias, alias_key in zip(aliases, keys):
            m = re.match(rf"^\s*(?:\d+[.)]?\s*)?{re.escape(alias)}\s*[:—-]\s*(.+)$", line.strip(), re.I)
            if m:
        ...
        if line_key in keys:
            for nxt in lines[i + 1:i + 5]:
                ...
                if not clean:
                    continue
                if _v102_is_major_heading(clean):
                    break
...
def _extract_tagline(text: str) -> str:
    return _v102_exact_field(text, ["слоган", "девиз"], 180)
...
def _extract_contacts(text: str) -> list[str]:
    ...
    for line in lines:
        ...
        for kind, names in labels.items():
            for name in names:
                m = re.match(rf"^\s*{re.escape(name)}\s*[:—-]\s*(.+)$", clean, re.I)
                if m:
                    value = re.sub(r"\s+", " ", m.group(1)).strip().rstrip(".,;")
                    if value:
                        found[kind] = value
    # Regex fallback for missing labelled values.
    if "phone" not in found:
        m = re.search(r"\+?\d[\d\s()\-]{8,}\d", raw)
        if m: found["phone"] = re.sub(r"\s+", " ", m.group(0)).strip()
    if "telegram" not in found:
        m = re.search(r"(?<![A-Za-z0-9._%+-])@[A-Za-z0-9_]{4,}", raw)
        if m: found["telegram"] = m.group(0)
    if "email" not in found:
        m = re.search(r"[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}", raw)
        if m: found["email"] = m.group(0)
    if "site" not in found:
        for m in re.finditer(r"\b(?:https?://)?(?:www\.)?[A-Za-z0-9-]+(?:\.[A-Za-z0-9-]+)+(?:/[A-Za-z0-9_./?&=%+-]*)?", raw):
            value=m.group(0)
            if "@" not in value and not value.lower().endswith(('.example.com',)):
                found["site"] = value.rstrip(".,;")
                break
    return [found[k] for k in ["phone", "telegram", "email", "site"] if found.get(k)]


def _v102_is_product_name(line: str) -> bool:
    clean = _v102_strip_numbering(line)
    if not clean or ":" in clean or len(clean) > 80:
        return False
    key = _v102_heading_key(clean)
    banned = _V102_SUBLABELS | {
        "название бренда", "слоган", "ключевая фраза для обложки", "описание компании",
        "цель презентации", "позиционирование", "проблемы клиента", "решение", "целевая аудитория b2c",
        "целевая аудитория b2b", "продуктовая линейка", "ключевые преимущества", "этапы работы",
        "структура презентации", "призыв к действию", "тестовые контакты", "требования к тексту",
        "визуальная концепция", "цвета", "правила использования",
    }
    if key in banned:
        return False
    letters = re.sub(r"[^A-Za-zА-Яа-яЁё]", "", clean)
    return bool(letters and clean == clean.upper() and 1 <= len(clean.split()) <= 6)
...
def _extract_offer_objects(raw: str) -> list[dict[str, Any]]:
    ...
    start = -1
    for i, line in enumerate(lines):
        if _v102_heading_key(line) in {"продуктовая линейка", "линейка продуктов", "тарифы", "модели"}:
            start = i + 1
            break
    if start < 0:
        start = 0
    ...
    def flush():
        ...
        if not pm:
            current_name=""; block=[]; return
        details=[]
        for value in block:
            clean=re.sub(r"^\s*(?:[-–—•*]|\d+[.)])\s*", "", value).strip(" ;—-")
            key=_v102_heading_key(clean)
            if not clean or key in _V102_SUBLABELS or price_re.search(clean):
                continue
            if clean not in details:
                details.append(clean[:180])
        offers.append({
            "name": _normalize_exact_brand(current_name),
            "price": re.sub(r"\s+", " ", pm.group(0)).strip(),
            "details": _sanitize_bullets(details)[:4],
            "raw": joined[:1200],
        })
        current_name=""; block=[]
    ...
    for line in lines[start:]:
        ...
        if not raw_line:
            continue
        if re.match(r"^\s*1[3-9]\s*[.)]", raw_line) and current_name:
            flush(); break
        if _v102_is_major_heading(raw_line) and _v102_heading_key(raw_line) in {
            "ключевые преимущества", "этапы работы", "структура презентации", "призыв к действию",
            "тестовые контакты", "требования к тексту", "визуальная концепция"
        }:
            flush(); break
        if _v102_is_product_name(raw_line):
            flush()
            current_name=_v102_strip_numbering(raw_line)
            continue
        if current_name:
...
def _product_offer_objects(raw: str) -> list[dict[str, Any]]:
    return [o for o in _extract_offer_objects(raw) if not re.search(r"care|service|сервис|обслуж", o["name"], re.I)]


def _service_offer_objects(raw: str) -> list[dict[str, Any]]:
    return [o for o in _extract_offer_objects(raw) if re.search(r"care|service|сервис|обслуж", o["name"], re.I)]


_v102_parse_profile_base = _parse_profile


async def _parse_profile(raw_brief: str, update: Update | None = None) -> dict[str, Any]:
    profile = await _v102_parse_profile_base(raw_brief, update)
    exact_brand = _extract_brand_name_regex(raw_brief)
    exact_tagline = _extract_tagline(raw_brief)
    exact_contacts = _extract_contacts(raw_brief)
    exact_prices = _extract_prices(raw_brief)
    if exact_brand:
        profile["brand_name"] = _normalize_exact_brand(exact_brand)
    if exact_tagline:
        profile["tagline"] = exact_tagline
    if exact_contacts:
        profile["contacts"] = exact_contacts
    if exact_prices:
        profile["prices"] = exact_prices
    return profile


def _v102_compact_positioning(raw: str, brand: str) -> list[str]:
    text = _v102_exact_field(raw, ["позиционирование"], 700)
    if not text:
        items = _v102_section_items(raw, ["позиционирование"], 4)
        text = " ".join(items)
    chunks = [c.strip(" .") for c in re.split(r"[.!?]\s+|\n+", text) if c.strip()]
    out=[]
    for chunk in chunks:
        chunk=re.sub(r"\s+", " ", chunk)
        if brand and chunk.upper().startswith(brand.upper()):
            chunk=re.sub(rf"^{re.escape(brand)}\s*[—-]\s*", "", chunk, flags=re.I)
        if chunk and chunk not in out:
            out.append(chunk[:200])
    return out[:3]


def _v102_group_stages(raw: str) -> list[str]:
    items=_v102_section_items(raw,["этапы работы","этапы реализации проекта","этапы реализации"],20)
    joined=" ".join(items).lower()
    phases=[]
    if any(w in joined for w in ["консультац", "информац", "диагност", "анализ"]):
        phases.append("Консультация, диагностика и сбор исходных данных")
    if any(w in joined for w in ["подбор", "предлож", "комплектац", "проектирован"]):
        phases.append("Подбор решения и согласование комплектации")
    if any(w in joined for w in ["достав", "производ"]):
        phases.append("Подготовка и доставка оборудования")
    if any(w in joined for w in ["монтаж", "установ", "настрой"]):
        phases.append("Монтаж, настройка и тестирование")
    if any(w in joined for w in ["инструктаж", "сервис", "обслуж"]):
        phases.append("Инструктаж и сервисное сопровождение")
    return phases[:5] or items[:5]
...
def _build_deterministic_slide(project: dict[str, Any], base: dict[str, Any], slide: dict[str, Any], idx: int) -> dict[str, Any]:
    ...
    raw=project.get("raw_brief","")
    brand=_normalize_exact_brand(profile.get("brand_name") or base.get("title") or slide.get("title") or "Бренд")
    title=_normalize_exact_brand(slide.get("title") or base.get("title") or "")
    key=title.lower()
    contacts=profile.get("contacts") or _extract_contacts(raw)
    offers=project.get("_offer_objects") or _product_offer_objects(raw)
    service_offers=project.get("_service_offer_objects") or _service_offer_objects(raw)
    ...
    slide["title"]=title
    slide["subtitle"]="" if _has_placeholder(subtitle) else subtitle
    slide["image_prompt"]=str(slide.get("image_prompt") or base.get("image_prompt") or "").strip()

    if idx==0 or base.get("layout")=="cover" or slide.get("layout")=="cover":
        slide["title"]=brand
        slide["subtitle"]=_extract_tagline(raw) or profile.get("tagline") or "Коммерческая презентация"
        cover=_v102_exact_field(raw,["ключевая фраза для обложки"],200)
        slide["bullets"]=[cover or _infer_cover_line(project)]
        slide["layout"]="cover"; slide["image_needed"]=True
        return slide

    if any(k in key for k in ["эмоцион", "новый уровень", "идея бренда", "позиционир"]):
        positioning=_v102_compact_positioning(raw,brand)
        slide["layout"]="split"; slide["image_needed"]=True
        slide["bullets"]=_sanitize_bullets([_extract_tagline(raw), *positioning])[:4] or bullets[:4]
    elif "проблем" in key:
        slide["layout"]="cards"; slide["image_needed"]=False
        slide["bullets"]=_v102_section_items(raw,["проблемы клиента","проблемы воздуха в современных помещениях","проблемы"],6) or bullets[:6]
    elif "решение" in key:
        slide["layout"]="split"; slide["image_needed"]=True
        slide["bullets"]=_v102_section_items(raw,[f"решение {brand}","решение","система объединяет"],6) or bullets[:6]
    elif any(k in key for k in ["технолог", "как работает", "схема"]):
        slide["layout"]="split"; slide["image_needed"]=True
        slide["bullets"]=_v102_section_items(raw,["как работает технология","как работает система","основные этапы","технология"],6) or bullets[:6]
    elif "преим" in key:
        slide["layout"]="cards"; slide["image_needed"]=False
        slide["bullets"]=_v102_section_items(raw,["ключевые преимущества","преимущества"],6) or bullets[:6]
    elif any(k in key for k in ["варианты использования","сценарии использования","кому подходит"]):
        slide["layout"]="split"; slide["image_needed"]=True
        b2c=_v102_section_items(raw,["целевая аудитория b2c"],3)
        b2b=_v102_section_items(raw,["целевая аудитория b2b"],3)
        slide["bullets"]=_sanitize_bullets(b2c+b2b)[:6] or bullets[:6]
    elif any(k in key for k in ["продуктовая линейка","сравнение","модел","тариф"]):
        slide["layout"]="comparison"; slide["image_needed"]=False
        slide["bullets"]=[f"{o['name']} — {o['price']}" for o in offers[:4]] or bullets[:4]
    elif any(k in key for k in ["care","сервис","обслуж","сопровожд"]):
        slide["layout"]="split"; slide["image_needed"]=True
        details=[]
        for offer in service_offers[:2]:
            details.append(f"{offer['name']} — {offer['price']}")
            details.extend(offer.get("details") or [])
        if not details:
            details=_v102_section_items(raw,[title,"сервисное обслуживание системы","сервисное обслуживание"],6)
        slide["bullets"]=_sanitize_bullets(details)[:6] or bullets[:6]
    elif any(k in key for k in ["этап", "реализац", "процесс"]):
        slide["layout"]="process"; slide["image_needed"]=False
        slide["bullets"]=_v102_group_stages(raw) or bullets[:5]
    elif any(k in key for k in ["почему", "выбирают", "отличия"]):
        slide["layout"]="split"; slide["image_needed"]=True
        benefits=_v102_section_items(raw,["ключевые преимущества","преимущества"],4)
        positioning=_v102_compact_positioning(raw,brand)
        slide["bullets"]=_sanitize_bullets(benefits+positioning)[:5] or bullets[:5]
    elif any(k in key for k in ["контакт", "призыв", "следующий шаг"]) or slide.get("layout")=="cta":
        slide["layout"]="cta"; slide["image_needed"]=True
        cta=_v102_exact_field(raw,["призыв к действию"],260)
        test_note="Контакты указаны для тестирования" if re.search(r"контакты указаны для тестирования", raw, re.I) else ""
        slide["subtitle"]="Свяжитесь с нами для консультации и расчёта проекта"
        slide["bullets"]=_sanitize_bullets([cta,*contacts,test_note])[:6]
    else:
        slide["layout"]=slide.get("layout") or "split"
        slide["image_needed"]=bool(slide.get("image_needed"))
        slide["bullets"]=bullets[:6]

    if not slide.get("bullets"):
        slide["bullets"]=["Информация подготовлена по главному брифу"]
    return slide


def _v102_luminance(hex_color: str) -> float:
    rgb=_hex(hex_color)
    vals=[]
    for c in rgb:
        v=c/255.0
...
def _v102_contrast(a: str, b: str) -> float:
    l1,l2=sorted([_v102_luminance(a),_v102_luminance(b)],reverse=True)
    return (l1+0.05)/(l2+0.05)


def _parse_palette(project: dict[str, Any]) -> dict[str, str]:
    palette=deepcopy(DEFAULT_PALETTE)
    text="\n".join(project.get("palette_notes",[]) or [])
    ...
    for line in lines:
        ...
        label=(re.sub(r"#[0-9A-Fa-f]{6}","",line).strip(" :—-") or prev).lower()
        if not hexes:
            prev=line.strip(" :—-")
            continue
        value=hexes[0].upper()
        if "текст" in label:
            palette["text"]=value
        elif "фон карточ" in label or "серо-беж" in label or "каменн" in label:
            palette["card"]=value
        elif "фон" in label:
            palette["background"]=value
        elif "акцент" in label or "серебр" in label or "латун" in label or "золот" in label:
            palette["accent"]=value
        elif "дополнитель" in label or "бирюз" in label:
            palette["secondary"]=value
        elif "голуб" in label or "беж" in label or "sand" in label:
            palette["sand"]=value
        elif "глубок" in label or "основн" in label or "primary" in label:
            palette["primary"]=value
        elif "бел" in label:
            palette["white"]=value
        prev=""
    # Readability safeguards: accent is used as subtitle text in current templates.
    if _v102_contrast(palette["text"],palette["background"])<4.5:
        palette["text"]=DEFAULT_PALETTE["text"]
    if _v102_contrast(palette["accent"],palette["background"])<3.0:
        palette["accent"]=palette["secondary"] if _v102_contrast(palette["secondary"],palette["background"])>=3.0 else palette["primary"]
    if _v102_contrast(palette["text"],palette["card"])<3.5:
        palette["card"]=DEFAULT_PALETTE["card"]
    return palette


def _v102_sanitize_visual_text(value: str, brand: str = "") -> str:
    text=str(value or "")
    if brand:
        text=re.sub(re.escape(brand),"the product",text,flags=re.I)
    text=re.sub(r"[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}","",text)
    text=re.sub(r"(?<![A-Za-z0-9._%+-])@[A-Za-z0-9_]{4,}","",text)
    text=re.sub(r"\+?\d[\d\s()\-]{8,}\d","",text)
    text=re.sub(r"\b(?:https?://)?(?:www\.)?[A-Za-z0-9-]+(?:\.[A-Za-z0-9-]+)+(?:/[A-Za-z0-9_./?&=%+-]*)?","",text)
    text=re.sub(r"(?:от\s+)?\d[\d\s.,]{1,12}\s*(?:₽|руб(?:лей|ля|ль)?|THB|USD|EUR|\$|€)","",text,flags=re.I)
    text=re.sub(r"(?im)^\s*слайд\s+\d+\s*[.:—-]*\s*","",text)
    lines=[]
    for line in text.splitlines():
        if re.search(r"(?i)обязательно разместить|точное название|контакты|телефон|telegram|email|сайт|логотип",line):
            continue
        clean=re.sub(r"\s+"," ",line).strip()
...
async def _prepare_slide_images(project: dict[str, Any], slides: list[dict[str, Any]], update: Update, context: ContextTypes.DEFAULT_TYPE) -> dict[int, str]:
    mode=project.get("image_mode","auto")
    uploaded=[p for p in project.get("uploaded_images",[]) if os.path.exists(p)]
    result={int(k):v for k,v in (project.get("slide_images") or {}).items() if os.path.exists(v)}
    target_indexes=[i for i,s in enumerate(slides) if s.get("image_needed")][:MAX_IMAGES]
    upload_iter=iter(uploaded)
    if mode in {"upload","mixed"}:
        for idx in target_indexes:
            if idx in result: continue
            try: result[idx]=next(upload_iter)
            except StopIteration: break
    if mode=="skip":
        project["slide_images"]={str(k):v for k,v in result.items()}; return result
    ...
    pdir=_project_dir(project["user_id"],project["project_id"])
    brand=_normalize_exact_brand(project.get("profile",{}).get("brand_name", ""))
    product=_v102_sanitize_visual_text(project.get("profile",{}).get("product", ""),brand)
    positioning=_v102_sanitize_visual_text(project.get("profile",{}).get("positioning", ""),brand)
    visual_rules=_v102_sanitize_visual_text(_all_notes(project,"visual_notes"),brand)
    style_rules=_v102_sanitize_visual_text(_all_notes(project,"style_notes"),brand)
    selected=(project.get("generation_engine") or "auto").lower()
    grouped: dict[str,list[tuple[int,str]]]={}
    for idx in missing:
        slide=slides[idx]
        scene=_v102_sanitize_visual_text(slide.get("image_prompt") or _default_scene_prompt(project,slide),brand)
        details=_v102_sanitize_visual_text("; ".join(slide.get("bullets",[])[:5]),brand)
        prompt=(
            f"Create a photorealistic premium editorial image for a commercial presentation.\n"
            f"Scene: {scene}\nSlide meaning: {slide.get('title','')}. Visual details: {details}.\n"
            f"Product context: {product}. Positioning: {positioning}.\n"
            f"Visual requirements: {visual_rules}. Style requirements: {style_rules}.\n"
            "Landscape 16:9, realistic contemporary interior or product photography, clean composition, generous negative space. "
            "ABSOLUTELY NO readable text, no words, no letters, no numbers, no brand names, no logo, no watermark, no UI, "
            "no poster, no brochure, no presentation slide, no infographic, no collage, no screenshot."
        )
        engine=selected if selected in {"openai","midjourney"} else _route_engine_for_slide(slide)
        if engine=="midjourney":
            prompt += " --no text words letters numbers typography logo watermark UI poster brochure presentation slide infographic collage screenshot"
        grouped.setdefault(engine,[]).append((idx,prompt[:7200]))
    for engine,entries in grouped.items():
        prompts=[p for _,p in entries]
        generated=[]
        if prompts and _STUDIO_IMAGE_BATCH_CALL is not None:
            await _reply(update,f"Создаю {len(prompts)} визуал(ов) через {'Midjourney' if engine=='midjourney' else 'OpenAI'}…")
            batch=await _STUDIO_IMAGE_BATCH_CALL(update,context,prompts,engine,"presentation_slide_images")
            if batch: generated=list(batch)
        if not generated:
            for prompt in prompts:
                try: generated.append(await _generate_image_bytes(prompt,IMAGE_SIZE))
                except Exception: generated.append(b"")
        for (idx,_),data in zip(entries,generated):
...
async def _v102_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE, kind: str = "presentation") -> bool:
    if not update.effective_user or not update.effective_message:
        return False
    selected_kind="catalog" if kind=="catalog" else "presentation"
    context.user_data["presentation_kind"]=selected_kind
    project=_load(update.effective_user.id)
    noun="каталога" if selected_kind=="catalog" else "презентации"
    if project and project.get("stage") not in {"cancelled"}:
        context.user_data["presentation_studio_active"]=project.get("project_id")
        brand=project.get("profile",{}).get("brand_name") or "без названия"
        stage=project.get("stage") or "не определён"
        await _reply(
            update,
            f"Режим создания {noun}.\n\nНайден сохранённый проект: «{brand}».\nТекущий этап: {stage}.\n\n"
            "Выберите: продолжить его, удалить либо создать новый проект с чистого листа.",
            PROJECT_EXISTING_KB,
        )
    else:
        context.user_data.pop("presentation_studio_active",None)
        await _reply(
            update,
            f"Режим создания {noun}. Сохранённого проекта нет.\n\nНажмите «Создать новый проект», чтобы перейти к главному брифу.",
            PROJECT_EMPTY_KB,
        )
    return True


PresentationStudio.start = _v102_start


_v102_on_callback_base = on_callback


async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    q=update.callback_query
    action=(q.data or "")[len(CALLBACK_PREFIX):] if q and (q.data or "").startswith(CALLBACK_PREFIX) else ""
    if q and action=="cancel":
        with contextlib.suppress(Exception): await q.answer()
        _delete(q.from_user.id)
        context.user_data.pop("presentation_studio_active",None)
        await _reply(update,"Сохранённый проект удалён. Теперь можно создать новый проект.",PROJECT_EMPTY_KB)
        raise ApplicationHandlerStop
    await _v102_on_callback_base(update,context)


# ─────────────────────────────────────────────────────────────────────────────
# v103: semantic brief parser, deterministic preflight, native technical slides,
#       prompt-hash image cache, and strict final-slide/product/service repair.
# ─────────────────────────────────────────────────────────────────────────────

_V103_GENERIC_FORBIDDEN = {
    'информация подготовлена по главному брифу',
    'содержание будет подготовлено строго по брифу',
    'система объединяет',
    'основные этапы',
...
def _v103_clean_line(value: str) -> str:
    value = re.sub(r'^\s*(?:[-–—•*]|\d{1,3}[.)])\s*', '', str(value or '')).strip()
    value = re.sub(r'\s+', ' ', value).strip(' \t;—-')
    return value


def _v103_is_sublabel(value: str) -> bool:
    key = _v102_heading_key(value)
    prefixes = (
        'система объединяет', 'основные этапы', 'подходит для', 'комплектация',
        'включает', 'основные цвета', 'правила', 'обязательно', 'запрещено',
        'тестовая цена', 'тестовая стоимость', 'обязательно разместить',
...
def _v103_section_bounds(raw: str, aliases: list[str]) -> tuple[list[str], int, int]:
    lines = (raw or '').replace('\r\n', '\n').replace('\r', '\n').split('\n')
    alias_keys = [_v102_heading_key(a) for a in aliases]
    start = -1
    for i, line in enumerate(lines):
    ...
    for j in range(start, len(lines)):
        if _v102_is_major_heading(lines[j]):
            end = j
            break
    return lines, start, end


def _v103_section_items(raw: str, aliases: list[str], limit: int = 8) -> list[str]:
    lines, start, end = _v103_section_bounds(raw, aliases)
    if start < 0:
        return []
    items: list[str] = []
    for raw_line in lines[start:end]:
        clean = _v103_clean_line(raw_line)
        if not clean or _v103_is_sublabel(clean):
            continue
        key = _v102_heading_key(clean)
        if key in _V103_GENERIC_FORBIDDEN or any(key.startswith(x + ' ') for x in _V103_GENERIC_FORBIDDEN):
            continue
        if re.match(r'^слайд\s+\d+', key):
            continue
...
def _v103_price(value: str) -> str:
    m = re.search(r'(?:от\s+)?\d[\d\s.,]{1,12}\s*(?:₽|руб(?:лей|ля|ль)?|THB|USD|EUR|\$|€)(?:\s*(?:в\s*месяц|/мес\.?))?', value or '', re.I)
    if not m:
        return ''
    price = re.sub(r'\s+', ' ', m.group(0)).strip(' .,:;')
    if not price.lower().startswith('от '):
        price = 'от ' + price
    return price


def _v103_product_header(line: str) -> bool:
    clean = _v102_strip_numbering(line)
    if not clean or len(clean) > 90 or ':' in clean:
        return False
    key = _v102_heading_key(clean)
    if _v103_is_sublabel(clean):
        return False
    banned = {
        'продуктовая линейка', 'ключевые преимущества', 'этапы работы', 'структура презентации',
        'призыв к действию', 'тестовые контакты', 'требования к тексту', 'визуальная концепция',
        'целевая аудитория b2c', 'целевая аудитория b2b', 'позиционирование', 'описание компании',
        'цель презентации', 'проблемы клиента', 'решение', 'как работает технология',
    }
    if key in banned:
        return False
    letters = re.sub(r'[^A-Za-zА-Яа-яЁё]', '', clean)
...
def _v103_structure_product_lines(raw: str) -> dict[str, str]:
    out: dict[str, str] = {}
    for line in (raw or '').splitlines():
        clean = re.sub(r'^\s*слайд\s+\d+\s*[.:—-]\s*', '', line.strip(), flags=re.I)
        parts = re.split(r'\s*[—-]\s*', clean, maxsplit=1)
        if len(parts) != 2:
            continue
        name = _normalize_exact_brand(parts[0])
        price = _v103_price(parts[1])
        if not name or not price:
            continue
        letters = re.sub(r'[^A-Za-zА-Яа-яЁё]', '', name)
        if not letters or name != name.upper() or len(name.split()) > 6:
            continue
        out[name] = price
    return out
...
def _extract_offer_objects(raw: str) -> list[dict[str, Any]]:
    ...
    stop_keys = {
        'ключевые преимущества', 'этапы работы', 'этапы реализации', 'структура презентации',
        'призыв к действию', 'тестовые контакты', 'требования к тексту', 'визуальная концепция',
    }
    for j in range(start, len(lines)):
        if re.match(r'^\s*\d{1,3}\s*[.)]\s*', lines[j]) and _v102_heading_key(lines[j]) in stop_keys:
            end = j
            break

    structural_prices = _v103_structure_product_lines(raw)
    ...
    def flush() -> None:
        ...
        if not current:
            return
        current['price'] = current.get('price') or structural_prices.get(current['name'], '')
        if current.get('price'):
            suitable = _sanitize_bullets(current.pop('_suitable', []))[:2]
            features = _sanitize_bullets(current.pop('_features', []))
            general = _sanitize_bullets(current.pop('_general', []))[:2]
            details: list[str] = []
            is_service = bool(re.search(r'care|service|сервис|обслуж', current['name'], re.I))
            source_values = [*features[:5], *general[:1]] if is_service else [*suitable, *features[:2], *general[:1]]
            for value in source_values:
                if value and value not in details:
                    details.append(value)
            current['details'] = details[:5 if is_service else 4]
            current['raw'] = '\n'.join(current.pop('_raw', []))[:1600]
    ...
    for raw_line in lines[start:end]:
        ...
        if not clean:
            continue
        if _v103_product_header(clean):
            flush()
            current = {
                'name': _normalize_exact_brand(_v102_strip_numbering(clean)),
                'price': '', '_suitable': [], '_features': [], '_general': [], '_raw': [],
            }
            continue
        if current is None:
            continue
        current['_raw'].append(clean)
        key = _v102_heading_key(clean)
        if key.startswith('подходит для'):
            mode = 'suitable'; continue
        if key.startswith('комплектация') or key.startswith('включает'):
            mode = 'features'; continue
        price = _v103_price(clean)
        if price:
            current['price'] = price
            continue
        value = _v103_clean_line(clean)
        if not value or _v103_is_sublabel(value):
            continue
        if mode == 'suitable':
            current['_suitable'].append(value)
        elif mode == 'features':
            current['_features'].append(value)
        else:
            current['_general'].append(value)
    flush()

    existing = {p['name'].lower() for p in products}
    for name, price in structural_prices.items():
        if name.lower() not in existing:
            products.append({'name': name, 'price': price, 'details': [], 'raw': ''})
    ...
    for line in raw.replace('\r\n', '\n').replace('\r', '\n').split('\n'):
        ...
        for label, aliases in labels:
            for alias in aliases:
                m = re.match(rf'^\s*{re.escape(alias)}\s*[:—-]\s*(.+)$', clean, re.I)
                if m:
                    value = re.sub(r'\s+', ' ', m.group(1)).strip().rstrip('.,;')
                    if value:
                        found[label] = value
    if 'Телефон' not in found:
        m = re.search(r'\+?\d[\d\s()\-]{8,}\d', raw)
        if m: found['Телефон'] = re.sub(r'\s+', ' ', m.group(0)).strip()
    if 'Telegram' not in found:
        m = re.search(r'(?<![A-Za-z0-9._%+-])@[A-Za-z0-9_]{4,}', raw)
        if m: found['Telegram'] = m.group(0)
    if 'Email' not in found:
        m = re.search(r'[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}', raw)
        if m: found['Email'] = m.group(0)
    if 'Сайт' not in found:
        for m in re.finditer(r'\b(?:https?://)?(?:www\.)?[A-Za-z0-9-]+(?:\.[A-Za-z0-9-]+)+(?:/[A-Za-z0-9_./?&=%+-]*)?', raw):
            value = m.group(0).rstrip('.,;')
            if '@' not in value:
                found['Сайт'] = value
                break
    return [f'{label}: {found[label]}' for label, _ in labels if found.get(label)]


def _v103_contact_value(line: str) -> str:
    return str(line or '').split(':', 1)[-1].strip()


def _v103_usage_contexts(raw: str) -> list[str]:
    # The approved structure is the most precise source for a slide called
    # “Варианты использования: квартира, дом, офис …”.
    for line in (raw or '').splitlines():
        m = re.search(r'(?i)слайд\s+\d+\s*[.:—-]*\s*(?:варианты|сценарии)\s+использования[^:]*:\s*(.+)', line)
        if not m:
            continue
        tail = m.group(1).strip().rstrip('.')
        parts = re.split(r'\s*,\s*|\s+и\s+', tail)
        values = [_v103_clean_line(p) for p in parts if _v103_clean_line(p)]
        if values:
            return values[:6]
    # Conservative entity extraction from audience sections.
    text = ' '.join([
        *_v103_section_items(raw, ['целевая аудитория b2c'], 12),
        *_v103_section_items(raw, ['целевая аудитория b2b'], 12),
    ]).lower()
    mapping = [
        ('Квартира', r'квартир'), ('Загородный дом', r'загородн\w* дом|больш\w* дом'),
        ('Офис', r'офис'), ('Ресторан', r'ресторан|кофейн'), ('Отель', r'отел|гостиниц'),
        ('Клиника', r'клиник'), ('Салон', r'салон'), ('Шоурум', r'шоурум'),
        ('Общественное пространство', r'общественн\w* простран'),
    ]
    out = [label for label, pattern in mapping if re.search(pattern, text)]
    return out[:6]


def _v103_slide_key(base: dict[str, Any], slide: dict[str, Any]) -> str:
    return (' '.join([
        str(base.get('title') or ''), str(slide.get('title') or ''),
        str(base.get('layout') or ''), str(slide.get('layout') or ''),
    ])).lower()


def _v103_visual_scene(project: dict[str, Any], slide_number: int, fallback: str) -> str:
    notes = '\n'.join(project.get('visual_notes', []) or [])
    pattern = re.compile(
        rf'(?is)слайд\s*{slide_number}\s*[.:—-]*\s*(.*?)(?=\n\s*слайд\s*\d+\s*[.:—-]|\Z)'
    )
    m = pattern.search(notes)
    if m:
        scene = _v102_sanitize_visual_text(m.group(1), project.get('profile', {}).get('brand_name', ''))
        if scene:
            return scene[:1200]
    return _v102_sanitize_visual_text(fallback, project.get('profile', {}).get('brand_name', ''))
...
def _build_deterministic_slide(project: dict[str, Any], base: dict[str, Any], slide: dict[str, Any], idx: int) -> dict[str, Any]:
    ...
    raw = project.get('raw_brief', '')
    brand = _normalize_exact_brand(profile.get('brand_name') or base.get('title') or slide.get('title') or 'Бренд')
    key = _v103_slide_key(base, slide)
    title = _normalize_exact_brand(base.get('title') or slide.get('title') or '')
    contacts = _extract_contacts(raw)
    offers = _product_offer_objects(raw)
    service_offers = _service_offer_objects(raw)
    project['_offer_objects'] = offers
    project['_service_offer_objects'] = service_offers
    slide = {**base, **slide}
    candidate_bullets = _sanitize_bullets(slide.get('bullets') or base.get('bullets') or [])
    slide['title'] = title
    slide['subtitle'] = ''
    slide['image_prompt'] = ''

    if idx == 0 or 'cover' in key or 'облож' in key:
        slide['title'] = brand
        slide['subtitle'] = _extract_tagline(raw) or profile.get('tagline') or 'Коммерческая презентация'
        cover = _v102_exact_field(raw, ['ключевая фраза для обложки'], 220)
        slide['bullets'] = [cover or 'Премиальное технологическое решение для вашего пространства']
        slide['layout'] = 'cover'; slide['image_needed'] = True
        slide['image_prompt'] = _v103_visual_scene(project, 1, profile.get('visual_direction') or profile.get('product') or '')
        return slide

    if any(k in key for k in ['эмоцион', 'новый уровень', 'свет как часть', 'позиционир']):
        positioning = _v103_section_items(raw, ['позиционирование'], 3) or _v102_compact_positioning(raw, brand)
        slide['layout'] = 'split'; slide['image_needed'] = True
        slide['bullets'] = _sanitize_bullets([_extract_tagline(raw), *positioning])[:4]
    elif 'проблем' in key:
        slide['layout'] = 'cards'; slide['image_needed'] = False
        slide['bullets'] = _v103_section_items(raw, ['проблемы клиента', 'проблемы'], 6) or candidate_bullets[:6]
    elif 'решение' in key:
        slide['layout'] = 'split'; slide['image_needed'] = True
        slide['bullets'] = _v103_section_items(raw, [f'решение {brand}', 'решение'], 6) or candidate_bullets[:6]
    elif any(k in key for k in ['технолог', 'как работает', 'схема', 'система освещения']):
        # Native editable process diagram prevents AI-generated labels and keeps
        # PDF/PPTX semantically consistent.
        slide['layout'] = 'process'; slide['image_needed'] = False
        slide['bullets'] = _v103_section_items(raw, ['как работает технология', 'как работает система', 'основные этапы системы', 'основные этапы'], 6) or candidate_bullets[:6]
    elif 'преим' in key:
        slide['layout'] = 'cards'; slide['image_needed'] = False
        slide['bullets'] = _v103_section_items(raw, ['ключевые преимущества', 'преимущества'], 6) or candidate_bullets[:6]
    elif any(k in key for k in ['варианты использования', 'сценарии использования', 'кому подходит']):
        slide['layout'] = 'split'; slide['image_needed'] = True
        slide['bullets'] = _v103_usage_contexts(raw) or candidate_bullets[:6]
    elif any(k in key for k in ['продуктовая линейка', 'сравнение', 'модел', 'тариф']):
        slide['layout'] = 'comparison'; slide['image_needed'] = False
        slide['bullets'] = [f"{o['name']} — {o['price']}" for o in offers[:4]]
    elif any(k in key for k in ['care', 'сервис', 'обслуж', 'сопровожд']):
        slide['layout'] = 'split'; slide['image_needed'] = True
        if service_offers:
            service = service_offers[0]
            slide['subtitle'] = f"{service['name']} — {service['price']}"
            slide['bullets'] = _sanitize_bullets(service.get('details') or [])[:5]
        else:
            slide['bullets'] = _v103_section_items(raw, ['сервисное обслуживание системы', 'сервисное обслуживание'], 6) or candidate_bullets[:6]
    elif any(k in key for k in ['этап', 'реализац', 'процесс']):
        slide['layout'] = 'process'; slide['image_needed'] = False
        slide['bullets'] = _v102_group_stages(raw) or candidate_bullets[:5]
    elif any(k in key for k in ['почему', 'выбирают', 'отличия']):
        slide['layout'] = 'split'; slide['image_needed'] = True
        benefits = _v103_section_items(raw, ['ключевые преимущества', 'преимущества'], 6)
        preferred = []
        priorities = ['индивиду', 'расчёт', 'проект', 'умн', 'монтаж', 'сервис', 'интеграц', 'качество']
        for token in priorities:
            for item in benefits:
                if token in item.lower() and item not in preferred:
                    preferred.append(item)
                    break
        slide['bullets'] = _sanitize_bullets(preferred + benefits)[:5] or candidate_bullets[:5]
    elif idx == len(project.get('structure', [])) - 1 or any(k in key for k in ['контакт', 'призыв', 'следующий шаг', 'cta']):
        slide['layout'] = 'cta'; slide['image_needed'] = True
        cta = _v102_exact_field(raw, ['призыв к действию'], 300)
        slide['subtitle'] = cta or 'Свяжитесь с нами для консультации и расчёта проекта'
        test_note = 'Контакты указаны для тестирования' if re.search(r'контакты указаны для тестирования', raw, re.I) else ''
        slide['bullets'] = _sanitize_bullets([*contacts, test_note])[:5]
    else:
        slide['layout'] = slide.get('layout') or 'split'
        slide['image_needed'] = bool(slide.get('image_needed'))
        slide['bullets'] = candidate_bullets[:6]

    if slide.get('image_needed'):
        slide['image_prompt'] = _v103_visual_scene(project, idx + 1, profile.get('visual_direction') or profile.get('product') or '')
    slide['bullets'] = [b for b in _sanitize_bullets(slide.get('bullets') or []) if _v102_heading_key(b) not in _V103_GENERIC_FORBIDDEN]
    return slide


def _v103_repair_slides(project: dict[str, Any], slides: list[dict[str, Any]]) -> list[dict[str, Any]]:
    structure = project.get('structure') or []
    repaired: list[dict[str, Any]] = []
    for idx, base in enumerate(structure):
        candidate = slides[idx] if idx < len(slides) and isinstance(slides[idx], dict) else {}
        repaired.append(_build_deterministic_slide(project, deepcopy(base), deepcopy(candidate), idx))
    return repaired
...
def _validate_slides_or_raise(project: dict[str, Any], slides: list[dict[str, Any]]) -> None:
    structure = project.get('structure') or []
    raw = project.get('raw_brief', '')
    brand = _normalize_exact_brand(project.get('profile', {}).get('brand_name') or '')
    products = _product_offer_objects(raw)
    services = _service_offer_objects(raw)
    contacts = _extract_contacts(raw)
    errors: list[str] = []
    if not slides or len(slides) != len(structure):
        errors.append(f'ожидалось {len(structure)} слайдов, получено {len(slides)}')
    if slides and brand and _normalize_exact_brand(slides[0].get('title', '')) != brand:
        errors.append('на обложке изменено точное название бренда')
    all_text = '\n'.join(
        ' '.join([str(s.get('title') or ''), str(s.get('subtitle') or ''), *[str(b) for b in s.get('bullets', []) or []]])
        for s in slides
    )
    for i, slide in enumerate(slides, 1):
        text = ' '.join([str(slide.get('title') or ''), str(slide.get('subtitle') or ''), *[str(b) for b in slide.get('bullets', []) or []]])
        if _has_placeholder(text) or 'Информация подготовлена по главному брифу' in text:
            errors.append(f'слайд {i} содержит техническую заглушку')
        if not (slide.get('bullets') or slide.get('subtitle')):
            errors.append(f'слайд {i} не содержит полезного текста')
    comparison = next((s for s in slides if s.get('layout') == 'comparison'), None)
    if products:
        if not comparison:
            errors.append('отсутствует слайд сравнения продуктов')
        for item in products:
            hay = ' '.join(comparison.get('bullets', []) if comparison else [])
            if item['name'] not in hay or item['price'] not in hay:
                errors.append(f"на продуктовом слайде отсутствует {item['name']} / {item['price']}")
    if services:
        service_text = ' '.join(
            [str(s.get('subtitle') or ''), *[str(b) for b in s.get('bullets', []) or []]]
            for s in []
        )
        service_slide = next((s for s in slides if re.search(r'care|сервис|обслуж', str(s.get('title') or ''), re.I)), None)
        if not service_slide:
            errors.append('отсутствует сервисный слайд')
        else:
            service_text = ' '.join([str(service_slide.get('subtitle') or ''), *[str(b) for b in service_slide.get('bullets', []) or []]])
            if services[0]['name'] not in service_text or services[0]['price'] not in service_text:
                errors.append('на сервисном слайде отсутствует название или цена сервиса')
            if len(service_slide.get('bullets', []) or []) < 3:
                errors.append('сервисный слайд содержит слишком мало услуг')
    if slides:
        final_text = ' '.join([str(slides[-1].get('subtitle') or ''), *[str(b) for b in slides[-1].get('bullets', []) or []]])
        for contact in contacts:
            value = _v103_contact_value(contact)
            if value and value not in final_text:
                errors.append(f'на финальном слайде отсутствует контакт: {value}')
        if re.search(r'контакты указаны для тестирования', raw, re.I) and 'Контакты указаны для тестирования' not in final_text:
            errors.append('на финальном слайде отсутствует пометка о тестовых контактах')
    project['preflight_report'] = {
        'ok': not errors, 'errors': errors[-20:], 'products': products,
        'service': services[:1], 'contacts': contacts, 'slide_count': len(slides),
    }
    _save(project['user_id'], project)
    if errors:
        raise RuntimeError('Предэкспортная проверка: ' + '; '.join(errors[:8]))


_v103_generate_deck_base = _generate_deck_content


async def _generate_deck_content(project: dict[str, Any], update: Update | None = None) -> list[dict[str, Any]]:
    # LLM may improve phrasing, but critical facts and all commercial slides are
    # deterministically rebuilt from the original brief before export.
    slides = await _v103_generate_deck_base(project, update)
    slides = _v103_repair_slides(project, slides)
    _validate_slides_or_raise(project, slides)
    return slides


def _v103_prompt_for_slide(project: dict[str, Any], slide: dict[str, Any], idx: int) -> str:
    brand = _normalize_exact_brand(project.get('profile', {}).get('brand_name', ''))
    scene = _v103_visual_scene(project, idx + 1, slide.get('image_prompt') or project.get('profile', {}).get('visual_direction', ''))
    scene = _v102_sanitize_visual_text(scene, brand)
    style = _v102_sanitize_visual_text(_all_notes(project, 'style_notes'), brand)
    visual = _v102_sanitize_visual_text(_all_notes(project, 'visual_notes'), brand)
    # Do not pass slide title or bullet copy to the image model; doing so strongly
    # increases the chance that it renders words inside the image.
    return (
        'Create one photorealistic premium editorial photograph for a commercial presentation.\n'
        f'Visual scene only: {scene}.\n'
        f'Consistent art direction: {style[:1400]}.\n'
        f'Additional visual constraints: {visual[:1600]}.\n'
        'Landscape 16:9, realistic materials, clean composition, generous negative space. '
        'ABSOLUTELY NO readable text, no words, no letters, no numbers, no brand name, no logo, '
        'no watermark, no UI, no poster, no brochure, no presentation slide, no infographic, '
        'no collage, no screenshot, no labels, no signage.'
    )[:6500]
...
async def _prepare_slide_images(project: dict[str, Any], slides: list[dict[str, Any]], update: Update, context: ContextTypes.DEFAULT_TYPE) -> dict[int, str]:
    mode = project.get('image_mode', 'auto')
    uploaded = [p for p in project.get('uploaded_images', []) if os.path.exists(p)]
    existing = {int(k): v for k, v in (project.get('slide_images') or {}).items() if os.path.exists(v)}
    hashes = {str(k): str(v) for k, v in (project.get('slide_image_prompt_hashes') or {}).items()}
    target = [i for i, s in enumerate(slides) if s.get('image_needed')][:MAX_IMAGES]
    result: dict[int, str] = {}
    upload_iter = iter(uploaded)
    if mode in {'upload', 'mixed'}:
        for idx in target:
    ...
    for idx in target:
        if idx in result:
            continue
        prompt = _v103_prompt_for_slide(project, slides[idx], idx)
        digest = hashlib.sha256(prompt.encode('utf-8')).hexdigest()[:20]
        old = existing.get(idx)
        # Preserve user uploads; generated files are reused only when their prompt
        # hash is unchanged.
        if old and not Path(old).name.startswith(('slide_', '_normalized_slide_')):
            result[idx] = old
            continue
        if old and hashes.get(str(idx)) == digest:
            result[idx] = old
            continue
        engine = selected if selected in {'openai', 'midjourney'} else _route_engine_for_slide(slides[idx])
        if engine == 'midjourney':
            prompt += ' --no text words letters numbers typography logo watermark UI poster brochure slide infographic collage screenshot labels signage'
        grouped.setdefault(engine, []).append((idx, prompt, digest))

    for engine, entries in grouped.items():
        prompts = [p for _, p, _ in entries]
        generated: list[bytes] = []
        if prompts and _STUDIO_IMAGE_BATCH_CALL is not None:
            await _reply(update, f"Создаю {len(prompts)} визуал(ов) через {'Midjourney' if engine == 'midjourney' else 'OpenAI'}…")
            try:
                ...
                log.exception('Batch visual generation failed engine=%s', engine)
        generated = (generated + [b''] * len(entries))[:len(entries)]
        for pos, (idx, prompt, digest) in enumerate(entries):
            data = generated[pos]
            if not data:
                try:
                    data = await _generate_image_bytes(prompt, IMAGE_SIZE)
                except Exception:
                    log.exception('Visual generation failed slide=%s', idx + 1)
                    continue
            path = pdir / f'slide_{idx + 1:02d}.png'
            path.write_bytes(data)
            result[idx] = str(path)
            hashes[str(idx)] = digest
    project['slide_images'] = {str(k): v for k, v in result.items()}
    project['slide_image_prompt_hashes'] = hashes
    return result


def _render_comparison_cards_ppt(slide, project: dict[str, Any], palette: dict[str, str]):
    items = (_product_offer_objects(project.get('raw_brief', '')) or project.get('_offer_objects') or [])[:4]
    if not items:
        return
    cols = len(items)
    gap = 0.18
    ...
    for i, item in enumerate(items):
        x = Inches(0.72 + i * (card_w + gap))
        _ppt_rect(slide, x, Inches(1.55), Inches(card_w), Inches(4.95), palette['white'], palette['sand'], True)
        _ppt_add_text(slide, item['name'], x + Inches(0.16), Inches(1.82), Inches(card_w - 0.32), Inches(0.58), 18, palette['primary'], True, PP_ALIGN.CENTER)
        _ppt_add_text(slide, item['price'], x + Inches(0.16), Inches(2.40), Inches(card_w - 0.32), Inches(0.45), 15, palette['accent'], True, PP_ALIGN.CENTER)
        details = item.get('details') or []
        if details:
            _ppt_add_bullets(slide, details[:4], x + Inches(0.16), Inches(3.0), Inches(card_w - 0.32), Inches(2.85), palette, 13)
...
def _render_comparison_cards_pdf(c: canvas.Canvas, project: dict[str, Any], palette: dict[str, str], regular: str, bold: str):
    ...
    if not items:
        return
    cols = len(items); gap = 10; left = 42; total_w = 876
    card_w = (total_w - gap * (cols - 1)) / cols
    for i, item in enumerate(items):
        x = left + i * (card_w + gap)
        c.setFillColor(HexColor(palette['white'])); c.roundRect(x, 70, card_w, 370, 10, stroke=0, fill=1)
        c.setStrokeColor(HexColor(palette['sand'])); c.roundRect(x, 70, card_w, 370, 10, stroke=1, fill=0)
        c.setFillColor(HexColor(palette['primary'])); c.setFont(bold, 13)
        ty = 410
        for line in _pdf_wrap(item['name'], max(13, int(card_w / 8))):
            c.drawCentredString(x + card_w / 2, ty, line); ty -= 17
        c.setFillColor(HexColor(palette['accent'])); c.setFont(bold, 11)
        c.drawCentredString(x + card_w / 2, ty - 2, item.get('price', '')); ty -= 24
        c.setFillColor(HexColor(palette['text'])); c.setFont(regular, 9.5)
        for detail in (item.get('details') or [])[:4]:
            wrapped = _pdf_wrap(detail, max(18, int(card_w / 7)))
            for li, line in enumerate(wrapped):
                c.drawString(x + 12 if li == 0 else x + 20, ty, ('• ' if li == 0 else '') + line)
                ty -= 13
            ty -= 4
...
# ─────────────────────────────────────────────────────────────────────────────
# v104: deterministic self-healing preflight.
# Fixes a v103 regression where the old generator called the new strict validator
# before the v103 repair pass. A harmless empty emotional slide therefore aborted
# the whole build before any visual generation started.
# ─────────────────────────────────────────────────────────────────────────────


def _v104_role(project: dict[str, Any], base: dict[str, Any], slide: dict[str, Any], idx: int) -> str:
    key = _v103_slide_key(base, slide)
    if idx == 0 or any(k in key for k in ['cover', 'облож']): return 'cover'
    if any(k in key for k in ['проблем', 'боли']): return 'problems'
    if any(k in key for k in ['решение']): return 'solution'
    if any(k in key for k in ['технолог', 'как работает', 'схема', 'принцип работы']): return 'technology'
    if any(k in key for k in ['преим']): return 'benefits'
    if any(k in key for k in ['варианты использования', 'сценарии использования', 'кому подходит', 'применение']): return 'usage'
    if any(k in key for k in ['продуктовая линейка', 'сравнение', 'модел', 'тариф']): return 'products'
    if any(k in key for k in ['care', 'сервис', 'обслуж', 'сопровожд']): return 'service'
    if any(k in key for k in ['этап', 'реализац', 'процесс работы']): return 'process'
    if any(k in key for k in ['почему', 'выбирают', 'отличия']): return 'why'
    if idx == len(project.get('structure') or []) - 1 or any(k in key for k in ['контакт', 'призыв', 'следующий шаг', 'cta']): return 'cta'
    if any(k in key for k in ['эмоцион', 'новый уровень', 'идея бренда', 'позиционир', 'комфорт начинается', 'часть архитектуры']): return 'emotional'
    # The standard commercial deck has a stable semantic order. Use position only
    # as a fallback when a creative heading does not contain a role keyword.
    if len(project.get('structure') or []) == 12:
        return {1:'emotional',2:'problems',3:'solution',4:'technology',5:'benefits',6:'usage',7:'products',8:'service',9:'process',10:'why',11:'cta'}.get(idx,'generic')
    return 'generic'


def _v104_group_stages(raw: str) -> list[str]:
    items = _v103_section_items(raw, ['этапы работы', 'этапы реализации проекта', 'этапы реализации', 'процесс работы'], 30)
    joined = ' '.join(items).lower()
    phases: list[str] = []
    tests = [
        ('Консультация, выезд и сбор исходных данных', ['консультац','фотограф','план помещ','выезд','замер','измерен','анализ','диагност']),
        ('Концепция, расчёт и согласование решения', ['концепц','расчёт','проект','подбор материал','визуализац','согласован']),
        ('Производство и подготовка к установке', ['производ','изготов','комплектац','поставка','достав']),
        ('Монтаж, настройка и контрольная проверка', ['монтаж','установ','настрой','тестирован','контрольн']),
        ('Передача проекта и сервисное сопровождение', ['передач','инструктаж','сервис','обслуж','сопровожд']),
    ]
    for label, tokens in tests:
        if any(token in joined for token in tokens): phases.append(label)
    if len(phases) >= 3:
        return phases[:5]
    # Do not collapse a valid process to one vague card.
    return items[:5]


def _v104_emotional_copy(project: dict[str, Any]) -> list[str]:
    raw = project.get('raw_brief', '')
    brand = _normalize_exact_brand(project.get('profile', {}).get('brand_name') or '')
    tagline = _extract_tagline(raw) or project.get('profile', {}).get('tagline') or ''
    positioning = _v103_section_items(raw, ['позиционирование'], 4) or _v102_compact_positioning(raw, brand)
    values = _sanitize_bullets([tagline, *positioning])
    if not values:
        product = str(project.get('profile', {}).get('product') or '').strip()
        if product: values = [product[:220]]
    return values[:4]


def _v104_generic_copy(project: dict[str, Any], title: str, base: dict[str, Any]) -> list[str]:
    raw = project.get('raw_brief', '')
    aliases = [title]
    title_key = _v102_heading_key(title)
    if title_key:
        aliases.append(title_key)
    values = _v103_section_items(raw, aliases, 6)
    if values: return values[:6]
    base_values = _sanitize_bullets(base.get('bullets') or [])
    if base_values: return base_values[:6]
    for field in ('positioning','product','goal'):
        value = str(project.get('profile', {}).get(field) or '').strip()
        if value: return [value[:240]]
    return [f'Ключевой раздел: {title}']
...
def _build_deterministic_slide(project: dict[str, Any], base: dict[str, Any], slide: dict[str, Any], idx: int) -> dict[str, Any]:
    ...
    raw = project.get('raw_brief', '')
    brand = _normalize_exact_brand(profile.get('brand_name') or base.get('title') or slide.get('title') or 'Бренд')
    role = _v104_role(project, base, slide, idx)
    title = _normalize_exact_brand(base.get('title') or slide.get('title') or '')
    contacts = _extract_contacts(raw)
    offers = _product_offer_objects(raw)
    services = _service_offer_objects(raw)
    project['_offer_objects'] = offers
    project['_service_offer_objects'] = services
    out = {**base, **slide}
    out['title'] = title
    out['subtitle'] = ''
    out['image_prompt'] = ''
    ...
    if role == 'cover':
        out['title'] = brand
        out['subtitle'] = _extract_tagline(raw) or profile.get('tagline') or 'Коммерческая презентация'
        cover = _v102_exact_field(raw, ['ключевая фраза для обложки'], 220)
        out['bullets'] = [cover or 'Премиальное решение для вашего пространства']
        out['layout'] = 'cover'; out['image_needed'] = True
    elif role == 'emotional':
        out['layout'] = 'split'; out['image_needed'] = True
        out['bullets'] = _v104_emotional_copy(project)
    elif role == 'problems':
        out['layout'] = 'cards'; out['image_needed'] = False
        out['bullets'] = _v103_section_items(raw, ['проблемы клиента','проблемы'], 6)
    elif role == 'solution':
        out['layout'] = 'split'; out['image_needed'] = True
        out['bullets'] = _v103_section_items(raw, [f'решение {brand}','решение'], 6)
    elif role == 'technology':
        out['layout'] = 'process'; out['image_needed'] = False
        out['bullets'] = _v103_section_items(raw, ['как работает технология','как работает система','основные этапы системы','основные этапы'], 6)
    elif role == 'benefits':
        out['layout'] = 'cards'; out['image_needed'] = False
        out['bullets'] = _v103_section_items(raw, ['ключевые преимущества','преимущества'], 6)
    elif role == 'usage':
        out['layout'] = 'split'; out['image_needed'] = True
        explicit = _v103_section_items(raw, ['варианты использования','сценарии использования'], 6)
        out['bullets'] = explicit or _v103_usage_contexts(raw)
    elif role == 'products':
        out['layout'] = 'comparison'; out['image_needed'] = False
        out['bullets'] = [f"{item['name']} — {item['price']}" for item in offers[:4]]
    elif role == 'service':
        out['layout'] = 'split'; out['image_needed'] = True
        if services:
            service = services[0]
            out['subtitle'] = f"{service['name']} — {service['price']}"
            out['bullets'] = _sanitize_bullets(service.get('details') or [])[:5]
        else:
            out['bullets'] = _v103_section_items(raw, ['сервисное сопровождение','сервисное обслуживание системы','сервисное обслуживание'], 6)
    elif role == 'process':
        out['layout'] = 'process'; out['image_needed'] = False
        out['bullets'] = _v104_group_stages(raw)
    elif role == 'why':
        out['layout'] = 'split'; out['image_needed'] = True
        benefits = _v103_section_items(raw, ['ключевые преимущества','преимущества'], 8)
        priority_tokens = ['индивиду','расчёт','проект','интеграц','монтаж','контрольн','качество','сервис']
        preferred: list[str] = []
        for token in priority_tokens:
            for item in benefits:
                if token in item.lower() and item not in preferred:
                    preferred.append(item); break
        out['bullets'] = _sanitize_bullets(preferred + benefits)[:5]
    elif role == 'cta':
        out['layout'] = 'cta'; out['image_needed'] = True
        out['subtitle'] = _v102_exact_field(raw, ['призыв к действию'], 300) or 'Свяжитесь с нами для консультации'
        note = 'Контакты указаны для тестирования' if re.search(r'контакты указаны для тестирования', raw, re.I) else ''
        out['bullets'] = _sanitize_bullets([*contacts, note])[:5]
    else:
        out['layout'] = out.get('layout') or 'split'
        out['image_needed'] = bool(out.get('image_needed'))
        out['bullets'] = _v104_generic_copy(project, title, base)

    out['bullets'] = [b for b in _sanitize_bullets(out.get('bullets') or []) if _v102_heading_key(b) not in _V103_GENERIC_FORBIDDEN]
    if not out['bullets'] and not out.get('subtitle'):
        out['bullets'] = _v104_generic_copy(project, title, base)
    if out.get('image_needed'):
        out['image_prompt'] = _v103_visual_scene(project, idx + 1, profile.get('visual_direction') or profile.get('product') or '')
    return out


def _v104_repair_slides(project: dict[str, Any], candidates: list[dict[str, Any]] | None = None) -> list[dict[str, Any]]:
    structure = project.get('structure') or []
    result: list[dict[str, Any]] = []
    candidates = candidates or []
    for idx, base in enumerate(structure):
        candidate = candidates[idx] if idx < len(candidates) and isinstance(candidates[idx], dict) else {}
        result.append(_build_deterministic_slide(project, deepcopy(base), deepcopy(candidate), idx))
    return result
...
def _validate_slides_or_raise(project: dict[str, Any], slides: list[dict[str, Any]]) -> None:
    ...
    contacts = _extract_contacts(raw)
    errors: list[str] = []
    if len(slides) != len(structure): errors.append(f'ожидалось {len(structure)} слайдов, получено {len(slides)}')
    if slides and brand and _normalize_exact_brand(slides[0].get('title','')) != brand: errors.append('на обложке изменено точное название бренда')
    for i, s in enumerate(slides, 1):
        text = ' '.join([str(s.get('title') or ''), str(s.get('subtitle') or ''), *map(str, s.get('bullets') or [])])
        if _has_placeholder(text) or 'Информация подготовлена по главному брифу' in text: errors.append(f'слайд {i} содержит техническую заглушку')
        if not (s.get('bullets') or s.get('subtitle')): errors.append(f'слайд {i} не содержит полезного текста')
    comparison = next((s for s in slides if s.get('layout') == 'comparison'), None)
    comparison_text = ' '.join(comparison.get('bullets') or []) if comparison else ''
    for item in products:
        if item['name'] not in comparison_text or item['price'] not in comparison_text:
            errors.append(f"на продуктовом слайде отсутствует {item['name']} / {item['price']}")
    if services:
        service_slide = next((s for s in slides if _v104_role(project, s, s, slides.index(s)) == 'service'), None)
        service_text = ' '.join([str(service_slide.get('subtitle') or ''), *map(str, service_slide.get('bullets') or [])]) if service_slide else ''
        if not service_slide: errors.append('отсутствует сервисный слайд')
        elif services[0]['name'] not in service_text or services[0]['price'] not in service_text: errors.append('на сервисном слайде отсутствует название или цена сервиса')
        elif len(service_slide.get('bullets') or []) < 3: errors.append('сервисный слайд содержит слишком мало услуг')
    if slides:
        final_text = ' '.join([str(slides[-1].get('subtitle') or ''), *map(str, slides[-1].get('bullets') or [])])
        for contact in contacts:
            value = _v103_contact_value(contact)
            if value and value not in final_text: errors.append(f'на финальном слайде отсутствует контакт: {value}')
        if re.search(r'контакты указаны для тестирования', raw, re.I) and 'Контакты указаны для тестирования' not in final_text:
            errors.append('на финальном слайде отсутствует пометка о тестовых контактах')
    project['preflight_report'] = {'ok': not errors, 'errors': errors[-20:], 'products': products, 'service': services[:1], 'contacts': contacts, 'slide_count': len(slides), 'version': 'v104'}
    _save(project['user_id'], project)
    if errors: raise RuntimeError('Предэкспортная проверка: ' + '; '.join(errors[:8]))


async def _generate_deck_content(project: dict[str, Any], update: Update | None = None) -> list[dict[str, Any]]:
    # Deliberately do not call the v102/v103 generator chain: that chain performs a
    # strict global validation before the later repair pass. Critical commercial
    # copy is safer and faster when rebuilt directly from the approved brief.
    slides = _v104_repair_slides(project)
    _validate_slides_or_raise(project, slides)
    project['final_slides'] = slides
    _save(project['user_id'], project)
    return slides
