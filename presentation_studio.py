...
STATE_LABELS = {
    ...
    "awaiting_logo_brief": "ожидает уточнение логотипа",
    "generating_logo": "создаёт варианты логотипа",
...
def _extract_brand_name(brief: str, outline: JsonDict | None = None) -> str:
    """Best-effort brand extraction so logo cards do not use the whole presentation title."""
    brief = _clean_text(brief, 1800)
    outline = outline or {}
    patterns = [
        r"(?:торгов(?:ая|ой)\s+марка|марка|бренд|название|компания|проект)\s*[:—-]?\s*[«\"']?([A-Za-zА-ЯЁ][A-Za-zА-Яа-яЁё0-9._-]{2,40})",
        r"(?:водк[аиу]?|вилл[аиу]?|ресторан[ау]?|отел[ьяю]?|сервис[ау]?|продукт[ау]?)\s+(?:марки\s+)?[«\"']?([A-Za-zА-ЯЁ][A-Za-zА-Яа-яЁё0-9._-]{2,40})",
    ]
    for pat in patterns:
        m = re.search(pat, brief, re.I)
        if m:
            candidate = _clean_text(m.group(1), 40).strip(".,;:—-«»\"'")
            if candidate and candidate.lower() not in {"компания", "бренд", "марка", "проект"}:
                return candidate
    title = _clean_text(outline.get("title") or "", 120)
    # Prefer a distinctive capitalised Latin token, often the actual brand name.
    latin = re.findall(r"\b[A-Z][A-Za-z0-9._-]{2,40}\b", title)
    if latin:
        return latin[-1]
    words = [w for w in re.findall(r"[A-Za-zА-Яа-яЁё0-9._-]+", title) if len(w) > 2]
    return " ".join(words[-2:])[:40] if words else "Brand"


def _text_sensitive_visual(prompt: str) -> bool:
    """True when readable lettering is likely to appear inside a generated image."""
    return bool(re.search(
        r"(label|etiket|packag|bottle|box|poster|menu|sign|screen|document|book|magazine|logo|brand|"
        r"этикет|упаков|бутыл|короб|плакат|меню|вывеск|экран|документ|книг|журнал|логотип|бренд)",
        prompt or "", re.I
    ))
...
def _image_prompts(outline: JsonDict, style_label: str, palette: JsonDict, notes: str, count: int, brand: str = "") -> list[tuple[int, str]]:
    ...
    for idx, slide in enumerate(slides):
        ...
        text_guard = (
            " Any product packaging, bottle, box, sign or label must be completely blank and unprinted: "
            "no letters, no pseudo-letters, no glyphs, no numbers, no fake brand marks. "
            "The approved brand copy will be added later by the presentation renderer."
            if _text_sensitive_visual(vp) else ""
        )
        prompt = (
            ...
            f"{text_guard} "
            f"Brand context for art direction only: {brand or 'not specified'}. "
...
class PresentationStudio:
    ...
    def _auto_logo_brief(self, project: JsonDict, refinement: str = "") -> tuple[str, str]:
        outline = _safe_json_loads(project.get("outline_json"), {})
        brand = _extract_brand_name(project.get("brief") or "", outline)
        brief = (
            f"Brand name: {brand}. "
            f"Project title: {_clean_text(outline.get('title') or project.get('title') or '', 160)}. "
            f"Subtitle: {_clean_text(outline.get('subtitle') or '', 180)}. "
            f"Project summary: {_clean_text(outline.get('summary') or '', 350)}. "
            f"Audience: {_clean_text(outline.get('audience') or '', 220)}. "
            f"Approved user brief: {_clean_text(project.get('brief') or '', 900)}. "
            "Create a distinctive professional identity suitable for the approved presentation/catalog. "
            "The generated image must contain only the symbol; the renderer will add the exact brand name separately."
        )
        if refinement:
            brief += f" User refinement: {_clean_text(refinement, 500)}."
        return brand, brief
    ...
    async def handle_text(self, update: Update, context: ContextTypes.DEFAULT_TYPE, text: str) -> bool:
        ...
        if state == "awaiting_logo_brief":
            # This is an optional refinement only; the default generation no longer asks for a second brief.
            await self._generate_logo_variants(update, context, project, refinement=text)
            return True

        if state == "generating_logo":
            await update.effective_message.reply_text("🎨 Варианты логотипа уже создаются. Дождитесь трёх карточек для выбора.")
            return True
    ...
    async def handle_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
        ...
        if action == "outline_ok":
            ...
            await q.message.reply_text(
                "✅ Структура утверждена. Есть ли у компании готовый логотип?\n\n"
                "Если готового файла нет, я автоматически создам 3 варианта на основе уже утверждённого брифа и проекта — повторно описывать логотип не потребуется.",
        ...
        if action == "logo_generate":
            current = self._fetch_project(pid) or project
            if current.get("state") == "generating_logo" or self._project_lock(pid).locked():
                await q.message.reply_text("🎨 Три варианта уже создаются. Дождитесь завершения — повторный запуск не нужен.")
                return True
            await self._generate_logo_variants(update, context, current)
            return True
        if action == "logo_refine":
            self._update_project(pid, state="awaiting_logo_brief")
            await q.message.reply_text(
                "✏️ Укажите только дополнительные пожелания к логотипу: символ, настроение, цвета или чего избегать. "
                "Основные данные бренда и проекта я возьму из утверждённого брифа."
            )
            return True
        if action == "logo_skip":
    ...
    async def _resume_prompt(self, update: Update, project: JsonDict) -> None:
        ...
        if state == "awaiting_brief":
            ...
            await update.effective_message.reply_text("Укажите дополнительные пожелания к логотипу. Основные данные уже взяты из проекта.")
        elif state == "generating_logo":
            await update.effective_message.reply_text("🎨 Логотипы создавались во время перезапуска. Нажмите кнопку ниже, чтобы безопасно запустить генерацию заново.", reply_markup=self._logo_choice_kb())
    ...
    async def _generate_logo_variants(
        self,
        update: Update,
        ...
        """Generate exactly three selectable logo concepts without leaking partial failures to the user."""
        pid = project["id"]
        lock = self._project_lock(pid)
        if lock.locked():
            await update.effective_message.reply_text("🎨 Варианты логотипа уже создаются. Пожалуйста, дождитесь результата.")
            return

        async with lock:
            self._update_project(pid, state="generating_logo")
            brand, logo_brief = self._auto_logo_brief(self._fetch_project(pid) or project, refinement=refinement)
            prompts = _logo_prompt_variants(logo_brief, brand)
            await update.effective_message.reply_text(
                f"🎨 Создаю 3 варианта логотипа для «{brand}» по утверждённому проекту. "
                "Дополнительный бриф не требуется. Это может занять несколько минут…"
            )

            images: list[bytes] = []
            try:
                generated = await self.image_batch_call(update, context, prompts, "openai", "presentation_logo_variants")
                if generated:
                    images.extend(x for x in generated if isinstance(x, (bytes, bytearray)) and len(x) > 100)
            except Exception as e:
                # Do not let a provider/Telegram exception trigger the global generic “Упс”.
                log.exception("Logo provider batch failed for project %s: %s", pid, e)

            # A presentation project must never get stuck because one provider returned only 1-2 files.
            for variant_idx in range(len(images), 3):
                try:
                    local = self._make_local_logo_variant(brand, logo_brief, variant_idx)
                    if local:
                        images.append(local)
                except Exception as e:
                    log.warning("Local logo fallback failed variant=%s: %s", variant_idx, e)

            if not images:
                self._update_project(pid, state="awaiting_logo_choice")
                await update.effective_message.reply_text(
                    "Не удалось подготовить варианты логотипа. Можно повторить автоматическую генерацию, загрузить свой файл или продолжить без логотипа.",
                    reply_markup=self._logo_choice_kb(),
                )
                return
            ...
            self._clear_assets(pid, ["logo", "logo_variant"])
            ids: list[int] = []
            for idx, raw in enumerate(images):
                try:
                    normalized = self._normalise_logo(bytes(raw), brand, idx)
                    ids.append(self._add_asset(pid, normalized, "logo_variant", "generated", mime="image/png", label=f"Вариант {idx + 1}", prompt=prompts[idx]))
                except Exception as e:
                    log.exception("Logo variant save failed project=%s index=%s: %s", pid, idx, e)

            if not ids:
                self._update_project(pid, state="awaiting_logo_choice")
                await update.effective_message.reply_text("Не удалось сохранить варианты логотипа. Повторите генерацию или загрузите свой файл.", reply_markup=self._logo_choice_kb())
                return

            self._update_project(pid, state="awaiting_logo_selection")
            assets = self._assets(pid, ["logo_variant"], selected_only=False)
            by_id = {a["id"]: a for a in assets}
            sent = 0
            for idx, asset_id in enumerate(ids):
                asset = by_id.get(asset_id)
                if not asset or not os.path.exists(asset.get("file_path") or ""):
                    continue
                try:
                    raw = Path(asset["file_path"]).read_bytes()
                    bio = io.BytesIO(raw); bio.name = f"logo_variant_{idx + 1}.png"
                    markup = InlineKeyboardMarkup([[InlineKeyboardButton(f"✅ Выбрать вариант {idx + 1}", callback_data=f"ps:logo_select:{asset_id}")]])
                    try:
                        await update.effective_message.reply_photo(photo=InputFile(bio), caption=f"Вариант {idx + 1}", reply_markup=markup)
                    except Exception as photo_error:
                        log.warning("Logo reply_photo failed, using document: %s", photo_error)
                        bio.seek(0)
                        await update.effective_message.reply_document(document=InputFile(bio), caption=f"Вариант {idx + 1}", reply_markup=markup)
                    sent += 1
                except Exception as e:
                    # Continue sending the remaining cards. A single Telegram media error must not abort the wizard.
                    log.exception("Logo variant send failed project=%s asset=%s: %s", pid, asset_id, e)

            rows = [
                [InlineKeyboardButton("🔄 Создать ещё 3 автоматически", callback_data="ps:logo_generate")],
                [InlineKeyboardButton("✏️ Уточнить пожелания", callback_data="ps:logo_refine")],
                [InlineKeyboardButton("📤 Загрузить свой логотип", callback_data="ps:logo_upload")],
                [InlineKeyboardButton("⏭ Без логотипа", callback_data="ps:logo_skip")],
            ]
            if sent:
                await update.effective_message.reply_text("Выберите один из вариантов выше. При необходимости можно создать новую тройку или уточнить пожелания.", reply_markup=InlineKeyboardMarkup(rows))
            else:
                await update.effective_message.reply_text("Варианты сохранены, но Telegram не принял изображения. Создайте новую тройку или загрузите свой логотип.", reply_markup=InlineKeyboardMarkup(rows))
    ...
    async def _ask_generation_engine(self, update: Update, project: JsonDict) -> None:
        await update.effective_message.reply_text(
            ...
            "• Midjourney — художественные и премиальные визуалы без надписей.\n"
            "• OpenAI/Comet Images — более точное следование ТЗ.\n\n"
            "Для бутылок, упаковки, вывесок и других объектов с текстом бот создаёт пустую этикетку/поле, "
            "а точное название и цены накладывает уже при сборке PDF и PPTX. Это исключает нечитаемые псевдобуквы.",
            reply_markup=InlineKeyboardMarkup([
    ...
    def _generation_plan(self, project: JsonDict) -> list[tuple[int, str]]:
        ...
        brand = _extract_brand_name(project.get("brief") or "", outline)
        return _image_prompts(outline, style_label, palette, project.get("generation_notes") or "", missing, brand=brand)
    ...
    def _render_pptx(self, outline: JsonDict, style: JsonDict, palette: JsonDict, logo_path: str | None, visuals: list[JsonDict], out_path: str) -> None:
        ...
        slides = outline.get("slides") or []
        brand_name = _extract_brand_name("", outline)
        for idx, item in enumerate(slides):
            ...
            if image_path:
                ...
                if layout == "product" and brand_name:
                    # Readable approved brand tag is rendered by us; never rely on AI-generated label lettering.
                    add_rect(slide, img_x + 0.25, 5.95, min(3.2, 0.9 + len(brand_name) * 0.11), 0.48, surface, radius=True, line=accent)
                    add_text(slide, brand_name, img_x + 0.42, 6.02, 2.75, 0.3, 12, text_c, True)
                text_w = 5.35
            else:
        ...
        for idx, item in enumerate(slides):
            ...
            if idx == 0 or item.get("layout") == "cover":
                ...
                if image_path:
                    ...
                    if item.get("layout") == "product" and brand_name:
                        tag_w = min(170, 48 + len(brand_name) * 6.2)
                        fill(surface); stroke(accent)
                        c.roundRect(img_x + 14, img_y + 14, tag_w, 28, 7, fill=1, stroke=1)
                        fill(text_c); c.setFont(bold, 10)
                        c.drawString(img_x + 24, img_y + 24, brand_name[:28])
                    max_chars = 44
                else:
    ...
    def _logo_choice_kb() -> InlineKeyboardMarkup:
        return InlineKeyboardMarkup([
            [InlineKeyboardButton("📤 Да, загрузить готовый логотип", callback_data="ps:logo_upload")],
            [InlineKeyboardButton("✨ Нет — создать 3 варианта автоматически", callback_data="ps:logo_generate")],
