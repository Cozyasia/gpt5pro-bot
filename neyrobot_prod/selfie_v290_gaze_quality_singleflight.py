# -*- coding: utf-8 -*-
"""V290 camera-gaze, high-resolution native identity and single-flight guard.
...
VERSION = "v290-gaze-hires-singleflight-2026-08-16"
_INSTALLED = False
...
def _work_canvas(raw: bytes, min_long_side: int = 1800) -> bytes:
    ...
    resampling = Image.Resampling.LANCZOS if hasattr(Image, "Resampling") else Image.LANCZOS
    img = img.resize(size, resampling)
    # Conservative micro-contrast only. This improves warp precision without inventing skin.
...
def _preserve_camera_gaze(target_raw: bytes, identity_raw: bytes, log: Any, *, trace: str) -> bytes:
    ...
    The target composition was explicitly generated to look into the camera. We keep
    the source-native cheeks/nose/mouth/skin identity but preserve the target eye
    direction in two small feathered anatomical eye masks. This avoids inheriting an
    unrelated sideways glance from photo #3 without a second generative pass.
    """
    ...
    try:
        ...
        log("AI_SELFIE_V290_GAZE trace=%s status=skip reason=face_detection error=%s", trace, str(exc)[:300])
        return identity_raw

    # Anatomical masks intentionally stay inside the eyelids/iris zone and avoid brows.
    centers = [
    ...
    log(
        "AI_SELFIE_V290_GAZE trace=%s status=applied face=%s eye_masks=2 target_sha=%s identity_sha=%s out_sha=%s",
...
async def _identity_swap(target_crop: bytes, source_crop: bytes, log: Any, *, trace: str) -> tuple[bytes, str]:
    ...
    if safe:
        try:
            ...
            log(
                "AI_SELFIE_V290_HIRES trace=%s stage=prepare target_native=%s target_work=%s source_native=%s source_work=%s gate=%s",
            ...
            # Preserve lens-directed eyes from the composition after identity transfer.
            ...
            if len(candidate) < 1024 or fs.sha(candidate) == fs.sha(target_crop):
                raise RuntimeError("V290 native identity returned unchanged/empty target")

            geometry_ok, geometry_reason = v289._geometry_status(target_crop, candidate, log, trace=trace)
            if not geometry_ok:
                raise RuntimeError(f"V290 catastrophic geometry change: {geometry_reason}")
            log(
                "AI_SELFIE_V290_IDENTITY trace=%s stage=native_primary_success mode=%s target_native=%s source_native=%s work_target=%s work_source=%s out=%s remote_provider=false gaze=camera_target_eyes quality=hires_local",
                trace, meta.get("mode"), fs.dims(target_crop), fs.dims(source_crop), fs.dims(target_hi), fs.dims(source_hi), fs.dims(candidate),
            )
            return candidate, "source_native_face_core_v290_hires_camera_gaze"
        except Exception as exc:
            log(
                "AI_SELFIE_V290_IDENTITY trace=%s stage=native_primary_failed error_type=%s error=%s fallback=remote_stack",
...
async def _generate_singleflight(update: Any, context: Any, scene: str = "") -> bool:
    ...
    if key in _SEEN:
        _log("AI_SELFIE_V290_SINGLEFLIGHT status=duplicate_callback_suppressed user_id=%s key=%s", user_id, key)
        return True
    if user_id and user_id in _ACTIVE_USERS:
        _SEEN[key] = now
        _log("AI_SELFIE_V290_SINGLEFLIGHT status=concurrent_generation_suppressed user_id=%s key=%s", user_id, key)
        return True

    _SEEN[key] = now
    if user_id:
        _ACTIVE_USERS[user_id] = now
    _log("AI_SELFIE_V290_SINGLEFLIGHT status=acquired user_id=%s key=%s", user_id, key)
    try:
        return bool(await _ORIGINAL_GENERATE(update, context, scene))
    finally:
        if user_id:
            _ACTIVE_USERS.pop(user_id, None)
        _log("AI_SELFIE_V290_SINGLEFLIGHT status=released user_id=%s key=%s", user_id, key)
...
def install() -> bool:
    ...
    terminal.TRACE_PREFIX = "AI_SELFIE_V290"
    ...
    _INSTALLED = True
    print(f"[neyrobot-prod] V290 camera gaze + hires identity + singleflight installed version={VERSION}", flush=True)
    return True
