...
def build_scene_prompt(character_title: str, scene: str, mode: CaptureMode) -> str:
    """Legacy identity-first composition used before the Star Selfie rewrite.

    Gemini/Comet must create the celebrity identity from all catalogue references.
    The external Face Swap stage is reserved only for the user's face.
    """
    shot = (
    ...
    return (
        ...
        f"PERSON B IS {character_title} and must remain on image-right. Every CHARACTER reference depicts the same celebrity. "
        "Use all CHARACTER references jointly as a strict multi-reference identity set. Reconstruct one exact consistent person, "
        "not an averaged or generic lookalike. Preserve the celebrity's craniofacial proportions, forehead, hairline, eye spacing, "
        "eyelids, brows, nose bridge and tip, lips, philtrum, jaw angle, chin, ears, beard contour and density, age, build, tattoos "
        "and habitual expression. PERSON B must already be immediately recognizable in the generated base scene. Do not reserve "
        "a placeholder face for PERSON B and do not expect an external celebrity Face Swap. "
