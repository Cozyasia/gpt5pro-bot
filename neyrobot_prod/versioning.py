...
def _install_current_release() -> bool:
    """Install and re-apply v153 without relying on import order."""
    ...
    try:
        ...
        from celebrity_selfie_v153 import install_builder_hook as install_v153_builder

        install_v153()
        install_v153_builder()
...
async def _cmd_version(update: Any, context: Any) -> None:
    ...
    lines = [
        ...
        f"release_overlay={'v153' if release_overlay else 'load-error'}",
...
def _start_runtime_stamper() -> None:
    ...
    threading.Thread(
        target=worker,
        name="neyrobot-version-contract-v153",
        daemon=True,
    ).start()
