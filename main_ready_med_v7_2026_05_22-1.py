...
PATCH_VERSION = "revive-med-v7-2026-05-22"
...
async def on_doc_analyze(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if _is_medical_context(update.effective_user.id, caption, doc.file_name or "file"):
...
async def on_mode_cb(update, context):
    ...
    if data.startswith("mode:"):
        ...
        if key == "medicine":
            ...
            _mode_set(uid, "Работа")
            _mode_track_set(uid, "")
        elif key == "fun":
            _mode_set(uid, "Развлечения")
            _mode_track_set(uid, "")
...
def _is_medical_context(user_id: int, caption_or_text: str = "", filename: str = "") -> bool:
    ...
    return mode == "Медицина" or (track or "").startswith("med_") or bool(_MEDICAL_TERMS_RE.search(combined))
...
async def on_text(
    ...
    if mode == "Медицина" or (track or "").startswith("med_") or _MEDICAL_TERMS_RE.search(text):
...
async def on_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if _is_medical_context(update.effective_user.id, caption, "photo"):
...
async def on_doc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if mt.startswith("image/"):
            ...
            if _is_medical_context(update.effective_user.id, caption, doc.file_name or "image"):
