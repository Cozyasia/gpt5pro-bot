...
PATCH_VERSION = "v35.2-suno-button-fix-2026-07-08"
...
SUNO_MODEL = os.environ.get("SUNO_MODEL", "suno-v4.5").strip() or "suno-v4.5"
SUNO_CREATE_PATH = os.environ.get("SUNO_CREATE_PATH", "/suno/v1/music").strip() or "/suno/v1/music"
SUNO_STATUS_PATH = os.environ.get("SUNO_STATUS_PATH", "/suno/v1/music/{id}").strip() or "/suno/v1/music/{id}"
...
HELP_TEXT = globals().get("HELP_TEXT") or (
    ...
    "Фото/бизнес: оживление через Runway/Kling/Sora 2 без людей, замена лица, удаление/замена фона, удаление водяного знака/надписи, расширение кадра, раскадровка, анализ фото, логотипы, презентации и PDF-каталоги.\n"
    "Документы, голос и музыка: PDF/EPUB/DOCX/TXT/FB2, сводки, конспекты, таблицы, речь ↔ текст, озвучка, песни/минусовки через Suno.\n"
...
START_TEXT = (
    ...
    "• 🔥 *Развлечения* — оживление фото, Reels/Shorts, мини-фильмы, сценарии, замена лица, удаление/замена фона, ретушь, удаление водяных знаков и музыка через Suno.\n"
...
async def on_mode_cb(update, context):
    ...
    if data == "act:fun:music":
        ...
        await _show_suno_help_from_callback(q, context, reply_markup=_mode_kb("fun"))
        return
...
def capability_answer(text: str) -> str | None:
    ...
    if (
        ...
        return (
            "Да, могу запускать генерацию музыки через Suno/CometAPI. "
...
async def _poll_suno_task(update: Update, client: httpx.AsyncClient, headers: dict, task_id: str):
    started = time.time()
    status_paths = [SUNO_STATUS_PATH, "/suno/v1/music/{id}", "/api/v1/task/{id}", "/v1/tasks/{id}"]
    last = ""
...
async def _run_suno_music(update: Update, context: ContextTypes.DEFAULT_TYPE, brief: str):
    ...
    if not brief:
        await update.effective_message.reply_text(_suno_help_text(), reply_markup=main_kb)
        return
    if not SUNO_ENABLED:
    ...
    async def _go():
        await update.effective_message.reply_text("🎵 Запускаю Suno через CometAPI. Обычно трек готовится 1–5 минут…")
        headers = {"Authorization": f"Bearer {SUNO_API_KEY}", "Content-Type": "application/json", "Accept": "application/json"}
        payloads = [
            (SUNO_CREATE_PATH, {"model": SUNO_MODEL, "prompt": brief, "customMode": False, "instrumental": bool(re.search(r"инструментал|instrumental|без вокала|минусов", brief, re.I))}),
            (SUNO_CREATE_PATH, {"model": SUNO_MODEL, "input": {"prompt": brief}}),
            ("/suno/v1/music", {"model": SUNO_MODEL, "prompt": brief}),
            ("/api/v1/task", {"model": SUNO_MODEL, "task_type": "generate_music", "input": {"prompt": brief}}),
            ("/v1/audio/music", {"model": SUNO_MODEL, "prompt": brief}),
        ]
        last_err = ""
...
async def _show_suno_help_from_callback(q, context, reply_markup=None):
    ...
    """
    context.user_data["awaiting_suno_brief"] = True
    text = _suno_help_text()
    try:
        await q.edit_message_text(text, reply_markup=reply_markup or _mode_kb("fun"))
    except Exception as e:
        log.warning("Suno menu edit failed, fallback to reply: %s", e)
        with contextlib.suppress(Exception):
            await q.message.reply_text(text, reply_markup=reply_markup or _mode_kb("fun"))
...
async def on_cb_fun(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "music":
        ...
        await _show_suno_help_from_callback(q, context, reply_markup=_fun_quick_kb())
        return
