...
PATCH_VERSION = "v33.7.2-bg-photoroom-selfie-2026-07-06"
...
PHOTOROOM_EDIT_NEGATIVE_PROMPT = (
    ...
    or "cartoon, illustration, painting, anime, low detail, extra people, extra limbs, distorted face, distorted horizon, surreal background, 3d render"
).strip()
...
PHOTOROOM_INPUT_MAX_SIDE = int(os.environ.get("PHOTOROOM_INPUT_MAX_SIDE", "1600") or 1600)
BG_OUTPUT_MAX_SIDE = int(os.environ.get("BG_OUTPUT_MAX_SIDE", "1600") or 1600)
...
def _build_selfie_background_prompt(kind: str, prompt: str = "") -> str:
    ...
    preset_map = {
        "beach": "a photorealistic beach with sea, shore and sky, as if the person took a real handheld smartphone selfie on the beach",
        "mountains": "a photorealistic mountain landscape, as if the person took a real handheld smartphone selfie in the mountains",
        "nature": "a photorealistic nature setting with greenery, trees or park scenery, as if the person took a real handheld smartphone selfie outdoors",
        "roof": "a photorealistic rooftop or city skyline scene, as if the person took a real handheld smartphone selfie on a roof terrace in the city",
        "custom": user_prompt,
    }
    scene = user_prompt or preset_map.get(kind) or user_prompt or "a photorealistic environment"
    if kind == "custom" and user_prompt:
        scene = user_prompt
    base = (
        "Generate only a new background and keep the original main subject unchanged. "
        "The result must look like a genuine smartphone selfie taken in that location. "
        "Use realistic perspective for a standing portrait selfie, natural eye-level camera angle, "
        "clean composition, realistic lighting that matches the person, subtle depth of field, and natural colors. "
