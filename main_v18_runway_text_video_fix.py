...
PATCH_VERSION = "revive-fix-v18-runway-text-video-fix-2026-05-22"
...
# Text→Video через CometAPI. Luma временно скрыта, поэтому текстовое видео выбирается между Runway / Sora 2 / Kling.
...
HELP_TEXT = globals().get("HELP_TEXT") or (
    ...
    "Можно отправить фото и выбрать: оживить через Runway/Sora/Kling, удалить/заменить фон, расширить кадр, сделать раскадровку или анализ.\n"
...
def _modes_root_text() -> str:
    return (
        ...
        "• Runway / Kling / Sora 2 — видео, Midjourney — изображения\n"
...
def _mode_desc(key: str) -> str:
    if key == "study":
        return (
            ...
            "STT/TTS для голосовых, + Midjourney (иллюстрации) и Runway/Kling/Sora 2 (учебные ролики).\n\n"
    ...
    if key == "work":
        return (
            ...
            "STT/TTS (диктовка/озвучка), + Midjourney (визуалы), Runway/Kling/Sora 2 (презентационные ролики).\n\n"
            "Быстрые действия ниже. Можно написать свободный запрос (например: "
    ...
    if key == "fun":
        return (
            ...
            "Runway/Kling/Sora 2 (короткие видео и оживление фото), STT/TTS (голос и озвучка).\n\n"
...
def _fun_revive_help_text() -> str:
    return (
        ...
        "Рекомендация: Runway — лучший вариант для стабильного оживления портретов и референсов; Kling — для динамики; Sora 2 — если канал в Comet доступен."
    )
...
def _fun_film_help_text() -> str:
    return (
        ...
        "3) сцены генерируются короткими фрагментами 5–10 сек через Runway/Kling/Sora 2, затем склеиваются.\n\n"
        "Рекомендация: Runway — для контроля и качества, Kling — для динамичных сцен, Sora 2 — если доступен через ваш Comet-канал."
    )
...
async def on_cb_fun(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "fun:clip":
        return await q.edit_message_text("Пришли текст/голос и формат (Reels/Shorts), музыку/стиль — соберу клип (Runway/Kling/Sora 2).")
...
def _photo_revival_capability_text() -> str:
    return (
        ...
        "Как запустить:\n"
        "1) загрузите фото;\n"
        "2) нажмите кнопку ✨ Оживить (Runway / Sora 2 / Kling);\n"
...
def capability_answer(text: str) -> str | None:
    ...
    if (
        ...
        return (
            "Да, могу оживить фотографию и сделать из неё короткое видео. "
            "Загрузите фото — я покажу кнопки Runway, Sora 2 и Kling. "
    ...
    if (
        ...
        return (
            ...
            "После этого я покажу выбор движка: Runway, Sora 2 или Kling. "
    ...
    try:
        ...
        if data.startswith("engine:"):
            ...
            if engine == "luma" and LUMA_TEMP_DISABLED:
                await q.edit_message_text("⚠️ Luma временно отключена и скрыта из меню. Используйте Runway, Sora 2, Kling, Images или другие доступные движки.")
                return
            ...
            if engine in ("gpt", "stt_tts", "midjourney", "sora", "kling"):
                await q.edit_message_text(
                    ...
                    f"Для видео напишите: «создай видео … 5 секунд 16:9» — я предложу Runway, Sora 2 или Kling."
                )
                return
            ...
            if ok:
                await q.edit_message_text(
                    ...
                    ("Запустите: /img кот в очках" if engine == "images"
                     else "Напишите: «сделай видео … 9 секунд 9:16» — предложу доступный видеодвижок.")
                )
                return
        ...
        if data.startswith("pedit:"):
            ...
            if data in ("pedit:revive", "pedit:revive_runway", "pedit:revive_luma", "pedit:revive_sora", "pedit:revive_kling"):
                ...
                if engine == "luma" and LUMA_TEMP_DISABLED:
                    await q.message.reply_text("⚠️ Luma временно отключена и скрыта из меню. Используйте Runway, Sora 2 или Kling.")
                    return
        ...
        if data.startswith("choose:"):
            ...
            if engine == "luma" and LUMA_TEMP_DISABLED:
                ...
                await q.edit_message_text("⚠️ Luma временно отключена. Выберите другой движок, например Runway.")
                return
...
async def cmd_diag_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    lines = [
        ...
        f"• Нормализация duration: Runway/Kling 5 или 10 сек; Sora 10 или 15 сек; Luma временно скрыта",
...
# ───────── Runway video ─────────
def _runway_model_candidates() -> list[str]:
    """
    Возвращает список моделей для попыток Runway.
    Сначала берём ENV, затем актуальные варианты Runway API.
    """
    out: list[str] = []
    for m in (RUNWAY_MODEL, "gen4.5", "gen4_turbo"):
        m = (m or "").strip()
        if m and m not in out:
            out.append(m)
    return out or ["gen4.5"]
...
async def _run_runway_video(update: Update, context: ContextTypes.DEFAULT_TYPE, prompt: str, duration_s: int, aspect: str):
    ...
    if RUNWAY_USE_COMET and COMET_API_KEY:
        ...
        for model in _runway_model_candidates():
    ...
    details = "\n".join(all_errors[-6:]) or "API не вернул подробности."
...
async def _start_photo_revival(update: Update, context: ContextTypes.DEFAULT_TYPE, engine: str, img_bytes: bytes, prompt: str = ""):
    ...
    if engine == "luma" and LUMA_TEMP_DISABLED:
        await update.effective_message.reply_text("⚠️ Luma временно отключена и скрыта из меню. Используйте Runway, Sora 2 или Kling.")
        return
...
async def _run_comet_text_video(update: Update, context: ContextTypes.DEFAULT_TYPE, engine: str, prompt: str, duration_s: int, aspect: str):
    ...
    await update.effective_message.reply_text("❌ Неизвестный text→video движок. Доступны Runway, Sora 2 и Kling.")
...
async def on_text(
    ...
    if context.user_data.get("awaiting_reels_material"):
        ...
        prompt = (
            ...
            "3) текст на экране, 4) voice-over, 5) CTA, 6) промпты для Runway/Kling/Sora, "
    ...
    if context.user_data.get("awaiting_film_material"):
        ...
        prompt = (
            ...
            "через Runway/Kling/Sora, 5) монтаж/звук/титры, 6) план сборки. Запрос пользователя:\n" + text
        )
        reply = await ask_openai_text(prompt)
        await update.effective_message.reply_text(reply[:3900], reply_markup=main_kb)
        if len(reply) > 3900:
    ...
    if mtype == "video":
        ...
        await update.effective_message.reply_text(
            ...
            f"Запрос: «{prompt}»\n\n"
            f"Luma временно скрыта; доступны Runway, Sora 2 и Kling.",
            reply_markup=kb,
        )
        return
...
async def on_mode_work_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    txt = (
        ...
        "_Гибриды:_ GPT-5 (текст/логика) + Images (иллюстрации) + Runway/Kling/Sora 2 (клипы/мокапы).\n\n"
        "_Быстрые действия:_\n"
