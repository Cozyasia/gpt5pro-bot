...
PATCH_VERSION = "v53-full-v35.6-plus-avatar-photoclip-2026-07-09"
...
async def on_cb_fun(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "avatar":
        ...
        if img:
            _set_avatar_wait(context)
            await q.message.reply_text("🗣 Использую последнее загруженное фото. Пришлите текст, voice или аудиофайл для речи аватара.")
        else:
            context.user_data["awaiting_avatar_photo"] = True
            await q.edit_message_text("🗣 Пришлите портрет человека. После загрузки нажмите 🗣 Говорящий аватар или отправьте текст для речи.", reply_markup=_fun_quick_kb())
        return
    ...
    if action == "photoclip":
        ...
        if img:
            _set_photo_clip_wait(context)
            await q.message.reply_text("🎵 Использую последнее загруженное фото. Опишите стиль видеоклипа: музыка, движение, настроение, длительность и формат.")
        else:
            context.user_data["awaiting_photo_clip_photo"] = True
            await q.edit_message_text("🎵 Пришлите фото человека. После загрузки нажмите 🎵 Фото → видеоклип или отправьте описание клипа.", reply_markup=_fun_quick_kb())
        return
