...
PATCH_VERSION = "revive-fun-med-v11-2026-05-22"
...
def _mode_desc(key: str) -> str:
    ...
    if key == "fun":
        return (
            ...
            "Runway/Luma/Kling/Sora 2 (короткие видео и оживление фото), STT/TTS (голос и озвучка).\n\n"
...
def _should_route_medical(context: ContextTypes.DEFAULT_TYPE, user_id: int, caption_or_text: str = "", filename: str = "") -> bool:
    ...
    return (
        (track or "").startswith("med_")
        or bool(context and context.user_data.get("medicine_waiting_for_material"))
        or bool(_MEDICAL_TERMS_RE.search(combined))
    )
...
def _fun_revive_help_text() -> str:
    return (
        ...
        "2) после загрузки появятся кнопки ✨ Оживить: Runway / Luma / Sora 2 / Kling;\n"
        "3) либо сразу отправьте фото с подписью: `оживи фото: лёгкая улыбка, плавное движение камеры, 5 секунд, 9:16`.\n\n"
        "Рекомендация: Runway — лучший вариант для стабильного оживления портретов и референсов; "
        "Luma — для более кинематографичного движения; Kling — для динамики; Sora 2 — если канал в Comet доступен."
    )
...
def _fun_reels_help_text() -> str:
    return (
        ...
        "• для AI-вставок лучше использовать Runway/Kling, для кинематографичных B-roll — Luma, Sora 2 — только если доступен канал провайдера.\n\n"
...
def _fun_film_help_text() -> str:
    return (
        ...
        "3) сцены генерируются короткими фрагментами 5–10 сек через Runway/Kling/Luma/Sora 2, затем склеиваются.\n\n"
        "Рекомендация: Runway — для контроля и качества, Kling — для динамичных сцен, Luma — для плавной cinematic-картинки, Sora 2 — если доступен через ваш Comet-канал."
    )
...
_MEDICAL_TERMS_RE = re.compile(
    r"(медицин|медкарт|карта|выписк|анамнез|анализ(?:ы|ов)?|результат(?:ы)?\s+анализ|"
    r"заключени|врачебн|диагноз|лабораторн|снимок|рентген|узи|мрт|кт|mri|ct|scan|"
    r"томограф|исследовани|эпикриз|назначени|рекомендаци(?:и|й)|blood\s*test|medical)",
    re.I,
)
...
def _is_medical_context(user_id: int, caption_or_text: str = "", filename: str = "") -> bool:
    ...
    with contextlib.suppress(Exception):
        track = _mode_track_get(user_id)
    combined = f"{caption_or_text or ''} {filename or ''}"
    return (track or "").startswith("med_") or bool(_MEDICAL_TERMS_RE.search(combined))
...
async def _reply_video_from_url(update: Update, client: httpx.AsyncClient, url: str, caption: str):
    """
    Отправляет результат именно видеофайлом.
    Если CDN/подписанная ссылка не скачивается с первого раза — пробуем redirects,
    потом просим Telegram забрать URL как video. Ссылку показываем только последним fallback.
    """
    headers = {
    ...
    # 1) Надёжный вариант: скачать в память и отправить InputFile.
    try:
        ...
        log.warning("reply_video_from_url: local download/send failed: %s", e)

    # 2) Иногда Telegram сам умеет забрать публичный https URL, даже если наш download не прошёл.
    try:
        await update.effective_message.reply_video(
            video=url,
            caption=caption,
    ...
    # 3) Запасной вариант — документ по URL.
    try:
...
async def _run_comet_i2v(update: Update, context: ContextTypes.DEFAULT_TYPE, engine: str, img_bytes: bytes, prompt: str, duration_s: int, aspect: str):
    ...
    if engine == "sora":
        ...
        # Comet/Sora /v1/videos НЕ принимает старые поля:
        #   - aspect_ratio -> Unknown parameter: 'aspect_ratio'
        #   - duration     -> Unknown parameter: 'duration'
        #   - input_image  -> Unknown parameter: 'input_image'
        # Поэтому для image→video используем только input_reference и совместимые fallback-форматы.
        # seconds отправляем строкой, как в videos API; size вместо aspect_ratio.
        size = "720x1280"
        if (aspect or "").strip() == "16:9":
            size = "1280x720"
        elif (aspect or "").strip() == "1:1":
            size = "1024x1024"

        payloads = []

        def _add(payload: dict):
            # защита от случайного возвращения запрещённых параметров
            payload.pop("input_image", None)
            payload.pop("duration", None)
            payload.pop("aspect_ratio", None)
            payloads.append((SORA_CREATE_PATH, payload))

        # 1) Comet в ваших логах явно ожидал input_reference как строку.
        # Сначала пробуем публичную Telegram URL-ссылку, потом data-uri.
        if tg_url and tg_url.startswith("https://"):
            _add({"model": SORA_MODEL, "prompt": prompt, "input_reference": tg_url, "seconds": str(d), "size": size})
            _add({"model": SORA_MODEL, "prompt": prompt, "input_reference": tg_url, "seconds": str(d)})

        _add({"model": SORA_MODEL, "prompt": prompt, "input_reference": data_url, "seconds": str(d), "size": size})
        _add({"model": SORA_MODEL, "prompt": prompt, "input_reference": data_url, "seconds": str(d)})

        # 2) Официальная JSON-форма OpenAI videos API использует объект input_reference.
        # У некоторых Comet-каналов это может быть уже включено; если нет — пойдём дальше.
        if tg_url and tg_url.startswith("https://"):
            _add({"model": SORA_MODEL, "prompt": prompt, "input_reference": {"image_url": tg_url}, "seconds": str(d), "size": size})
            _add({"model": SORA_MODEL, "prompt": prompt, "input_reference": {"image_url": tg_url}, "seconds": str(d)})

        _add({"model": SORA_MODEL, "prompt": prompt, "input_reference": {"image_url": data_url}, "seconds": str(d), "size": size})
        _add({"model": SORA_MODEL, "prompt": prompt, "input_reference": {"image_url": data_url}, "seconds": str(d)})

        # 3) Дополнительные совместимые aliases без input_image.
        if tg_url and tg_url.startswith("https://"):
            _add({"model": SORA_MODEL, "prompt": prompt, "image_url": tg_url, "seconds": str(d), "size": size})
            _add({"model": SORA_MODEL, "prompt": prompt, "image_urls": [tg_url], "seconds": str(d), "size": size})

        _add({"model": SORA_MODEL, "prompt": prompt, "image_url": data_url, "seconds": str(d), "size": size})
        _add({"model": SORA_MODEL, "prompt": prompt, "image_urls": [data_url], "seconds": str(d), "size": size})

        # 4) Последний image→video fallback: без seconds/size, если канал сам выбирает параметры.
        if tg_url and tg_url.startswith("https://"):
            _add({"model": SORA_MODEL, "prompt": prompt, "input_reference": tg_url})
        _add({"model": SORA_MODEL, "prompt": prompt, "input_reference": data_url})
...
async def _run_runway_animate_photo(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, prompt: str, duration_s: int, aspect: str):
    ...
    try:
        ...
        payload_variants = [
            (RUNWAY_I2V_PATH, {"model": RUNWAY_MODEL, "promptImage": image_ref, "promptText": prompt, "duration": seconds, "ratio": ratio}),
            (RUNWAY_I2V_PATH, {"model": RUNWAY_MODEL, "prompt_image": image_ref, "prompt_text": prompt, "duration": seconds, "ratio": ratio}),
            (RUNWAY_CREATE_PATH, {"model": RUNWAY_MODEL, "input": {"prompt": prompt, "duration": seconds, "ratio": ratio, "init_image": image_ref}}),
        ]
...
async def on_text(
    ...
    if context.user_data.get("awaiting_reels_material"):
        ...
        prompt = (
            ...
            "3) текст на экране, 4) voice-over, 5) CTA, 6) промпты для Runway/Kling/Luma, "
    ...
    if context.user_data.get("awaiting_film_material"):
        ...
        prompt = (
            ...
            "через Runway/Kling/Luma/Sora, 5) монтаж/звук/титры, 6) план сборки. Запрос пользователя:\n" + text
        )
        reply = await ask_openai_text(prompt)
        await update.effective_message.reply_text(reply[:3900], reply_markup=main_kb)
        if len(reply) > 3900:
    ...
    if (track or "").startswith("med_") or _MEDICAL_TERMS_RE.search(text):
        # Если это вопрос о возможности — уже ответили выше. Иначе разбираем как мед. вопрос.
...
async def on_doc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if mt.startswith("image/"):
            ...
            if _is_waiting_photo_revival(context):
                _clear_photo_revival_wait(context)
                await update.effective_message.reply_text("Изображение получено как документ. Выберите движок для оживления:", reply_markup=photo_quick_actions_kb())
                return
