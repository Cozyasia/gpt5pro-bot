...
PATCH_VERSION = "v85-presentation-studio-2026-07-13"
...
# Presentation/Catalog Studio v85
...
# ───────── Presentation / Catalog Studio v85 integration ─────────
...
async def _presentation_paid_runner(
    ...
    """Uses the v83/v84 reservation ledger and returns the action result after successful charge."""
    box = {"result": None}
...
async def _presentation_image_batch(
    ...
    async def _action():
        result: list[bytes] = []
        for idx, (prompt, selected_engine) in enumerate(zip(prompts, resolved_engines)):
            img = None
            try:
                if selected_engine == "midjourney":
                    ...
                    if not img:
                        img = await _comet_generate_image_bytes(prompt)
                    if not img and MIDJOURNEY_ENABLED and COMET_API_KEY:
                        with contextlib.suppress(Exception):
                            img, _task = await _midjourney_generate_image_bytes(prompt)
            except Exception as e:
...
async def on_text(
    ...
    # Legacy presentation flags from older deployments are migrated into the v85 studio.
