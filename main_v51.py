...
PATCH_VERSION = "v51-comet-bg-channel-diagnostics-2026-06-04"
...
# v51: Comet-first provider with strict route control.
# Default route is OpenAI-compatible /v1/responses. Replicate/Bria-compatible
# routes are disabled unless explicitly enabled in ENV.
...
COMET_BG_REPLICATE_MODEL = os.environ.get("COMET_BG_REPLICATE_MODEL", "bria/remove-background").strip() or "bria/remove-background"
COMET_BG_REPLICATE_PATH = os.environ.get(
    "COMET_BG_REPLICATE_PATH",
    f"/replicate/v1/models/{COMET_BG_REPLICATE_MODEL}/predictions"
).strip() or f"/replicate/v1/models/{COMET_BG_REPLICATE_MODEL}/predictions"
...
COMET_BG_STATUS_PATHS = _env_list("COMET_BG_STATUS_PATHS", "/v1/responses/{id}")
...
COMET_BG_ALLOW_REPLICATE = os.environ.get("COMET_BG_ALLOW_REPLICATE", "0").strip().lower() in ("1", "true", "yes", "on")
COMET_ALLOW_BRIA_COMPAT_ROUTES = os.environ.get("COMET_ALLOW_BRIA_COMPAT_ROUTES", "0").strip().lower() in ("1", "true", "yes", "on")
COMET_BG_MODEL_CANDIDATES = _env_list(
    "COMET_BG_MODEL_CANDIDATES",
    f"{COMET_BG_MODEL},replicate/bria/remove-background"
)
...
# Bria Increase Resolution via CometAPI (upscaling 2x/4x, alpha-preserving).
COMET_UPSCALE_MODEL = os.environ.get("COMET_UPSCALE_MODEL", "bria/increase-resolution").strip() or "bria/increase-resolution"
COMET_UPSCALE_REPLICATE_MODEL = os.environ.get("COMET_UPSCALE_REPLICATE_MODEL", "bria/increase-resolution").strip() or "bria/increase-resolution"
COMET_UPSCALE_REPLICATE_PATH = os.environ.get(
    "COMET_UPSCALE_REPLICATE_PATH",
    f"/replicate/v1/models/{COMET_UPSCALE_REPLICATE_MODEL}/predictions"
).strip() or f"/replicate/v1/models/{COMET_UPSCALE_REPLICATE_MODEL}/predictions"
COMET_UPSCALE_PATHS = _env_list(
    "COMET_UPSCALE_PATHS",
    "/v1/responses"
)
COMET_UPSCALE_STATUS_PATHS = _env_list(
    "COMET_UPSCALE_STATUS_PATHS",
    "/v1/responses/{id}"
)
COMET_UPSCALE_TIMEOUT_S = float(os.environ.get("COMET_UPSCALE_TIMEOUT_S", "240") or 240)
COMET_UPSCALE_MAX_WAIT_S = float(os.environ.get("COMET_UPSCALE_MAX_WAIT_S", "240") or 240)
COMET_UPSCALE_POLL_DELAY_S = float(os.environ.get("COMET_UPSCALE_POLL_DELAY_S", "3") or 3)
COMET_UPSCALE_DEFAULT_FACTOR = int(os.environ.get("COMET_UPSCALE_DEFAULT_FACTOR", "2") or 2)
if COMET_UPSCALE_DEFAULT_FACTOR not in (2, 4):
    COMET_UPSCALE_DEFAULT_FACTOR = 2
COMET_UPSCALE_USE_TELEGRAM_URL = os.environ.get("COMET_UPSCALE_USE_TELEGRAM_URL", "0").strip().lower() in ("1", "true", "yes", "on")
COMET_UPSCALE_DEBUG_TO_CHAT = os.environ.get("COMET_UPSCALE_DEBUG_TO_CHAT", "0").strip().lower() in ("1", "true", "yes", "on")
COMET_UPSCALE_ALLOW_REPLICATE = os.environ.get("COMET_UPSCALE_ALLOW_REPLICATE", "0").strip().lower() in ("1", "true", "yes", "on")
COMET_UPSCALE_MODEL_CANDIDATES = _env_list(
    "COMET_UPSCALE_MODEL_CANDIDATES",
    f"{COMET_UPSCALE_MODEL},replicate/bria/increase-resolution"
)
...
def photo_quick_actions_kb():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔎 Улучшить 2x/4x", callback_data="pedit:upscale")],
...
def _bg_runtime_status() -> str:
    ...
    parts.append("models=" + ",".join(COMET_BG_MODEL_CANDIDATES[:4]))
    ...
    parts.append("replicate=" + ("on" if COMET_BG_ALLOW_REPLICATE else "off"))
    parts.append("bria_compat=" + ("on" if COMET_ALLOW_BRIA_COMPAT_ROUTES else "off"))
    return "; ".join(parts)
...
async def _comet_image_get_status(client: httpx.AsyncClient, rid: str, api_key: str, status_paths: list[str]) -> dict | None:
    if not rid:
        return None
    ...
    for path in status_paths:
        try:
            ...
            log.warning("Comet image status error %s: %s", path, e)
    return None

async def _comet_bg_get_status(client: httpx.AsyncClient, rid: str, api_key: str) -> dict | None:
    return await _comet_image_get_status(client, rid, api_key, COMET_BG_STATUS_PATHS)

def _comet_bg_request_candidates(img_bytes: bytes, image_url: str = "") -> list[tuple[str, dict]]:
    """Build robust candidates for Comet Bria background removal.

    v51 order:
    1) Comet Responses route /v1/responses with model candidates.
    2) Replicate-compatible route only when COMET_BG_ALLOW_REPLICATE=1.
    3) Bria-compatible /bria/image/edit/* only when COMET_ALLOW_BRIA_COMPAT_ROUTES=1.

    This prevents Render ENV defaults or stale render.yaml from silently forcing
    unsupported relay routes.
    """
    mime = sniff_image_mime(img_bytes)
    b64 = base64.b64encode(img_bytes).decode("ascii")
    ...
    # 1) OpenAI-compatible route: POST /v1/responses with input_image.
    if any(p == "/v1/responses" for p in COMET_BG_REMOVE_PATHS):
        for model_name in COMET_BG_MODEL_CANDIDATES:
            ...
            for img in image_inputs:
                candidates.append((
                    "/v1/responses",
                    {"model": model_name, "prompt": "Remove the background from this image and return a transparent PNG.", "image": img}
                ))

    # 2) Replicate-compatible Comet route. Enabled only by COMET_BG_ALLOW_REPLICATE.
    replicate_paths = [p for p in COMET_BG_REMOVE_PATHS if "/replicate/v1/models/" in p]
    if COMET_BG_ALLOW_REPLICATE and COMET_BG_REPLICATE_PATH not in replicate_paths:
        replicate_paths.insert(0, COMET_BG_REPLICATE_PATH)
    if not COMET_BG_ALLOW_REPLICATE:
        replicate_paths = []
    ...
    # 3) Bria-compatible edit route: POST /bria/image/edit/{action}. Disabled by default.
    bria_payloads = []
    if image_url and COMET_BG_USE_TELEGRAM_URL:
    ...
    if COMET_ALLOW_BRIA_COMPAT_ROUTES:
        for path in COMET_BG_REMOVE_PATHS:
            if path in {"/v1/responses"} or "/replicate/v1/models/" in path or path.endswith("/remove-bg") or path.endswith("/remove_background"):
                continue
...
def _upscale_factor_from_text(text: str, default: int = None) -> int:
    """Parse 2x/4x from caption or return configured default."""
    if default is None:
        default = COMET_UPSCALE_DEFAULT_FACTOR
    tl = (text or "").lower().replace("×", "x")
    if re.search(r"\b4\s*x\b|\b4\s*крат", tl):
        return 4
    if re.search(r"\b2\s*x\b|\b2\s*крат", tl):
        return 2
    return 4 if int(default) == 4 else 2


def _upscale_runtime_status() -> str:
    return (
        f"COMET_UPSCALE_MODEL={COMET_UPSCALE_MODEL}; "
        f"models={','.join(COMET_UPSCALE_MODEL_CANDIDATES[:4])}; "
        f"paths={','.join(COMET_UPSCALE_PATHS[:3])}; "
        f"status_paths={','.join(COMET_UPSCALE_STATUS_PATHS[:3])}; "
        f"factor_default={COMET_UPSCALE_DEFAULT_FACTOR}; "
        f"replicate={'on' if COMET_UPSCALE_ALLOW_REPLICATE else 'off'}; "
        f"bria_compat={'on' if COMET_ALLOW_BRIA_COMPAT_ROUTES else 'off'}; "
        f"Comet={'on' if bool(COMET_API_KEY or COMET_BRIA_API_KEY) else 'missing'}"
    )


def _upscale_safe_factor(img_bytes: bytes, requested: int) -> tuple[int, str]:
    """Bria allows 2x/4x with max output dimension up to 8192 px. Downgrade or reject safely."""
    factor = 4 if int(requested or 2) == 4 else 2
    if Image is None:
        return factor, ""
    try:
        im = Image.open(BytesIO(img_bytes))
        w, h = im.size
        if w * factor <= 8192 and h * factor <= 8192:
            return factor, ""
        if factor == 4 and w * 2 <= 8192 and h * 2 <= 8192:
            return 2, f"Запрошено 4x, но исходник {w}×{h}; 4x превысит лимит 8192 px по стороне. Использую 2x."
        return 0, f"Исходник {w}×{h}; даже 2x превысит лимит 8192 px по стороне. Уменьшите фото или используйте меньший исходник."
    except Exception:
        return factor, ""


def _comet_upscale_request_candidates(img_bytes: bytes, image_url: str = "", factor: int = 2) -> list[tuple[str, dict]]:
    """Build candidates for Comet bria/increase-resolution.

    The Replicate schema exposes: image, image_url, desired_increase, preserve_alpha.
    Comet's catalog also advertises a Bria-compatible endpoint
    /bria/image/edit/increase-resolution, so we try both Comet relay modes.
    """
    factor = 4 if int(factor or 2) == 4 else 2
    mime = sniff_image_mime(img_bytes)
    b64 = base64.b64encode(img_bytes).decode("ascii")
    data_url = f"data:{mime};base64,{b64}"
    image_inputs: list[tuple[str, str]] = []
    if image_url and COMET_UPSCALE_USE_TELEGRAM_URL:
        image_inputs.append(("url", image_url))
    image_inputs.append(("data_url", data_url))
    image_inputs.append(("base64", b64))

    candidates: list[tuple[str, dict]] = []

    # 1) OpenAI-compatible Responses route.
    if any(p == "/v1/responses" for p in COMET_UPSCALE_PATHS):
        for model_name in COMET_UPSCALE_MODEL_CANDIDATES:
            for kind, img in image_inputs:
                # input_image works best with URL/data URL. Raw base64 is kept as a compatibility fallback only.
                image_value = img if kind != "base64" else data_url
                candidates.append((
                    "/v1/responses",
                    {
                        "model": model_name,
                        "input": [{
                            "role": "user",
                            "content": [
                                {"type": "input_text", "text": f"Increase the image resolution by {factor}x. Preserve original content and alpha transparency. Return the upscaled image."},
                                {"type": "input_image", "image_url": image_value},
                            ],
                        }],
                    }
                ))
                candidates.append(("/v1/responses", {"model": model_name, "image": image_value, "desired_increase": factor, "preserve_alpha": True}))

    # 2) Replicate-compatible route. Enabled only by COMET_UPSCALE_ALLOW_REPLICATE.
    replicate_paths = [p for p in COMET_UPSCALE_PATHS if "/replicate/v1/models/" in p]
    if COMET_UPSCALE_ALLOW_REPLICATE and COMET_UPSCALE_REPLICATE_PATH not in replicate_paths:
        replicate_paths.insert(0, COMET_UPSCALE_REPLICATE_PATH)
    if not COMET_UPSCALE_ALLOW_REPLICATE:
        replicate_paths = []
    for path in replicate_paths:
        for kind, img in image_inputs:
            key = "image_url" if kind == "url" else "image"
            candidates.append((path, {"input": {key: img, "desired_increase": factor, "preserve_alpha": True}}))
            # Compatibility with providers that expect desired_increase as string.
            candidates.append((path, {"input": {key: img, "desired_increase": str(factor), "preserve_alpha": True}}))

    # 3) Comet Bria-compatible route. Disabled by default.
    if COMET_ALLOW_BRIA_COMPAT_ROUTES:
        for path in COMET_UPSCALE_PATHS:
            if path == "/v1/responses" or "/replicate/v1/models/" in path:
                continue
            for kind, img in image_inputs:
                if kind == "url":
                    candidates.append((path, {"image_url": img, "desired_increase": factor, "preserve_alpha": True, "sync": True}))
                    candidates.append((path, {"image_url": img, "resolution_multiplier": factor, "preserve_alpha": True, "sync": True}))
                else:
                    candidates.append((path, {"image": img, "desired_increase": factor, "preserve_alpha": True, "sync": True}))
                    candidates.append((path, {"image": img, "resolution_multiplier": factor, "preserve_alpha": True, "sync": True}))

    # De-duplicate payloads while preserving order.
    dedup: list[tuple[str, dict]] = []
    seen = set()
    for path, payload in candidates:
        sig = (path, json.dumps(payload, sort_keys=True, ensure_ascii=False)[:1000])
        if sig not in seen:
            seen.add(sig); dedup.append((path, payload))
    return dedup


async def _comet_upscale_bytes(img_bytes: bytes, image_url: str = "", factor: int = 2) -> bytes | None:
    keys = _comet_bg_key_candidates()
    if not keys:
        return None
    last_err = None
    factor = 4 if int(factor or 2) == 4 else 2
    candidates = _comet_upscale_request_candidates(img_bytes, image_url=image_url or "", factor=factor)
    async with httpx.AsyncClient(timeout=COMET_UPSCALE_TIMEOUT_S, follow_redirects=True) as client:
        for api_key in keys:
            headers = {"Authorization": f"Bearer {api_key}", "Content-Type": "application/json", "Prefer": "wait"}
            for path, payload in candidates:
                try:
                    ...
                    if isinstance(data, dict):
                        ...
                        status_url = str(data.get("status_url") or data.get("poll_url") or data.get("urls", {}).get("get") or "")
                    start = time.time()
                    while rid and time.time() - start < COMET_UPSCALE_MAX_WAIT_S:
                        await asyncio.sleep(COMET_UPSCALE_POLL_DELAY_S)
                        sd = None
                        if status_url:
                            try:
                                rr = await client.get(status_url, headers=headers)
                                ...
                                sd = None
                        if sd is None:
                            sd = await _comet_image_get_status(client, rid, api_key, COMET_UPSCALE_STATUS_PATHS)
                        if not sd:
                            continue
                        img = _extract_image_url_from_json(sd)
                        if img:
                    ...
                    continue
    if last_err:
        log.warning("Comet increase-resolution failed: %s", last_err)
        try:
            globals()["_LAST_COMET_UPSCALE_ERROR"] = last_err
        except Exception:
            pass
    return None


def _upscale_factor_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("🔎 Улучшить 2x", callback_data="pedit:upscale2"), InlineKeyboardButton("🔬 Улучшить 4x", callback_data="pedit:upscale4")],
        [InlineKeyboardButton("⬅️ Фото", callback_data="nav:photo")],
    ])


async def _pedit_upscale_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = (
        "🔎 Улучшение разрешения через Comet Bria `bria/increase-resolution`.\n\n"
        "2x — безопасный вариант для большинства фото.\n"
        "4x — сильнее увеличивает, но может упереться в лимит 8192 px по стороне.\n"
        "Содержимое не перерисовывается, alpha/прозрачность сохраняется при наличии."
    )
    if getattr(update, "callback_query", None) and update.callback_query.message:
        await update.callback_query.message.reply_text(msg, reply_markup=_upscale_factor_menu_kb(), parse_mode="Markdown")
    else:
        await update.effective_message.reply_text(msg, reply_markup=_upscale_factor_menu_kb(), parse_mode="Markdown")


async def _pedit_upscale(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, factor: int = None):
    requested = factor or COMET_UPSCALE_DEFAULT_FACTOR
    safe_factor, warning = _upscale_safe_factor(img_bytes, requested)
    if safe_factor <= 0:
        await update.effective_message.reply_text(f"❌ Не удалось запустить upscale. {warning}")
        return
    user_id = update.effective_user.id if update.effective_user else 0
    img_url = _get_cached_photo_url(user_id) if user_id else ""
    with contextlib.suppress(Exception):
        await context.bot.send_chat_action(update.effective_chat.id, ChatAction.UPLOAD_DOCUMENT)
    preface = f"🔎 Запускаю улучшение разрешения через Comet Bria: {safe_factor}x."
    if warning:
        preface += "\n" + warning
    await update.effective_message.reply_text(preface)
    out = await _comet_upscale_bytes(img_bytes, image_url=img_url, factor=safe_factor)
    if not out:
        diag = _upscale_runtime_status()
        last = globals().get("_LAST_COMET_UPSCALE_ERROR", "")
        extra = f"\n\nПоследняя ошибка Comet: {str(last)[:900]}" if COMET_UPSCALE_DEBUG_TO_CHAT and last else ""
        await update.effective_message.reply_text(
            "Не удалось улучшить разрешение через Comet. Проверьте доступность модели bria/increase-resolution и COMET_API_KEY.\n"
            f"Диагностика: {diag}{extra}"
        )
        return
    bio = BytesIO(out)
    bio.seek(0)
    # Если модель вернула JPEG/WebP, Telegram всё равно примет документ; имя делаем безопасным PNG.
    bio.name = f"upscaled_{safe_factor}x.png"
    await update.effective_message.reply_document(InputFile(bio), caption=f"✅ Готово: разрешение увеличено через Comet Bria {safe_factor}x.")
...
async def _pedit_removebg(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes):
    try:
        ...
        if not out:
            ...
            await update.effective_message.reply_text(
                "Не удалось удалить фон через Comet. Comet ответил, что канал/модель недоступны или текущий route не поддерживается. Проверьте доступность модели bria/remove-background в кабинете Comet и корректность COMET_API_KEY.\n"
                f"Диагностика: {diag}{extra}"
            )
            return
...
async def _pedit_replacebg(update: Update, context: ContextTypes.DEFAULT_TYPE, img_bytes: bytes, preset: str = "blur"):
    ...
    try:
        ...
        await update.effective_message.reply_text(
            "Не удалось заменить фон через Comet. Сначала нужно получить alpha PNG через bria/remove-background, но Comet не вернул результат. Проверьте доступность модели/канала в Comet.\n"
...
async def on_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if data.startswith("pedit:"):
            ...
            if data == "pedit:upscale":
                await _pedit_upscale_menu(update, context); return
            if data == "pedit:upscale2":
                await _pedit_upscale(update, context, img, factor=2); return
            if data == "pedit:upscale4":
                await _pedit_upscale(update, context, img, factor=4); return
...
async def cmd_diag_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    lines = [
        ...
        f"  Bria BG={COMET_BG_MODEL} paths={','.join(COMET_BG_REMOVE_PATHS[:2])}",
        f"  Bria Upscale={COMET_UPSCALE_MODEL} paths={','.join(COMET_UPSCALE_PATHS[:2])}",
...
async def on_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    try:
        ...
        if caption:
            ...
            # улучшить разрешение / upscale
            if any(k in tl for k in ("upscale", "апскейл", "увеличь разреш", "увеличить разреш", "улучши качество", "улучшить качество", "increase resolution", "повысить разреш", "повысить качество")):
                await _pedit_upscale(update, context, img, factor=_upscale_factor_from_text(caption)); return
...
def _photo_menu_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🔎 Улучшить 2x/4x", callback_data="pedit:upscale"), InlineKeyboardButton("🧽 Ретушь", callback_data="pedit:retouch")],
        [InlineKeyboardButton("🖼 Обложка", callback_data="creative:cover")],
...
def photo_quick_actions_kb():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🌈 Замена фона", callback_data="pedit:replacebg"), InlineKeyboardButton("🔎 Улучшить", callback_data="pedit:upscale")],
        [InlineKeyboardButton("👁 Анализ", callback_data="pedit:vision")],
    ])
