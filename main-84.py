...
PATCH_VERSION = "v86-logo-runway-resilience-2026-07-13"
...
# v86: official Runway first for text→video; Comet is a secondary route and Kling is the production fallback.
...
async def cmd_diag_runway(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "reset" in args:
        ...
        await update.effective_message.reply_text("✅ Runway provider cooldown сброшен для image→video и text→video.")
        return
    ...
    lines = [
        ...
        f"• direct_key: {'✅' if bool(RUNWAY_API_KEY) else '❌'}  direct_first={'✅' if RUNWAY_DIRECT_FIRST else '—'}  text_model={RUNWAY_TEXT_MODEL}",
...
def _runway_direct_text_model_candidates() -> list[str]:
    """Official Runway text→video models. Gen-4.5 supports text-only input by omitting promptImage."""
    return _dedupe_models(RUNWAY_TEXT_MODEL, "gen4.5")
...
async def _run_runway_video(update: Update, context: ContextTypes.DEFAULT_TYPE, prompt: str, duration_s: int, aspect: str):
    ...
    Official Runway Gen-4.5 uses POST /v1/image_to_video without promptImage for text-only video.
    A Comet 503/model_not_found/no-available-channel is provider availability, so it is logged,
    put on cooldown, and never shown to a normal user as a raw JSON wall.
    """
    await context.bot.send_chat_action(update.effective_chat.id, ChatAction.RECORD_VIDEO)
    prompt = (prompt or "").strip()
    if not prompt:
    ...
    # Gen-4.5 documented ratios. Square/other requests use the nearest stable landscape/portrait ratio.
    ratio = "720:1280" if (aspect or "").strip() in {"9:16", "3:4", "4:5"} else "1280:720"
    errors: list[str] = []
        ...
        async with httpx.AsyncClient(timeout=90.0) as client:
            for base in _runway_direct_base_candidates():
                for model in _runway_direct_text_model_candidates():
                    ...
                    try:
                        r = await client.post(f"{base}/v1/image_to_video", headers=headers, json=payload)
                        if r.status_code >= 400:
                            err = f"Direct Runway {r.status_code}: {_api_error_preview(r)}"
                        ...
                        if not task_id:
                            errors.append(f"Direct Runway: no task id: {json.dumps(js, ensure_ascii=False)[:500]}")
                            continue
                        await update.effective_message.reply_text("⏳ Runway: задача принята, ожидаю результат…")
                        return bool(await _poll_video_task_generic(
                            update, client, headers, base, ["/v1/tasks/{id}", RUNWAY_STATUS_PATH],
                            task_id, "Runway text→video", RUNWAY_MAX_WAIT_S,
                        ))
                    except Exception as e:
                        err = f"Direct Runway exception: {e}"
                        errors.append(err); log.warning(err)
        return False
            ...
            for model in _runway_comet_text_model_candidates():
                ...
                try:
                    ...
                    if ready_url:
                        ...
                        await _reply_video_from_url(update, client, ready_url, "Runway/Comet text→video ✅")
                        return True
    ...
    if RUNWAY_HIDE_TECH_ERRORS:
        await update.effective_message.reply_text(
            ...
            "Попробуйте Kling либо добавьте официальный RUNWAY_API_KEY в Render."
        )
    else:
