...
PATCH_VERSION = "v39-comet-bria-cutout-2026-05-25"
...
COMET_BRIA_REMOVEBG_PATHS = [
    p.strip() for p in os.environ.get(
        "COMET_BRIA_REMOVEBG_PATHS",
        "/bria/image/edit/remove-background,/bria/image/edit/remove_bg,/bria/remove-background"
    ).split(",") if p.strip()
]
COMET_BRIA_STATUS_PATHS = [
    p.strip() for p in os.environ.get("COMET_BRIA_STATUS_PATHS", "/bria/{id},/bria/image/edit/{id}").split(",") if p.strip()
]
...
async def _v39_comet_bria_cutout(img_bytes: bytes, *, timeout_s: float | None = None) -> tuple[bytes, tuple[int, int]]:
    ...
    # Comet Bria-compatible docs say image URL/base64 depending endpoint usage. We try the common variants.
    json_payloads = [
        {"image": data_url, "format": COMET_BRIA_OUTPUT_FORMAT},
