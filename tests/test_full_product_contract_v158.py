...
This suite is deliberately static and secret-free: it prevents an unrelated
feature patch from deleting menus, payment wiring or production entrypoints.
Provider calls remain covered by manual staging tests because CI has no secrets.
"""
...
class FullProductContractV158Tests(unittest.TestCase):
    ...
    def test_work_mode_keeps_documents_logo_watermark_and_exports(self):
        self.assert_tokens(MAIN, [
            ...
            "act:work:plan",
            "act:work:idea",
            "act:work:presentation",
            "act:work:catalog_pdf",
            "act:work:logo",
            "act:work:watermark",
            "PDF + PPTX",
        ])
    ...
    def test_entertainment_mode_has_exact_required_fifteen_actions(self):
        ...
        menu = MAIN[MAIN.index('if key == "fun":'):MAIN.index('if key == "medicine":', MAIN.index('if key == "fun":'))]
